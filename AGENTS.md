...
- 当前活动任务：`T-P1A-043`（已完成 Wave 3B ledger open）；Wave 3B 已 open；next gate = `T-P1A-044`（PR #69，Shoulders Probe Reports）；DB vNext remains candidate-only / not SRD-v3 promoted authority / not migration approval / not runtime approval；Phase 2A migration dry-run plan 仍需 user 显式 gate；详见 `docs/current.md` / `docs/task-index.md`。
