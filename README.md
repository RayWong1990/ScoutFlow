...
- 当前焦点：Wave 1 ledger 已收口；当前无 active product task；Active count=`0/3`，Review count=`0`
- 当前不做：workers / frontend / 浏览器自动化 / 真实下载 / 真实 BBDown 执行 / 真实 `BBDown -info` / QR/manual auth / yt-dlp runtime / ffmpeg runtime / ASR / `audio_transcript` runtime / 06 自动启动
...
- 当前 active tasks：无；下一 gate 候选 `T-P1A-011 live no-auth BBDown -info probe` 仍是 `gated / requires explicit user approval`
