...
- 当前焦点：`T-P1A-009` BBDown local-only runtime spike report candidate；`T-P1A-008` BBDown sanitized fixture parser 已通过 PR `#17` 合入 `main`
- 当前不做：workers / frontend / 浏览器自动化 / 真实下载 / yt-dlp runtime / ffmpeg runtime / ASR / `audio_transcript` runtime；BBDown 仅限 `--version` 与一次已批准 no-auth `-info` metadata probe
...
- 当前无 active product task；`T-P1A-009` 处于 review/report-candidate
- `T-P1A-008` 仅表示 fixture-only parser / classifier baseline merged，不批准 broader BBDown runtime、媒体下载、ffmpeg、ASR、workers 或 frontend；`T-P1A-009` 只批准 `BBDown --version` 与一次 no-auth `BBDown -info` metadata probe
