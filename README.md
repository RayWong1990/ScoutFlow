...
- 当前焦点：`metadata_only only` 的 API-side quick_capture contract
- 当前不做：workers / frontend / 浏览器自动化 / `audio_transcript` runtime
...
- `services/api/**` 与 `tests/**` 允许进入首个 Phase 1A code-bearing task
- GitHub queue / sync smoke / Codex adapter 只作为通信测试，当前已暂停或关闭
- 当前仅批准 `T-P1A-001` 的最小 `metadata_only` contract 实现；不等于批准 broader Phase 1A runtime
