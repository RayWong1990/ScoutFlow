...
function buildRuntime(overrides: Partial<W2CRuntimeValue> = {}): W2CRuntimeValue {
  return {
    ...
    bridgeHealth: asyncState("idle"),
    bridgeVaultConfig: asyncState("idle"),
...
describe("LiveMetadata", () => {
  it("keeps queued metadata distinct from loaded metadata when no readback evidence exists", () => {
    runtimeHolder.value = buildRuntime();

    render(<LiveMetadata />);

    expect(screen.getByText("等待元数据证据")).toBeTruthy();
    expect(screen.getByText("metadata_fetch 已排队，但当前路由只证明 enqueue，不等于元数据已加载。")).toBeTruthy();
    expect(screen.getByText("future-gated / not routed")).toBeTruthy();
  });
});
