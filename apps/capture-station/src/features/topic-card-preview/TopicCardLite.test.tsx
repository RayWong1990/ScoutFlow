...
import TopicCardLite from "./TopicCardLite";
  ...
  return {
    ...
    trustTrace: {
      ...
      data: {
        ...
        receipt_ledger: { present: true, artifact_count: 2, artifact_kinds: ["safe_metadata_evidence", "metadata_probe_summary"], redaction: "applied" },
        media_audio: { status: "not_approved", audio_transcript: "blocked" },
        audit: { platform_result: null, evidence_file_path: null, artifact_count: 2, redaction_policy: null, safe_parsed_fields: { title: "ScoutFlow BV1xK4y1f7yC", duration: "12:34" } },
      },
      error: null,
    },
...
describe("TopicCardLite", () => {
  it("shows current capture context when available", () => {
    render(<TopicCardLite />);

    expect(screen.getByText("状态: 当前 capture 上下文")).toBeTruthy();
    expect(screen.getAllByText("ScoutFlow BV1xK4y1f7yC").length).toBeGreaterThan(0);
    expect(screen.getByText("metadata_fetch queued")).toBeTruthy();
  });

  it("keeps future-gated lanes explicit", () => {
    render(<TopicCardLite />);

    expect(screen.getByText("评论 / ASR / 多卡排序仍待后续 contract")).toBeTruthy();
    expect(screen.getByText("safe_metadata_evidence")).toBeTruthy();
  });
});
