...
last_refreshed_from_main_pr: 252
last_refreshed_from_main_sha: 02ccbdc
status_words_locked:
...
- **当前 baseline**: 见 §1 auto-managed 真态锚点 (`git log -1 origin/main` + authority docs); capture-station 已 landed 基线 = 13 surface 真接 existing route + 5 态 state machine + Trust Trace data shell (graph/timeline/error-path 留 W1B)
...
| main HEAD | `02ccbdc` (PR #252) ← `a15b948` (PR #251) ← `cd9d866` (PR #250) |
