...
- `T-P1A-156 / W2C` 已 closed (PR #252 merge `02ccbdc`)；W2C surface 13 个真接已落地，仅作历史 reference，不再开新 code branch；后续任何 capture-station 改动需新 dispatch + 新 Active row
...
- `T-P1A-156 / W2C` 已 closed；后续 W1B trust-trace graph/timeline/error-path 真实现 / 任何 capture-station 改动 / 任何 services/api 改动均需新 dispatch + 新 Active row + 外审，不得借 W2C closeout 默认延伸
...
- authority writer 当前空闲 `0/1`；后续并行 lane 仍受 `docs/specs/parallel-execution-protocol.md` 约束；W1B trust-trace graph/timeline/error-path 真实现 / W3E PF-C0-O1 first cluster / W4 5 lane authority-upgrade spec 仍是后续候选 next-wave，但需新 dispatch + 新 Active row + 外审
