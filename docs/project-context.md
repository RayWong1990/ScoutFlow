...
- user 的主要瓶颈是跨工具复制粘贴、人审节点和推进确认。
- 项目默认采用 GitHub 外部审计与单主多审模式。
- 当前工具栈中 `Codex Desktop` 是主写入工具，网页版 `GPT Pro` 是外部审计与 prompt 派单工具。
- `OpenClaw` / `Hermes` / `Claude` 可以参与 sidecar 审读或研究。
- 所有 sidecar 输出不得直接成为主线事实，必须经主写入窗口合并和 user 审批。
