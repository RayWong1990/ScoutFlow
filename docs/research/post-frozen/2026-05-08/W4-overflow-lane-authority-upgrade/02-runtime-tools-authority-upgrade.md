---
title: Runtime Tools Authority-Upgrade Roadmap
status: candidate
...
# Runtime Tools Authority-Upgrade Roadmap

> 本文产出的是 runtime_tools 的 candidate authority-upgrade roadmap。它不运行 BBDown / yt-dlp / ffmpeg / ASR, 也不把 bounded evidence 解释成 live runtime gate。
...
推荐路径: vendor diversification + ASR benchmark + Apple Silicon acceptance + sandbox spec → Hermes → explicit user gate → bounded runtime canary。

1. 先做模型/adapter 选择, 避免先接工具再补安全边界。
2. 再做 subprocess sandbox, 明确 stdout/stderr redaction 与 failure classification。
3. 然后用小样本 canary 验证: source metadata, audio extraction, ASR, cost, redaction。
4. closeout 时只说 canary evidence, 不泛化成全 runtime 体系完成。
...
- TL;DR: 建立 Whisper.cpp Metal / FunASR / Paraformer-large / cloud ASR 的 benchmark schema, 不运行 live runtime。
- 输入包:
...
- 验收标准:
  - 模型矩阵含质量、速度、成本、Apple Silicon、privacy
  - benchmark evidence 字段定义清楚
  - 不写真实执行命令
- 反模式 REGENERATE:
...
- TL;DR: 把 BBDown、yt-dlp、fallback vendor 的优先级、legal risk、capability matrix 写清, 降低单 vendor 风险。
- 输入包:
  - https://raw.githubusercontent.com/RayWong1990/ScoutFlow/main/docs/COLLECTION-LINE-MASTER-SPEC-2026-05-07.md
  - https://raw.githubusercontent.com/RayWong1990/ScoutFlow/main/docs/00-START-HERE.md
  - https://raw.githubusercontent.com/RayWong1990/ScoutFlow/main/docs/current.md
...
- 验收标准:
  ...
  - BBDown cease-and-desist 风险以 candidate risk 写法出现
...
- 验收标准:
  ...
  - stdout/stderr redaction rule 明确
- 反模式 REGENERATE:
...
### §5.3 §1 Mission
在 runtime_tools 通过 spec/audit/gate 后, 以 bounded canary 方式验证 vendor adapter、media extraction、ASR、redaction、cost ledger 与 process sandbox。
### §5.4 §2 Inputs
...
- Phase 1: ASR/vendor matrix contract
- Phase 2: Apple Silicon benchmark acceptance
- Phase 3: subprocess sandbox boundary
- Phase 4: bounded runtime canary evidence
- Phase 5: closeout with limitations
