---
title: write_enabled scan（bridge/vault true-write 扫描）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-BND-01
...
[candidate procedure] 本 runbook 把 `write_enabled scan（bridge/vault true-write 扫描）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `write_enabled, vault true write, dry_run` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `critical`。风险来自：不得把 preview/dry-run 当 true write；不得改配置解禁。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：write_enabled, vault true write, dry_run, commit。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得把 preview/dry-run 当 true write；不得改配置解禁。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[operator step] S1. 把场景改写成一句可审计任务：扫描 vault/bridge 相关输出是否仍保持 write_enabled=false、dry_run=true、committed=false。
...
[operator step] S8. 把禁止项写入任务头：不得把 preview/dry-run 当 true write；不得改配置解禁。
...
[lesson] L6. dry-run envelope 的存在是安全特性，不是下一步自动 true write 的理由。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: P4-BND-BND-WRITE-ENABLED, P4-BND-HARD-REDLINES

[linked reference] A2. linked_tools: grep write_enabled, bridge/config.py, vault/commit.py, PR diff
...
[footer boundary] RB-BND-01 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
...
[scenario triage] `RB-BND-01` 的最小可执行判断不是“这个场景是否有价值”，而是“在 `write_enabled scan（bridge/vault true-write 扫描）` 中，当前输入是否精确命中 `write_enabled / vault true write / dry_run / commit`，并且没有触发更高风险 lane”。把 redline 当成独立审计面：先证明未触碰，再讨论功能是否完成；grep 结果只是一层证据。 这张卡用于开工前 60 秒 readback：说清楚 source、owner、gate、evidence sink 与 stop condition；说不清楚时，状态保持 hold。
