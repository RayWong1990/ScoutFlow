---
title: Post-merge integrity check（origin/main SHA / authority untouched / hard redlines）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-BND-08
...
[candidate procedure] 本 runbook 把 `Post-merge integrity check（origin/main SHA / authority untouched / hard redlines）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `post-merge, origin/main, SHA` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `high`。风险来自：不得只看 PR merged 标签；不得跳过 authority/readback。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：post-merge, origin/main, SHA, integrity, redlines。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得只看 PR merged 标签；不得跳过 authority/readback。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[operator step] S1. 把场景改写成一句可审计任务：PR 合并后复核 origin/main SHA、authority surface、hard redline、CI status 与 expected diff shape。
...
[operator step] S8. 把禁止项写入任务头：不得只看 PR merged 标签；不得跳过 authority/readback。
...
[lesson] L6. 合并不是收口；post-merge readback 才能证明真实主线状态。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: P4-BND-BND-POST-MERGE, P4-BND-HARD-REDLINES

[linked reference] A2. linked_tools: git fetch, git rev-parse origin/main, git diff, workflow status
...
[footer boundary] RB-BND-08 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
...
[scenario triage] `RB-BND-08` 的最小可执行判断不是“这个场景是否有价值”，而是“在 `Post-merge integrity check（origin/main SHA / authority untouched / hard redlines）` 中，当前输入是否精确命中 `post-merge / origin/main / SHA / integrity / redlines`，并且没有触发更高风险 lane”。把 redline 当成独立审计面：先证明未触碰，再讨论功能是否完成；grep 结果只是一层证据。 这张卡用于开工前 60 秒 readback：说清楚 source、owner、gate、evidence sink 与 stop condition；说不清楚时，状态保持 hold。
