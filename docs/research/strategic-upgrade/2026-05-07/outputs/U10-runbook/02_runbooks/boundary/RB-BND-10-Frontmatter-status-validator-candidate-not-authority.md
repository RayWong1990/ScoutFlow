---
title: Frontmatter status validator（candidate / not-authority）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-BND-10
...
linked_dispatch:
  - P4-BND-BND-FRONTMATTER
linked_pr:
  - none
linked_tools:
  - frontmatter parser
  - YAML scan
  - candidate status grep
linked_skill:
  - boundary-audit
linked_rule:
...
[candidate procedure] 本 runbook 把 `Frontmatter status validator（candidate / not-authority）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `frontmatter, status, candidate` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `medium`。风险来自：不得让 filename 或目录名代替 frontmatter；不得缺少 risk_level。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：frontmatter, status, candidate, not-authority, validator。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得让 filename 或目录名代替 frontmatter；不得缺少 risk_level。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[operator step] S1. 把场景改写成一句可审计任务：检查候选文件是否显式标注 candidate / not-authority / not execution approval。
...
[operator step] S8. 把禁止项写入任务头：不得让 filename 或目录名代替 frontmatter；不得缺少 risk_level。
...
[lesson] L6. 候选口径必须写在文件里；否则下一窗口容易把草稿当事实。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: P4-BND-BND-FRONTMATTER

[linked reference] A2. linked_tools: frontmatter parser, YAML scan, candidate status grep
...
[footer boundary] RB-BND-10 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
...
[scenario triage] `RB-BND-10` 的最小可执行判断不是“这个场景是否有价值”，而是“在 `Frontmatter status validator（candidate / not-authority）` 中，当前输入是否精确命中 `frontmatter / status / candidate / not-authority / validator`，并且没有触发更高风险 lane”。把 redline 当成独立审计面：先证明未触碰，再讨论功能是否完成；grep 结果只是一层证据。 这张卡用于开工前 60 秒 readback：说清楚 source、owner、gate、evidence sink 与 stop condition；说不清楚时，状态保持 hold。

[failure mode] 本场景风险等级为 `medium`，需要优先防止“把候选 SOP 当执行批准”的漂移。最危险的捷径是用“没有明显关键词”替代 authority scan，或忽略 frontmatter wording 中的隐性批准语。 复盘时要把失败分成三类：前置证据不足、步骤越界、验证口径过宽；不要笼统写成“模型理解有误”。
