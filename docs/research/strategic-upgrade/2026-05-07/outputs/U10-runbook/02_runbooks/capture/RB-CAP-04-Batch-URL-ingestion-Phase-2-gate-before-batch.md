---
title: Batch URL ingestion（Phase 2 gate before batch）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-CAP-04
...
[candidate procedure] 本 runbook 把 `Batch URL ingestion（Phase 2 gate before batch）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `batch, URL list, 批量导入` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `high`。风险来自：不得把 batch list 当作多个单条 quick capture 自动展开。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：batch, URL list, 批量导入, Phase 2, capture plan。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得把 batch list 当作多个单条 quick capture 自动展开。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[precondition] P6. 本 runbook 的直接场景是：用户给出多条 URL 或 URL 列表时，先走 capture_plan 与 batch gate，而不是逐条偷跑 quick capture。 这条前置条件必须在执行前被 readback；如果当前环境无法验证，就在 ledger 中写 `unverified_in_current_environment`，不要补成肯定事实。
...
[operator step] S1. 把场景改写成一句可审计任务：用户给出多条 URL 或 URL 列表时，先走 capture_plan 与 batch gate，而不是逐条偷跑 quick capture。
...
[operator step] S8. 把禁止项写入任务头：不得把 batch list 当作多个单条 quick capture 自动展开。
...
[lesson] L6. 高吞吐会制造完成感；先证明列表来源、去重、边界，再允许并发。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: P2-CAP-CAP-BATCH-GATE, P4-BND-HARD-REDLINES, P3-DSP-LANE-GUARD

[linked reference] A2. linked_tools: capture_plan draft, dedupe table, scope approval checklist
...
[footer boundary] RB-CAP-04 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
