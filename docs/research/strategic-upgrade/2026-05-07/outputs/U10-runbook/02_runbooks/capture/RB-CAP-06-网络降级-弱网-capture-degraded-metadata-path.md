---
title: 网络降级 / 弱网 capture（degraded metadata path）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-CAP-06
...
[candidate procedure] 本 runbook 把 `网络降级 / 弱网 capture（degraded metadata path）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `弱网, network degraded, timeout` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `medium`。风险来自：不得在超时后追加未 redacted stdout；不得扩大重试次数导致平台风险。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：弱网, network degraded, timeout, offline, 429。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得在超时后追加未 redacted stdout；不得扩大重试次数导致平台风险。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[precondition] P6. 本 runbook 的直接场景是：网络不稳定时收缩到预检、URL canonicalize、排队记录与 later retry，不让半截数据进入 durable path。 这条前置条件必须在执行前被 readback；如果当前环境无法验证，就在 ledger 中写 `unverified_in_current_environment`，不要补成肯定事实。
...
[operator step] S1. 把场景改写成一句可审计任务：网络不稳定时收缩到预检、URL canonicalize、排队记录与 later retry，不让半截数据进入 durable path。
...
[operator step] S8. 把禁止项写入任务头：不得在超时后追加未 redacted stdout；不得扩大重试次数导致平台风险。
...
[lesson] L6. 弱网不是降格安全要求的理由；它只允许降级执行范围。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: P2-CAP-CAP-WEAK-NET

[linked reference] A2. linked_tools: timeout budget, retry budget, job_events, redaction scan
...
[footer boundary] RB-CAP-06 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
