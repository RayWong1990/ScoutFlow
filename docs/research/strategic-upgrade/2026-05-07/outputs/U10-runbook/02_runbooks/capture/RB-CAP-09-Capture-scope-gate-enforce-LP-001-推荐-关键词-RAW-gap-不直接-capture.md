---
title: Capture scope gate enforce（LP-001 推荐/关键词/RAW gap 不直接 capture）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-CAP-09
...
linked_dispatch:
  - P2-CAP-CAP-SCOPE-GATE
  - P4-BND-HARD-REDLINES
linked_pr:
  - none
linked_tools:
  - scope classifier
  - capture_plan candidate
  - decision-log question list
linked_skill:
  - capture-scope-gate
linked_rule:
...
[candidate procedure] 本 runbook 把 `Capture scope gate enforce（LP-001 推荐/关键词/RAW gap 不直接 capture）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `LP-001, recommendation, keyword` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `high`。风险来自：不得直接创建 capture，不得把用户想法等同 URL approval。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：LP-001, recommendation, keyword, RAW gap, scope gate。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得直接创建 capture，不得把用户想法等同 URL approval。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[precondition] P6. 本 runbook 的直接场景是：当输入来自推荐信号、关键词扫描、RAW 缺口或 topic idea 时，先转成 signal/hypothesis/capture_plan candidate。 这条前置条件必须在执行前被 readback；如果当前环境无法验证，就在 ledger 中写 `unverified_in_current_environment`，不要补成肯定事实。
...
[operator step] S1. 把场景改写成一句可审计任务：当输入来自推荐信号、关键词扫描、RAW 缺口或 topic idea 时，先转成 signal/hypothesis/capture_plan candidate。
...
[operator step] S8. 把禁止项写入任务头：不得直接创建 capture，不得把用户想法等同 URL approval。
...
[lesson] L6. LP-001 是防止系统从研究流量滑进采集 runtime 的第一道闸。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: P2-CAP-CAP-SCOPE-GATE, P4-BND-HARD-REDLINES

[linked reference] A2. linked_tools: scope classifier, capture_plan candidate, decision-log question list
...
[footer boundary] RB-CAP-09 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
...
[scenario triage] `RB-CAP-09` 的最小可执行判断不是“这个场景是否有价值”，而是“在 `Capture scope gate enforce（LP-001 推荐/关键词/RAW gap 不直接 capture）` 中，当前输入是否精确命中 `LP-001 / recommendation / keyword / RAW gap / scope gate`，并且没有触发更高风险 lane”。把 URL、平台、preset 与 evidence sink 分开确认；任何 recommendation、keyword、RAW gap 都先降级成 signal，不直接进入 capture。 这张卡用于开工前 60 秒 readback：说清楚 source、owner、gate、evidence sink 与 stop condition；说不清楚时，状态保持 hold。
