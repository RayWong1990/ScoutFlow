---
title: Single dispatch 派给 Codex（主写入 / PR owner）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-DSP-01
...
[candidate procedure] 本 runbook 把 `Single dispatch 派给 Codex（主写入 / PR owner）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `Codex, single dispatch, PR owner` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `medium`。风险来自：不得让 Codex 绕过 ledger、不得让 review-only 任务变成 authority edit。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：Codex, single dispatch, PR owner, 主写入。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得让 Codex 绕过 ledger、不得让 review-only 任务变成 authority edit。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[precondition] P7. 本 runbook 的直接场景是：把一个明确 scoped、可验证、边界清楚的任务交给 Codex 作为主写入者。 这条前置条件必须在执行前被 readback；如果当前环境无法验证，就在 ledger 中写 `unverified_in_current_environment`，不要补成肯定事实。
...
[operator step] S1. 把场景改写成一句可审计任务：把一个明确 scoped、可验证、边界清楚的任务交给 Codex 作为主写入者。
...
[operator step] S8. 把禁止项写入任务头：不得让 Codex 绕过 ledger、不得让 review-only 任务变成 authority edit。
...
[lesson] L6. Codex 适合主写入，但越强的写入工具越需要 preflight 和 rollback。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: P3-DSP-DSP-CODEX-SINGLE, P3-DSP-LANE-GUARD

[linked reference] A2. linked_tools: docs/dispatch-template.md, git branch, pytest/npm validation, PR diff
...
[footer boundary] RB-DSP-01 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
