---
title: 3-window cloud audit（Codex / GPT Pro / Hermes 三方外审）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-DSP-06
...
[candidate procedure] 本 runbook 把 `3-window cloud audit（Codex / GPT Pro / Hermes 三方外审）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `3-window audit, cloud audit, Codex` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `medium`。风险来自：不得让三份报告相互投票替代事实源；PR diff/workflow 才是 high-risk truth。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：3-window audit, cloud audit, Codex, GPT Pro, Hermes。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得让三份报告相互投票替代事实源；PR diff/workflow 才是 high-risk truth。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[precondition] P7. 本 runbook 的直接场景是：对 material task 同时收集主写入、外审、反驳窗口意见，再合成 single operator decision。 这条前置条件必须在执行前被 readback；如果当前环境无法验证，就在 ledger 中写 `unverified_in_current_environment`，不要补成肯定事实。
...
[operator step] S1. 把场景改写成一句可审计任务：对 material task 同时收集主写入、外审、反驳窗口意见，再合成 single operator decision。
...
[operator step] S8. 把禁止项写入任务头：不得让三份报告相互投票替代事实源；PR diff/workflow 才是 high-risk truth。
...
[lesson] L6. 多窗口提升 coverage，但 final synthesis 必须标注 canonical fact / candidate inference / unsupported。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: P3-DSP-DSP-THREE-WINDOW-AUDIT, P3-DSP-LANE-GUARD

[linked reference] A2. linked_tools: audit brief, claim reconciliation table, PR diff, workflow run
...
[footer boundary] RB-DSP-06 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
...
[scenario triage] `RB-DSP-06` 的最小可执行判断不是“这个场景是否有价值”，而是“在 `3-window cloud audit（Codex / GPT Pro / Hermes 三方外审）` 中，当前输入是否精确命中 `3-window audit / cloud audit / Codex / GPT Pro / Hermes`，并且没有触发更高风险 lane”。先决定 owner、lane、allowed_paths、forbidden_paths 与 validation command，再派模型；多窗口只解决吞吐，不替代 authority。 这张卡用于开工前 60 秒 readback：说清楚 source、owner、gate、evidence sink 与 stop condition；说不清楚时，状态保持 hold。

[failure mode] 本场景风险等级为 `medium`，需要优先防止“把候选 SOP 当执行批准”的漂移。最危险的捷径是让两个窗口同时写同一 authority surface，或者把模型自行扩展的 silent flexibility 当作默认修正。 复盘时要把失败分成三类：前置证据不足、步骤越界、验证口径过宽；不要笼统写成“模型理解有误”。
