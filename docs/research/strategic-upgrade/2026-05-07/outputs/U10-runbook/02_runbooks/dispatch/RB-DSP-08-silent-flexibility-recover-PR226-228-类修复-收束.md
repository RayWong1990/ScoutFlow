---
title: silent flexibility recover（PR226-228 类修复/收束）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-DSP-08
cluster: Dispatch / Multi-Agent
trigger_keywords:
  - silent flexibility recover
  - scope repair
  - PR226
  - PR227
  - PR228
...
[candidate procedure] 本 runbook 把 `silent flexibility recover（PR226-228 类修复/收束）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `silent flexibility recover, scope repair, PR226` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `high`。风险来自：不得把越界产物直接保留为 authority；不得事后补一句“按用户意图”。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：silent flexibility recover, scope repair, PR226, PR227, PR228。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得把越界产物直接保留为 authority；不得事后补一句“按用户意图”。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[precondition] P7. 本 runbook 的直接场景是：当已经发生 silent scope drift 时，用 repair note、supersede、rollback 或 narrower PR 收束。 这条前置条件必须在执行前被 readback；如果当前环境无法验证，就在 ledger 中写 `unverified_in_current_environment`，不要补成肯定事实。
...
[operator step] S1. 把场景改写成一句可审计任务：当已经发生 silent scope drift 时，用 repair note、supersede、rollback 或 narrower PR 收束。
...
[operator step] S8. 把禁止项写入任务头：不得把越界产物直接保留为 authority；不得事后补一句“按用户意图”。
...
[lesson] L6. 恢复不是责备模型，而是把事实链切回可验证、可回退、可解释。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: P3-DSP-DSP-SILENT-RECOVER, P4-BND-HARD-REDLINES, P3-DSP-LANE-GUARD

[linked reference] A2. linked_tools: repair note, supersede protocol, git revert, audit finding
...
[footer boundary] RB-DSP-08 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
...
[scenario triage] `RB-DSP-08` 的最小可执行判断不是“这个场景是否有价值”，而是“在 `silent flexibility recover（PR226-228 类修复/收束）` 中，当前输入是否精确命中 `silent flexibility recover / scope repair / PR226 / PR227 / PR228`，并且没有触发更高风险 lane”。先决定 owner、lane、allowed_paths、forbidden_paths 与 validation command，再派模型；多窗口只解决吞吐，不替代 authority。 这张卡用于开工前 60 秒 readback：说清楚 source、owner、gate、evidence sink 与 stop condition；说不清楚时，状态保持 hold。
