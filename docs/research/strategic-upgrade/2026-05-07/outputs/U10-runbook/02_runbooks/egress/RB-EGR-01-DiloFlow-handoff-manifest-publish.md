---
title: DiloFlow handoff（manifest publish）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-EGR-01
...
[candidate procedure] 本 runbook 把 `DiloFlow handoff（manifest publish）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `DiloFlow, handoff, manifest publish` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `medium`。风险来自：不得把 downstream request 写成 ScoutFlow authority；不得省略 evidence hashes。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：DiloFlow, handoff, manifest publish, downstream。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得把 downstream request 写成 ScoutFlow authority；不得省略 evidence hashes。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[precondition] P8. 本 runbook 的直接场景是：把 ScoutFlow proof packet 转给 DiloFlow 前生成 manifest、source labels、blocked lanes 与 redaction proof。 这条前置条件必须在执行前被 readback；如果当前环境无法验证，就在 ledger 中写 `unverified_in_current_environment`，不要补成肯定事实。

## 4. Steps / steps

[operator step] S1. 把场景改写成一句可审计任务：把 ScoutFlow proof packet 转给 DiloFlow 前生成 manifest、source labels、blocked lanes 与 redaction proof。
...
[operator step] S8. 把禁止项写入任务头：不得把 downstream request 写成 ScoutFlow authority；不得省略 evidence hashes。
...
[lesson] L6. 下游最需要的是可追溯输入，不是 ScoutFlow 内部长叙事。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: P3-EGR-EGR-DILOFLOW-HANDOFF

[linked reference] A2. linked_tools: handoff manifest, proof packet, redaction audit
...
[footer boundary] RB-EGR-01 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
...
[scenario triage] `RB-EGR-01` 的最小可执行判断不是“这个场景是否有价值”，而是“在 `DiloFlow handoff（manifest publish）` 中，当前输入是否精确命中 `DiloFlow / handoff / manifest publish / downstream`，并且没有触发更高风险 lane”。下游输出必须保留 manifest、redaction、supersede 与 destination schema；RAW vault staging 永远不是 true write approval。 这张卡用于开工前 60 秒 readback：说清楚 source、owner、gate、evidence sink 与 stop condition；说不清楚时，状态保持 hold。
