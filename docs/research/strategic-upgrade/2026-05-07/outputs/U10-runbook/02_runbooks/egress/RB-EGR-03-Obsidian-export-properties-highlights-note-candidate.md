---
title: Obsidian export（properties / highlights / note candidate）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-EGR-03
...
[candidate procedure] 本 runbook 把 `Obsidian export（properties / highlights / note candidate）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `Obsidian, export, properties` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `medium`。风险来自：不得写入 credential material；不得把 clipped note 当 final knowledge。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：Obsidian, export, properties, highlights, note candidate。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得写入 credential material；不得把 clipped note 当 final knowledge。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[precondition] P6. raw dispatch mirror 不是 repo authority；current/task-index/decision-log/contracts-index 才是 repo authority surface。 这条前置条件必须在执行前被 readback；如果当前环境无法验证，就在 ledger 中写 `unverified_in_current_environment`，不要补成肯定事实。
...
[precondition] P8. 本 runbook 的直接场景是：将候选材料导出为 Obsidian 可读 note，保留 metadata、source URL、blocked lanes、next action。 这条前置条件必须在执行前被 readback；如果当前环境无法验证，就在 ledger 中写 `unverified_in_current_environment`，不要补成肯定事实。

## 4. Steps / steps

[operator step] S1. 把场景改写成一句可审计任务：将候选材料导出为 Obsidian 可读 note，保留 metadata、source URL、blocked lanes、next action。

[operator step] S2. 确认下游系统角色：RAW 是长期知识/交付面，DiloFlow 是后续流程，Obsidian 是 note surface，ScoutFlow 是 proof/control 前端。

[operator step] S3. 准备 manifest：source URL、evidence hash、claim label、blocked_lanes、redaction proof、rollback pointer。

[operator step] S4. 对每条材料标注 preview / candidate / authority / knowledge 的状态，不让下游误读。

[operator step] S5. 先 staging 后 publish；任何 true write、knowledge final、script seed 都需要下游确认。

[operator step] S6. egress 前执行 secret/PII/legal-sensitive scan。

[operator step] S7. egress 后记录 acknowledgement 或 failure，不把传递成功当使用成功。

[operator step] S8. 把禁止项写入任务头：不得写入 credential material；不得把 clipped note 当 final knowledge。
...
[verification item] V6. manifest 是否含 blocked_lanes 与 redaction proof。

[verification item] V7. 是否明确 preview != RAW intake success != knowledge success。
...
[lesson] L6. 低摩擦入口重要，但模板必须明确 candidate 与 source-of-record。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: P3-EGR-EGR-OBSIDIAN-EXPORT

[linked reference] A2. linked_tools: frontmatter template, markdown renderer, PII scan

[linked reference] A3. linked_rules: ~/.claude/rules/security.md, ~/.claude/rules/session-closure.md, ~/.claude/rules/execution-discipline.md

[linked reference] A4. linked_skill: downstream-handoff
...
[footer boundary] RB-EGR-03 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
...
[scenario triage] `RB-EGR-03` 的最小可执行判断不是“这个场景是否有价值”，而是“在 `Obsidian export（properties / highlights / note candidate）` 中，当前输入是否精确命中 `Obsidian / export / properties / highlights / note candidate`，并且没有触发更高风险 lane”。下游输出必须保留 manifest、redaction、supersede 与 destination schema；RAW vault staging 永远不是 true write approval。 这张卡用于开工前 60 秒 readback：说清楚 source、owner、gate、evidence sink 与 stop condition；说不清楚时，状态保持 hold。

[failure mode] 本场景风险等级为 `medium`，需要优先防止“把候选 SOP 当执行批准”的漂移。最危险的捷径是把 preview artifact 直接发布到 Obsidian/DiloFlow/Hermes，或让旧版本和新版本并行有效。 复盘时要把失败分成三类：前置证据不足、步骤越界、验证口径过宽；不要笼统写成“模型理解有误”。

[verification cue] 人工复核 manifest checksum、egress destination、redaction status、supersede chain、schema validate 与 downstream notification。 对每个 cue 都要留下可审计文字：`checked_by`、`checked_at`、`source_ref`、`result`、`residual_risk`。如果 cue 只能凭记忆确认，就写 `unverified_in_current_environment`，不要补写成通过。

[handoff card] handoff 必须包含 artifact_id、destination、redaction_hash、schema_version、superseded_by 和 rollback broadcast plan。 推荐在下一窗口开头复制四行：`intent:`、`allowed:`、`blocked:`、`verify:`。这能把 ContentFlow L 提示里的多窗口换手教训转成可操作记忆，减少 21 次 swap 类场景中的边界遗失。
