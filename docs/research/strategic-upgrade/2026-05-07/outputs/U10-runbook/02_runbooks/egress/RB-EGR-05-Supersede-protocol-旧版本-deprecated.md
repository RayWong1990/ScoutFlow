---
title: Supersede protocol（旧版本 deprecated）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-EGR-05
...
[candidate procedure] 本 runbook 把 `Supersede protocol（旧版本 deprecated）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `supersede, deprecated, old version` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `high`。风险来自：不得删除旧证据；不得留下两个 active truth。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：supersede, deprecated, old version, replacement。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得删除旧证据；不得留下两个 active truth。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[precondition] P8. 本 runbook 的直接场景是：当旧 runbook、handoff、spec、asset 被新版本取代时，写 supersede chain 与 migration note。 这条前置条件必须在执行前被 readback；如果当前环境无法验证，就在 ledger 中写 `unverified_in_current_environment`，不要补成肯定事实。

## 4. Steps / steps

[operator step] S1. 把场景改写成一句可审计任务：当旧 runbook、handoff、spec、asset 被新版本取代时，写 supersede chain 与 migration note。
...
[operator step] S8. 把禁止项写入任务头：不得删除旧证据；不得留下两个 active truth。
...
[lesson] L6. supersede 的本质是保留历史，同时让当前执行路径唯一。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: P3-EGR-EGR-SUPERSEDE, P4-BND-HARD-REDLINES

[linked reference] A2. linked_tools: supersede header, archive pointer, readback table
...
[footer boundary] RB-EGR-05 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
...
[scenario triage] `RB-EGR-05` 的最小可执行判断不是“这个场景是否有价值”，而是“在 `Supersede protocol（旧版本 deprecated）` 中，当前输入是否精确命中 `supersede / deprecated / old version / replacement`，并且没有触发更高风险 lane”。下游输出必须保留 manifest、redaction、supersede 与 destination schema；RAW vault staging 永远不是 true write approval。 这张卡用于开工前 60 秒 readback：说清楚 source、owner、gate、evidence sink 与 stop condition；说不清楚时，状态保持 hold。

[failure mode] 本场景风险等级为 `high`，需要优先防止“把候选 SOP 当执行批准”的漂移。最危险的捷径是把 preview artifact 直接发布到 Obsidian/DiloFlow/Hermes，或让旧版本和新版本并行有效。 复盘时要把失败分成三类：前置证据不足、步骤越界、验证口径过宽；不要笼统写成“模型理解有误”。
