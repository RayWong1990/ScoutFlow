---
title: Redaction enforce（PII / 凭据 / 法律敏感）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-EGR-06
...
[candidate procedure] 本 runbook 把 `Redaction enforce（PII / 凭据 / 法律敏感）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `redaction enforce, PII, credential` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `critical`。风险来自：不得把内部日志原样传给下游；不得让 downstream 存放不可追溯敏感字段。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：redaction enforce, PII, credential, legal sensitive。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得把内部日志原样传给下游；不得让 downstream 存放不可追溯敏感字段。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[precondition] P8. 本 runbook 的直接场景是：任何交付到 RAW/DiloFlow/Obsidian/Hermes 前，都执行 redaction 与 legal-sensitive review。 这条前置条件必须在执行前被 readback；如果当前环境无法验证，就在 ledger 中写 `unverified_in_current_environment`，不要补成肯定事实。

## 4. Steps / steps

[operator step] S1. 把场景改写成一句可审计任务：任何交付到 RAW/DiloFlow/Obsidian/Hermes 前，都执行 redaction 与 legal-sensitive review。
...
[operator step] S8. 把禁止项写入任务头：不得把内部日志原样传给下游；不得让 downstream 存放不可追溯敏感字段。
...
[lesson] L6. 下游一旦持久化，修复成本更高；egress 前的 redaction 是最后关口。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: P3-EGR-EGR-REDACTION-ENFORCE, P4-BND-HARD-REDLINES

[linked reference] A2. linked_tools: redaction proof, sensitive_fields_removed, legal note
...
[footer boundary] RB-EGR-06 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
...
[scenario triage] `RB-EGR-06` 的最小可执行判断不是“这个场景是否有价值”，而是“在 `Redaction enforce（PII / 凭据 / 法律敏感）` 中，当前输入是否精确命中 `redaction enforce / PII / credential / legal sensitive`，并且没有触发更高风险 lane”。下游输出必须保留 manifest、redaction、supersede 与 destination schema；RAW vault staging 永远不是 true write approval。 这张卡用于开工前 60 秒 readback：说清楚 source、owner、gate、evidence sink 与 stop condition；说不清楚时，状态保持 hold。
