---
title: PreCompact 兜底 + SessionStart(compact) 恢复
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-MEM-07
...
linked_dispatch:
  - MOD-MEM-MEM-PRECOMPACT-RECOVER
...
[candidate procedure] 本 runbook 把 `PreCompact 兜底 + SessionStart(compact) 恢复` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `PreCompact, SessionStart, compact recovery` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `high`。风险来自：不得假设 compact 后上下文完整；不得继续执行高风险动作直到 readback。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：PreCompact, SessionStart, compact recovery, fallback。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得假设 compact 后上下文完整；不得继续执行高风险动作直到 readback。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[precondition] P7. 本 runbook 的直接场景是：当模型或工具触发自动 compact 前后，用兜底 note 恢复任务状态与边界。 这条前置条件必须在执行前被 readback；如果当前环境无法验证，就在 ledger 中写 `unverified_in_current_environment`，不要补成肯定事实。
...
[operator step] S1. 把场景改写成一句可审计任务：当模型或工具触发自动 compact 前后，用兜底 note 恢复任务状态与边界。
...
[operator step] S8. 把禁止项写入任务头：不得假设 compact 后上下文完整；不得继续执行高风险动作直到 readback。
...
[lesson] L6. 最危险的丢失不是内容丢失，而是边界、owner、rollback 丢失。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: MOD-MEM-MEM-PRECOMPACT-RECOVER, P4-BND-HARD-REDLINES, P3-DSP-LANE-GUARD

[linked reference] A2. linked_tools: precompact note, recovery readback, risk reclassification
...
[footer boundary] RB-MEM-07 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
...
[scenario triage] `RB-MEM-07` 的最小可执行判断不是“这个场景是否有价值”，而是“在 `PreCompact 兜底 + SessionStart(compact) 恢复` 中，当前输入是否精确命中 `PreCompact / SessionStart / compact recovery / fallback`，并且没有触发更高风险 lane”。跨 session 不是摘要越长越好，而是保留下一轮可执行的边界、未解锁项、证据位置和 next-session prompt。 这张卡用于开工前 60 秒 readback：说清楚 source、owner、gate、evidence sink 与 stop condition；说不清楚时，状态保持 hold。

[failure mode] 本场景风险等级为 `high`，需要优先防止“把候选 SOP 当执行批准”的漂移。最危险的捷径是用 /clear 或 /compact 清掉 boundary context，导致下一窗口重新发明 scope 或漏掉 rollback。 复盘时要把失败分成三类：前置证据不足、步骤越界、验证口径过宽；不要笼统写成“模型理解有误”。
