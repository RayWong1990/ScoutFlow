---
title: ~/workspace/raw wiped（RAW 工作区误删恢复）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-REC-01
cluster: Recovery
trigger_keywords:
  - raw wiped
  - workspace/raw
...
[candidate procedure] 本 runbook 把 `~/workspace/raw wiped（RAW 工作区误删恢复）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `raw wiped, workspace/raw, 误删` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `critical`。风险来自：不得用 ScoutFlow preview 反向重建 RAW truth；不得继续下游写入。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：raw wiped, workspace/raw, 误删, restore。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得用 ScoutFlow preview 反向重建 RAW truth；不得继续下游写入。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[operator step] S1. 把场景改写成一句可审计任务：发现 RAW 工作区被误删或大面积丢失时，立即冻结写入、保留现场、按备份/manifest 恢复。
...
[operator step] S8. 把禁止项写入任务头：不得用 ScoutFlow preview 反向重建 RAW truth；不得继续下游写入。
...
[lesson] L6. RAW 是 long-lived knowledge vault；恢复时必须先保护 SoR，不让 mirror 反向污染。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: MOD-REC-REC-RAW-WIPED, P4-BND-HARD-REDLINES

[linked reference] A2. linked_tools: backup manifest, hash manifest, git status, filesystem snapshot
...
[footer boundary] RB-REC-01 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
...
[scenario triage] `RB-REC-01` 的最小可执行判断不是“这个场景是否有价值”，而是“在 `~/workspace/raw wiped（RAW 工作区误删恢复）` 中，当前输入是否精确命中 `raw wiped / workspace/raw / 误删 / restore`，并且没有触发更高风险 lane”。恢复类 SOP 先保存证据，再收敛写入；不要因为焦虑恢复而覆盖 crash site、损坏 DB 或误删分支线索。 这张卡用于开工前 60 秒 readback：说清楚 source、owner、gate、evidence sink 与 stop condition；说不清楚时，状态保持 hold。

[failure mode] 本场景风险等级为 `critical`，需要优先防止“把候选 SOP 当执行批准”的漂移。最危险的捷径是直接 reset/recreate/rm，导致可诊断证据消失，或者让 worker/API 在损坏状态继续写。 复盘时要把失败分成三类：前置证据不足、步骤越界、验证口径过宽；不要笼统写成“模型理解有误”。
