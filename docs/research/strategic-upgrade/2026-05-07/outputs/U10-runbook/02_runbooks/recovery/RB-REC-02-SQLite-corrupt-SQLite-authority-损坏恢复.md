---
title: SQLite corrupt（SQLite authority 损坏恢复）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-REC-02
cluster: Recovery
trigger_keywords:
  - SQLite corrupt
  - database corruption
...
[candidate procedure] 本 runbook 把 `SQLite corrupt（SQLite authority 损坏恢复）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `SQLite corrupt, database corruption, authority DB` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `critical`。风险来自：不得直接手工编辑 DB；不得让 worker 继续写。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：SQLite corrupt, database corruption, authority DB, restore。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得直接手工编辑 DB；不得让 worker 继续写。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[operator step] S1. 把场景改写成一句可审计任务：SQLite authority DB 损坏时，停止 API/worker，备份损坏文件，使用 integrity_check 与最近备份恢复。
...
[operator step] S8. 把禁止项写入任务头：不得直接手工编辑 DB；不得让 worker 继续写。
...
[lesson] L6. SQLite 是 L0 authority；恢复要保留损坏证据与 replay plan。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: MOD-REC-REC-SQLITE-CORRUPT, P4-BND-HARD-REDLINES

[linked reference] A2. linked_tools: sqlite3 PRAGMA integrity_check, backup restore, state words audit
...
[footer boundary] RB-REC-02 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
...
[scenario triage] `RB-REC-02` 的最小可执行判断不是“这个场景是否有价值”，而是“在 `SQLite corrupt（SQLite authority 损坏恢复）` 中，当前输入是否精确命中 `SQLite corrupt / database corruption / authority DB / restore`，并且没有触发更高风险 lane”。恢复类 SOP 先保存证据，再收敛写入；不要因为焦虑恢复而覆盖 crash site、损坏 DB 或误删分支线索。 这张卡用于开工前 60 秒 readback：说清楚 source、owner、gate、evidence sink 与 stop condition；说不清楚时，状态保持 hold。
