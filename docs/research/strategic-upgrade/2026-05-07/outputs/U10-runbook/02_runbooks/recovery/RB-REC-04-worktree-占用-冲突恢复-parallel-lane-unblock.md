---
title: worktree 占用 / 冲突恢复（parallel lane unblock）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-REC-04
cluster: Recovery
trigger_keywords:
  - worktree occupied
  - branch lock
...
linked_dispatch:
  - MOD-REC-REC-WORKTREE-OCCUPIED
  - P3-DSP-LANE-GUARD
linked_pr:
  - none
linked_tools:
  - git worktree list
  - same-file lock table
  - stash policy
linked_skill:
  - recovery-playbook
linked_rule:
...
[candidate procedure] 本 runbook 把 `worktree 占用 / 冲突恢复（parallel lane unblock）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `worktree occupied, branch lock, same file conflict` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `medium`。风险来自：不得强删他人 worktree；不得在冲突状态写 authority。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：worktree occupied, branch lock, same file conflict, parallel lane。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得强删他人 worktree；不得在冲突状态写 authority。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[operator step] S1. 把场景改写成一句可审计任务：多 worktree 并行时出现占用、锁、同文件冲突，按 owner/lane/allowed_paths 解除。
...
[operator step] S8. 把禁止项写入任务头：不得强删他人 worktree；不得在冲突状态写 authority。
...
[lesson] L6. 并行冲突要靠显式 owner 解，不靠“谁先跑完”。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: MOD-REC-REC-WORKTREE-OCCUPIED, P3-DSP-LANE-GUARD

[linked reference] A2. linked_tools: git worktree list, same-file lock table, stash policy
...
[footer boundary] RB-REC-04 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
...
[scenario triage] `RB-REC-04` 的最小可执行判断不是“这个场景是否有价值”，而是“在 `worktree 占用 / 冲突恢复（parallel lane unblock）` 中，当前输入是否精确命中 `worktree occupied / branch lock / same file conflict / parallel lane`，并且没有触发更高风险 lane”。恢复类 SOP 先保存证据，再收敛写入；不要因为焦虑恢复而覆盖 crash site、损坏 DB 或误删分支线索。 这张卡用于开工前 60 秒 readback：说清楚 source、owner、gate、evidence sink 与 stop condition；说不清楚时，状态保持 hold。

[failure mode] 本场景风险等级为 `medium`，需要优先防止“把候选 SOP 当执行批准”的漂移。最危险的捷径是直接 reset/recreate/rm，导致可诊断证据消失，或者让 worker/API 在损坏状态继续写。 复盘时要把失败分成三类：前置证据不足、步骤越界、验证口径过宽；不要笼统写成“模型理解有误”。
