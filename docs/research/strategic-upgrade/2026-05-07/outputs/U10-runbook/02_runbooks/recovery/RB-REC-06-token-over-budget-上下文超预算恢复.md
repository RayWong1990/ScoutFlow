---
title: token over budget（上下文超预算恢复）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-REC-06
cluster: Recovery
trigger_keywords:
  - token over budget
  - context overflow
...
linked_dispatch:
  - MOD-REC-REC-TOKEN-OVER-BUDGET
  - P4-BND-HARD-REDLINES
  - P3-DSP-LANE-GUARD
linked_pr:
  - none
linked_tools:
  - token hygiene threshold
  - precompact note
  - readback checklist
linked_skill:
  - recovery-playbook
...
[candidate procedure] 本 runbook 把 `token over budget（上下文超预算恢复）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `token over budget, context overflow, 85%` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `high`。风险来自：不得继续 merge/authority edit；不得靠记忆补全 omitted constraints。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：token over budget, context overflow, 85%, recovery。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得继续 merge/authority edit；不得靠记忆补全 omitted constraints。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[operator step] S1. 把场景改写成一句可审计任务：上下文超过红线后，暂停高风险动作，写 PreCompact note，做 /compact 或 /clear 后 readback。
...
[operator step] S8. 把禁止项写入任务头：不得继续 merge/authority edit；不得靠记忆补全 omitted constraints。
...
[lesson] L6. token 预算超限时最容易丢 stop lines；恢复的第一目标是找回边界。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: MOD-REC-REC-TOKEN-OVER-BUDGET, P4-BND-HARD-REDLINES, P3-DSP-LANE-GUARD

[linked reference] A2. linked_tools: token hygiene threshold, precompact note, readback checklist
...
[footer boundary] RB-REC-06 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
...
[scenario triage] `RB-REC-06` 的最小可执行判断不是“这个场景是否有价值”，而是“在 `token over budget（上下文超预算恢复）` 中，当前输入是否精确命中 `token over budget / context overflow / 85% / recovery`，并且没有触发更高风险 lane”。恢复类 SOP 先保存证据，再收敛写入；不要因为焦虑恢复而覆盖 crash site、损坏 DB 或误删分支线索。 这张卡用于开工前 60 秒 readback：说清楚 source、owner、gate、evidence sink 与 stop condition；说不清楚时，状态保持 hold。
