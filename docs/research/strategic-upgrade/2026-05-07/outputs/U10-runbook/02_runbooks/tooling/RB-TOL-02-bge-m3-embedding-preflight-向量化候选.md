---
title: bge-m3 embedding preflight（向量化候选）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-TOL-02
cluster: Tooling
trigger_keywords:
  - bge-m3
  - embedding
...
linked_dispatch:
  - MOD-TOL-TOL-BGE-M3-PREFLIGHT
  - P4-BND-HARD-REDLINES
linked_pr:
  - none
linked_tools:
  - model card checklist
  - sample-free dry plan
  - PII scan
linked_skill:
  - tool-preflight
linked_rule:
...
[candidate procedure] 本 runbook 把 `bge-m3 embedding preflight（向量化候选）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `bge-m3, embedding, vector` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `high`。风险来自：不得把 RAW/private note 未脱敏送入模型；不得创建 production index。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：bge-m3, embedding, vector, preflight。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得把 RAW/private note 未脱敏送入模型；不得创建 production index。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[operator step] S1. 把场景改写成一句可审计任务：评估 bge-m3 是否适合本地向量化候选，检查数据脱敏、chunk lineage、storage boundary。
...
[operator step] S8. 把禁止项写入任务头：不得把 RAW/private note 未脱敏送入模型；不得创建 production index。
...
[lesson] L6. 向量索引会放大隐私问题；先定义 lineage 和删除路径。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: MOD-TOL-TOL-BGE-M3-PREFLIGHT, P4-BND-HARD-REDLINES

[linked reference] A2. linked_tools: model card checklist, sample-free dry plan, PII scan
...
[footer boundary] RB-TOL-02 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
...
[scenario triage] `RB-TOL-02` 的最小可执行判断不是“这个场景是否有价值”，而是“在 `bge-m3 embedding preflight（向量化候选）` 中，当前输入是否精确命中 `bge-m3 / embedding / vector / preflight`，并且没有触发更高风险 lane”。工具安装/探测只做 preflight，不把可安装性写成已批准运行；每个工具都要有环境、版本、资源与 rollback 条件。 这张卡用于开工前 60 秒 readback：说清楚 source、owner、gate、evidence sink 与 stop condition；说不清楚时，状态保持 hold。
