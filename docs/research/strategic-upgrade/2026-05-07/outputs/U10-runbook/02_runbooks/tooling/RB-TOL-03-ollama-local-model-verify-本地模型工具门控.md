---
title: ollama local model verify（本地模型工具门控）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-TOL-03
cluster: Tooling
trigger_keywords:
  - ollama
  - local model
...
[candidate procedure] 本 runbook 把 `ollama local model verify（本地模型工具门控）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `ollama, local model, LLM` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `medium`。风险来自：不得把生产凭据/RAW 私密材料用于测试；不得自动拉取模型。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：ollama, local model, LLM, verify。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得把生产凭据/RAW 私密材料用于测试；不得自动拉取模型。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[operator step] S1. 把场景改写成一句可审计任务：验证 ollama 本地模型候选时，确认模型来源、磁盘、prompt logging、PII policy。
...
[operator step] S8. 把禁止项写入任务头：不得把生产凭据/RAW 私密材料用于测试；不得自动拉取模型。
...
[lesson] L6. local-first 不等于无需安全；本地日志同样可能成为泄露面。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: MOD-TOL-TOL-OLLAMA-VERIFY

[linked reference] A2. linked_tools: model inventory, prompt logging check, redaction checklist
...
[footer boundary] RB-TOL-03 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
...
[scenario triage] `RB-TOL-03` 的最小可执行判断不是“这个场景是否有价值”，而是“在 `ollama local model verify（本地模型工具门控）` 中，当前输入是否精确命中 `ollama / local model / LLM / verify`，并且没有触发更高风险 lane”。工具安装/探测只做 preflight，不把可安装性写成已批准运行；每个工具都要有环境、版本、资源与 rollback 条件。 这张卡用于开工前 60 秒 readback：说清楚 source、owner、gate、evidence sink 与 stop condition；说不清楚时，状态保持 hold。

[failure mode] 本场景风险等级为 `medium`，需要优先防止“把候选 SOP 当执行批准”的漂移。最危险的捷径是在候选 runbook 中执行 pip/brew/model pull，或把本机 probe 当成所有用户环境的事实。 复盘时要把失败分成三类：前置证据不足、步骤越界、验证口径过宽；不要笼统写成“模型理解有误”。
