---
title: sqlite-vec preflight（SQLite vector extension 候选）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-TOL-04
cluster: Tooling
trigger_keywords:
  - sqlite-vec
  - SQLite
...
linked_dispatch:
  - MOD-TOL-TOL-SQLITE-VEC-PREFLIGHT
  - P4-BND-HARD-REDLINES
linked_pr:
  - none
linked_tools:
  - schema gate checklist
  - migration blocked registry
  - sandbox plan
linked_skill:
  - tool-preflight
linked_rule:
...
[candidate procedure] 本 runbook 把 `sqlite-vec preflight（SQLite vector extension 候选）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `sqlite-vec, SQLite, vector extension` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `critical`。风险来自：不得改 schema、不得加载 extension 到 production DB、不得写 migration。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：sqlite-vec, SQLite, vector extension, migration。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得改 schema、不得加载 extension 到 production DB、不得写 migration。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[operator step] S1. 把场景改写成一句可审计任务：评估 sqlite-vec 前确认 DB vNext/migration gate 未开，只能做 research note。
...
[operator step] S8. 把禁止项写入任务头：不得改 schema、不得加载 extension 到 production DB、不得写 migration。
...
[lesson] L6. SQLite 是 authority 核心；任何 extension 都接近 migration gate。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: MOD-TOL-TOL-SQLITE-VEC-PREFLIGHT, P4-BND-HARD-REDLINES

[linked reference] A2. linked_tools: schema gate checklist, migration blocked registry, sandbox plan
...
[footer boundary] RB-TOL-04 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
...
[scenario triage] `RB-TOL-04` 的最小可执行判断不是“这个场景是否有价值”，而是“在 `sqlite-vec preflight（SQLite vector extension 候选）` 中，当前输入是否精确命中 `sqlite-vec / SQLite / vector extension / migration`，并且没有触发更高风险 lane”。工具安装/探测只做 preflight，不把可安装性写成已批准运行；每个工具都要有环境、版本、资源与 rollback 条件。 这张卡用于开工前 60 秒 readback：说清楚 source、owner、gate、evidence sink 与 stop condition；说不清楚时，状态保持 hold。
