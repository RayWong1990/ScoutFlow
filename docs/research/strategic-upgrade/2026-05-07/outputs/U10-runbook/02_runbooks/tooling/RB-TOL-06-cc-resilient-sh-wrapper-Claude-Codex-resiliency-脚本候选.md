---
title: cc-resilient.sh wrapper（Claude/Codex resiliency 脚本候选）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-TOL-06
cluster: Tooling
trigger_keywords:
  - cc-resilient.sh
  - resiliency
...
[candidate procedure] 本 runbook 把 `cc-resilient.sh wrapper（Claude/Codex resiliency 脚本候选）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `cc-resilient.sh, resiliency, wrapper` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `high`。风险来自：不得自动重试高风险命令；不得记录 secrets；不得绕过 human gate。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：cc-resilient.sh, resiliency, wrapper, retry script。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得自动重试高风险命令；不得记录 secrets；不得绕过 human gate。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[operator step] S1. 把场景改写成一句可审计任务：为 Claude/Codex 命令包装 retry/resume 时，先定义日志脱敏、exit code、scope lock。
...
[operator step] S8. 把禁止项写入任务头：不得自动重试高风险命令；不得记录 secrets；不得绕过 human gate。
...
[lesson] L6. 自动重试会放大错误；resiliency 必须只包低风险、可回滚动作。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: MOD-TOL-TOL-CC-RESILIENT, P4-BND-HARD-REDLINES

[linked reference] A2. linked_tools: shellcheck, dry-run mode, log redaction, exit-code table
...
[footer boundary] RB-TOL-06 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
...
[scenario triage] `RB-TOL-06` 的最小可执行判断不是“这个场景是否有价值”，而是“在 `cc-resilient.sh wrapper（Claude/Codex resiliency 脚本候选）` 中，当前输入是否精确命中 `cc-resilient.sh / resiliency / wrapper / retry script`，并且没有触发更高风险 lane”。工具安装/探测只做 preflight，不把可安装性写成已批准运行；每个工具都要有环境、版本、资源与 rollback 条件。 这张卡用于开工前 60 秒 readback：说清楚 source、owner、gate、evidence sink 与 stop condition；说不清楚时，状态保持 hold。

[failure mode] 本场景风险等级为 `high`，需要优先防止“把候选 SOP 当执行批准”的漂移。最危险的捷径是在候选 runbook 中执行 pip/brew/model pull，或把本机 probe 当成所有用户环境的事实。 复盘时要把失败分成三类：前置证据不足、步骤越界、验证口径过宽；不要笼统写成“模型理解有误”。
