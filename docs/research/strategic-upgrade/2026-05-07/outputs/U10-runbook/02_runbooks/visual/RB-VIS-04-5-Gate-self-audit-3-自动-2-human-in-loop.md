---
title: 5-Gate self-audit（3 自动 + 2 human-in-loop）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-VIS-04
...
[candidate procedure] 本 runbook 把 `5-Gate self-audit（3 自动 + 2 human-in-loop）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `5 Gate, self-audit, human visual review` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `high`。风险来自：不得把 3 个自动门通过说成 5 Gate 全过。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：5 Gate, self-audit, human visual review, CI visual。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得把 3 个自动门通过说成 5 Gate 全过。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[precondition] P8. 本 runbook 的直接场景是：对视觉任务执行功能、边界、lint/build、截图/可访问性、人类视觉终判五门检查。 这条前置条件必须在执行前被 readback；如果当前环境无法验证，就在 ledger 中写 `unverified_in_current_environment`，不要补成肯定事实。

## 4. Steps / steps

[operator step] S1. 把场景改写成一句可审计任务：对视觉任务执行功能、边界、lint/build、截图/可访问性、人类视觉终判五门检查。
...
[operator step] S8. 把禁止项写入任务头：不得把 3 个自动门通过说成 5 Gate 全过。
...
[lesson] L6. current.md 明确无 screenshot evidence、无 Playwright execution、人类视觉未终判；不要虚构通过。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: P3-VIS-VIS-FIVE-GATE, P4-BND-HARD-REDLINES

[linked reference] A2. linked_tools: npm test, npm run build, contrast audit, human review checklist
...
[footer boundary] RB-VIS-04 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
...
[scenario triage] `RB-VIS-04` 的最小可执行判断不是“这个场景是否有价值”，而是“在 `5-Gate self-audit（3 自动 + 2 human-in-loop）` 中，当前输入是否精确命中 `5 Gate / self-audit / human visual review / CI visual`，并且没有触发更高风险 lane”。视觉生产先定义 state、token、asset lineage 与 5-Gate；生成图像或 TSX 之前先确认不改 authority state words。 这张卡用于开工前 60 秒 readback：说清楚 source、owner、gate、evidence sink 与 stop condition；说不清楚时，状态保持 hold。
