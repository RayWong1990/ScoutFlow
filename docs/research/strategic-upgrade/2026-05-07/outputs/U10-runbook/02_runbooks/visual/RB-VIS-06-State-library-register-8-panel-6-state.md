---
title: State library register（8 panel × 6 state）
status: candidate
authority: not-authority
created_at: 2026-05-07
runbook_id: RB-VIS-06
...
linked_dispatch:
  - P3-VIS-VIS-STATE-LIBRARY
linked_pr:
  - none
linked_tools:
  - state registry
  - state words check
  - component snapshot note
linked_skill:
  - visual-production
linked_rule:
...
[candidate procedure] 本 runbook 把 `State library register（8 panel × 6 state）` 固化为 single-user prosumer 可重复 SOP。目标不是批准动作，而是让用户在遇到 `state library, 8 panel, 6 state` 这类触发词时，能先检查边界、证据与 rollback，再决定是否另开获授权执行。
...
[boundary note] 该场景风险等级为 `medium`。风险来自：不得新增未批准 state words；不得把 UI 状态升级为 authority state。 因此本文件只提供候选步骤和审计点，不构成 runtime approval、migration approval、browser automation approval、ASR approval、vault true write approval 或 package approval。

## 2. Trigger / trigger

[operator trigger] 当用户输入、handoff、dispatch 或下游请求出现以下关键词时启用本 SOP：state library, 8 panel, 6 state, panel state。如果同时出现更高风险关键词，例如 credential、migration、runtime、browser automation、ASR、true write、authority edit，则先按 Boundary / Audit cluster 重新分级。

[negative trigger] 不要在以下情况下启用本 SOP 的执行路径：不得新增未批准 state words；不得把 UI 状态升级为 authority state。 如果只是研究、复盘、教育用途，可以阅读本 SOP；如果要实际运行命令或写文件，必须另有明确 dispatch、allowed_paths、owner 和 validation plan。
...
[precondition] P8. 本 runbook 的直接场景是：为 H5 八面板六状态建立状态素材库，绑定 empty/loading/error/metadata_fetched/blocked/review 等可视状态。 这条前置条件必须在执行前被 readback；如果当前环境无法验证，就在 ledger 中写 `unverified_in_current_environment`，不要补成肯定事实。

## 4. Steps / steps

[operator step] S1. 把场景改写成一句可审计任务：为 H5 八面板六状态建立状态素材库，绑定 empty/loading/error/metadata_fetched/blocked/review 等可视状态。
...
[operator step] S8. 把禁止项写入任务头：不得新增未批准 state words；不得把 UI 状态升级为 authority state。
...
[lesson] L6. state words 是跨 worker 语义契约；UI 可以投影，不能改写它。

## 8. Linked artifacts / linked

[linked reference] A1. linked_dispatch: P3-VIS-VIS-STATE-LIBRARY

[linked reference] A2. linked_tools: state registry, state words check, component snapshot note
...
[footer boundary] RB-VIS-06 是 candidate / not-authority SOP。读者可以用它做 checklist、audit、prompt scaffold 或 dispatch draft；不能把它当作用户授权、runtime unlock、legal conclusion、production code change、authority writeback 或 downstream publish approval。
...
[scenario triage] `RB-VIS-06` 的最小可执行判断不是“这个场景是否有价值”，而是“在 `State library register（8 panel × 6 state）` 中，当前输入是否精确命中 `state library / 8 panel / 6 state / panel state`，并且没有触发更高风险 lane”。视觉生产先定义 state、token、asset lineage 与 5-Gate；生成图像或 TSX 之前先确认不改 authority state words。 这张卡用于开工前 60 秒 readback：说清楚 source、owner、gate、evidence sink 与 stop condition；说不清楚时，状态保持 hold。
