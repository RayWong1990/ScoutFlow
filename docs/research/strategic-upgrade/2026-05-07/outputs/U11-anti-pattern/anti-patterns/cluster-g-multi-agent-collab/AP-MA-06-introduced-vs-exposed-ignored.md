...
anti_pattern_id: AP-MA-06
cluster: Multi-Agent-Collab
risk_level: critical
first_seen_in: "ContentFlow L1 meta lesson requested by prompt"
introduced_or_exposed: exposed
detect_method: audit
prevent_method: template
escape_difficulty: hard
linked_runbook: RB-MA-06
linked_dispatch: MOD-MA-06
...
[evidence-backed] 反例是：把被新动作暴露的历史债归因给最近 PR，导致修错层级并惩罚错误 owner。 这个反例危险的地方不在于某个单点动作必然错误，而在于它把“可继续分析”“可作为候选库存”“可在本地预览”“被用户追认保留”混写成“已获执行授权”。当多窗口同时工作时，这种写法会让后续 agent 只看到顺滑叙述，看不到 stop-line、partial、not-authority 或 blocked lane。
...
[evidence-backed] 触发 signal 需要同时看文本、diff 和 ledger。文本 signal 是出现 `把被新动作暴露的历史债归因给最近 PR，导致修错层级并惩罚错误 owner。` 或与其同义的含混授权；diff signal 是 touched path 超过派单或 authority writer 不清；ledger signal 是 closeout 只写 done/pass/clear，却缺少 amendment、partial、reject、needs-edit 或 user-authorized 字段。
...
[candidate] 对本条而言，正例句式是：`This change is candidate-only; no actual rule deployment implied; authority files untouched; introduced_or_exposed=exposed; evidence source = ContentFlow L1 meta lesson requested by prompt; escalation requires explicit user authorization.` 这类句式虽然啰嗦，但能让未来窗口快速判断自己能不能继续。
...
[candidate] 可执行伪规则：`if claim in ['works','PASS','DONE','approved','ready_for_next'] and not evidence_ref: flag`. 对路径类风险：`if touched_path not in allowed_paths and not explicit_amendment: flag`. 对本条特有风险：`if 'AP-MA-06' mapped signal appears and introduced_or_exposed is blank: flag`. 这些规则只是候选 detect rule，不表示已经部署。
...
[candidate] linked_runbook: `RB-MA-06`；linked_dispatch: `MOD-MA-06`；linked_rule: `~/.claude/rules/parallel-safety.md`。U9/U10 实源在本环境未命中，因此这些 ID 是候选映射形状，不是 authority runbook 事实。
...
[derived] 补充判据：当 reviewer 看到 `introduced vs exposed ignored` 时，应优先问三个问题：一，当前句子是否把证据层、推断层和授权层分开；二，当前动作是 introduced defect 还是 exposed debt；三，是否存在更窄的 safe continuation。只要任一问题没有答案，就不应把 closeout 写成 clear，而应写 partial 或 needs-amendment。
