...
ScoutFlow speaks like a calm technical operator, not like a consumer app, enterprise dashboard or marketing landing page. The voice should be precise, low-drama, and audit-friendly. It should tell the user what is known, what is blocked, what is candidate, and what evidence exists.

## Tone principles

| Principle | Use | Avoid |
|---|---|---|
| Evidence first | `metadata probe ready` | `magic insight generated` |
| Boundary clear | `vault preview only; true write blocked` | `commit ready` |
| Calm urgency | `review required before pass` | `critical failure!!!` |
| Single operator | `local workstation` | `team collaboration hub` |
| Dense but humane | short technical labels + readable notes | walls of raw JSON |

## Copy patterns

- Good: `Blocked: audio_transcript not approved by current gate.`
- Good: `Preview only: vault true write remains future-gated.`
- Good: `Metadata probe failed: no evidence returned; retry remains manual.`
- Bad: `Coming soon` for blocked lanes because it hides authority status.
- Bad: `Approved` unless an authority file truly approves that exact action.

## Internationalization

Chinese UI copy should preserve technical tokens where they are the real state object: `audio_transcript`、`capture_id`、`route`、`state_word` 可以保留英文；解释文本用中文清晰说明。英文 UI copy should avoid sales language and use exact state words.
...
### BRAND-VOICE-AND-TONE — audit expansion 1
...
本节适用于 `BRAND-VOICE-AND-TONE`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### BRAND-VOICE-AND-TONE — audit expansion 2
...
### BRAND-VOICE-AND-TONE — audit expansion 3
...
### BRAND-VOICE-AND-TONE — audit expansion 4
...
### BRAND-VOICE-AND-TONE — audit expansion 5
...
### BRAND-VOICE-AND-TONE — audit expansion 6
...
### BRAND-VOICE-AND-TONE — audit expansion 7
...
### BRAND-VOICE-AND-TONE — audit expansion 8
...
### BRAND-VOICE-AND-TONE — audit expansion 9
...
### BRAND-VOICE-AND-TONE — audit expansion 10
...
### BRAND-VOICE-AND-TONE — audit expansion 11
...
### BRAND-VOICE-AND-TONE — audit expansion 12
...
### BRAND-VOICE-AND-TONE — audit expansion 13
...
### BRAND-VOICE-AND-TONE — audit expansion 14
