---
title: SOURCE-DIGEST-AND-BOUNDARY-NOTES
...
# SOURCE-DIGEST-AND-BOUNDARY-NOTES

## Local source review

The local uploaded post176 packet was available and inspected. It contained visual dispatch rows, PRD-v2.1 strong visual H5 amendment, and the OpenDesign Tech Utility visual spec candidate v2. These sources support the four-panel station, token values, 5-Gate criteria and boundary language in this atlas.

## Unavailable local paths

The U13 prompt requested `~/workspace/ScoutFlow/apps/capture-station/src/styles/`, `src/features/**/*.tsx`, `docs/visual/`, `~/.claude/rules/aesthetic-first-principles.md`, local fonts and other workstation paths. Those paths were not present in the container. The atlas therefore does not claim to have extracted current production CSS/TSX tokens. Existing production style claims remain `needs_repo_refresh`.

## GitHub reference attempts

The GitHub connector was invoked for the U4 visual asset spine URL and searched for U4/U7 prompt filenames. The requested file was not returned. This atlas includes U4/U7 link placeholders and marks them pending external refresh rather than inventing content.

## Boundary effect

Because repo-local production CSS/TSX and live web were not available, this atlas is high-coverage as a candidate brand/style system, not an authority replacement for implementation truth.
...
### SOURCE-DIGEST-AND-BOUNDARY-NOTES — audit expansion 1
...
本节适用于 `SOURCE-DIGEST-AND-BOUNDARY-NOTES`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### SOURCE-DIGEST-AND-BOUNDARY-NOTES — audit expansion 2
...
### SOURCE-DIGEST-AND-BOUNDARY-NOTES — audit expansion 3
...
### SOURCE-DIGEST-AND-BOUNDARY-NOTES — audit expansion 4
...
### SOURCE-DIGEST-AND-BOUNDARY-NOTES — audit expansion 5
...
### SOURCE-DIGEST-AND-BOUNDARY-NOTES — audit expansion 6
...
### SOURCE-DIGEST-AND-BOUNDARY-NOTES — audit expansion 7
...
### SOURCE-DIGEST-AND-BOUNDARY-NOTES — audit expansion 8
...
### SOURCE-DIGEST-AND-BOUNDARY-NOTES — audit expansion 9
...
### SOURCE-DIGEST-AND-BOUNDARY-NOTES — audit expansion 10
