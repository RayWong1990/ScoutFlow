---
title: TOKEN-04 Shadow Elevation Overlay
...
# TOKEN-04 Shadow Elevation Overlay

## Purpose

Elevation in ScoutFlow is functional. It separates station slab, panel, row, tooltip and focus states without creating floating SaaS cards or cyberpunk glow.

| Ref name | Value | Role | Usage example |
|---|---:|---|---|
| `sf.shadow.elevation.0` | `none` | flat row | Not decorative neon |
| `sf.shadow.elevation.1` | `0 1px 0 rgba(255,255,255,.03)` | hairline lift | Not decorative neon |
| `sf.shadow.elevation.2` | `0 8px 20px rgba(0,0,0,.22)` | raised panel | Not decorative neon |
| `sf.shadow.elevation.3` | `0 16px 32px rgba(0,0,0,.28)` | modal/sheet | Not decorative neon |
| `sf.shadow.elevation.4` | `0 24px 48px rgba(0,0,0,.32)` | station slab | Not decorative neon |
| `sf.shadow.focus` | `0 0 0 2px rgba(80,212,255,.32)` | focus ring | Not decorative neon |
| `sf.shadow.overlay` | `rgba(7,17,27,.88)` | modal/tooltip backdrop | Not decorative neon |

## Overlay safety

Overlays may explain state but must never cover the state itself. Gate 3 fails if a toast, tooltip, detail shelf, poster badge, export caption or mobile keyboard covers URL input, Capture action, current scope lane, Trust Trace node label or active state word.

## CSS seam

```css
.panel { box-shadow: var(--sf-shadow-elevation-1); }
.station { box-shadow: var(--sf-shadow-elevation-4); }
:focus-visible { box-shadow: var(--sf-shadow-focus); outline: none; }
.tooltip { background: var(--sf-overlay-bg); z-index: var(--sf-z-tooltip); }
```

## Anti-patterns

- continuous cyan glow around every panel;
- deep drop shadows that suggest draggable cards;
- overlay scrims that dim the evidence chain until state truth is unreadable;
- focus styles implemented only by color or only by removing outline.
...
### TOKEN-04 Shadow Elevation Overlay — audit expansion 1
...
本节适用于 `TOKEN-04 Shadow Elevation Overlay`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### TOKEN-04 Shadow Elevation Overlay — audit expansion 2
...
### TOKEN-04 Shadow Elevation Overlay — audit expansion 3
...
### TOKEN-04 Shadow Elevation Overlay — audit expansion 4
...
### TOKEN-04 Shadow Elevation Overlay — audit expansion 5
...
### TOKEN-04 Shadow Elevation Overlay — audit expansion 6
...
### TOKEN-04 Shadow Elevation Overlay — audit expansion 7
...
### TOKEN-04 Shadow Elevation Overlay — audit expansion 8
...
### TOKEN-04 Shadow Elevation Overlay — audit expansion 9
...
### TOKEN-04 Shadow Elevation Overlay — audit expansion 10
...
### TOKEN-04 Shadow Elevation Overlay — audit expansion 11
