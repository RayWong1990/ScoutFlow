---
title: TOKEN-05 Motion Duration Easing
...
# TOKEN-05 Motion Duration Easing

## Purpose

Motion is a state explanation layer, not a branding flourish. It can confirm metadata arrival, active scope movement, graph path activation, focus and inline error reveal. It must not manufacture fake progress, unlock blocked lanes, or distract from evidence.

| Ref name | Value | Role | Usage example |
|---|---:|---|---|
| `sf.motion.duration.instant` | `80ms` | focus / click affordance | Motion explains state only |
| `sf.motion.duration.fast` | `120ms` | badge/edge shift | Motion explains state only |
| `sf.motion.duration.normal` | `180ms` | metadata settle | Motion explains state only |
| `sf.motion.duration.slow` | `240ms` | panel expansion | Motion explains state only |
| `sf.motion.ease.standard` | `cubic-bezier(.2,0,0,1)` | state reveal | Motion explains state only |
| `sf.motion.ease.emphasized` | `cubic-bezier(.2,.8,.2,1)` | primary action | Motion explains state only |
| `sf.motion.ease.exit` | `cubic-bezier(.4,0,1,1)` | dismiss/error close | Motion explains state only |

## Allowed vocabulary

| Interaction | Allowed | Rejected |
|---|---|---|
| Metadata arrival | opacity + 4px translate over 120-180ms | full-page shimmer |
| Scope active row | border/rail/badge emphasis | jumping row without text change |
| Trust Trace active edge | mild stroke opacity transition | neon pulse loop |
| Button hover | brightness/border shift, scale <= 1.01 | bounce/spring/glow noise |
| Error reveal | inline reason expands inside panel | toast covering URL/action/node |
...
Reduced motion must not remove state comprehension; text, badge, border and position still carry the state.
...
### TOKEN-05 Motion Duration Easing — audit expansion 1
...
本节适用于 `TOKEN-05 Motion Duration Easing`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### TOKEN-05 Motion Duration Easing — audit expansion 2
...
### TOKEN-05 Motion Duration Easing — audit expansion 3
...
### TOKEN-05 Motion Duration Easing — audit expansion 4
...
### TOKEN-05 Motion Duration Easing — audit expansion 5
...
### TOKEN-05 Motion Duration Easing — audit expansion 6
...
### TOKEN-05 Motion Duration Easing — audit expansion 7
...
### TOKEN-05 Motion Duration Easing — audit expansion 8
...
### TOKEN-05 Motion Duration Easing — audit expansion 9
...
### TOKEN-05 Motion Duration Easing — audit expansion 10
...
### TOKEN-05 Motion Duration Easing — audit expansion 11
