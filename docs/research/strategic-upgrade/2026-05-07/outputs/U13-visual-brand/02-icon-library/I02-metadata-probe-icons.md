---
title: "I02 Icon Spec — metadata-probe / scope-allowed"
...
# I02 Icon Spec — metadata-probe / scope-allowed
...
## Icon entry `metadata-probe`

| Field | Value |
|---|---|
| icon_id | `metadata-probe` |
| semantic_intent | Probe metadata evidence |
| preferred_surface | Live Metadata |
| linked_state | `requested/settling` |
| linked_visual_asset | `U4:metadata-probe` / pending external U4 refresh |
...
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `metadata-probe: requested/settling`.
...
## Icon entry `scope-allowed`

| Field | Value |
|---|---|
| icon_id | `scope-allowed` |
| semantic_intent | Current allowed lane |
| preferred_surface | Capture Scope |
| linked_state | `allowed` |
| linked_visual_asset | `U4:scope-allowed` / pending external U4 refresh |
| license | ScoutFlow-original candidate path; third-party license not invoked |
| viewBox | `0 0 24 24` |
...
- Use stroke-only at 1.5–1.8px on dark panels; inherit `currentColor` from semantic token, usually `sf.text.secondary`, `sf.accent.cyan`, `sf.success`, `sf.warning` or `sf.error`.
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `scope-allowed: allowed`.
- Do not use this icon as a decorative watermark behind evidence rows; it belongs inside row headers, badges, compact buttons or template annotations.
...
### I02 Icon Spec — metadata-probe / scope-allowed — audit expansion 1
...
本节适用于 `I02 Icon Spec — metadata-probe / scope-allowed`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### I02 Icon Spec — metadata-probe / scope-allowed — audit expansion 2
...
### I02 Icon Spec — metadata-probe / scope-allowed — audit expansion 3
...
### I02 Icon Spec — metadata-probe / scope-allowed — audit expansion 4
