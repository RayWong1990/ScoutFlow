---
title: "I03 Icon Spec — scope-blocked / trust-node"
...
# I03 Icon Spec — scope-blocked / trust-node
...
## Icon entry `scope-blocked`

| Field | Value |
|---|---|
| icon_id | `scope-blocked` |
| semantic_intent | Blocked future lane |
| preferred_surface | Capture Scope |
| linked_state | `blocked` |
| linked_visual_asset | `U4:scope-blocked` / pending external U4 refresh |
| license | ScoutFlow-original candidate path; third-party license not invoked |
| viewBox | `0 0 24 24` |

```svg
<svg viewBox="0 0 24 24" role="img" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M6 6L18 18 M18 6L6 18 M4 20H20"/></svg>
```
...
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `scope-blocked: blocked`.
...
## Icon entry `trust-node`

| Field | Value |
|---|---|
| icon_id | `trust-node` |
| semantic_intent | Trust Trace node |
| preferred_surface | Trust Trace |
| linked_state | `ready` |
| linked_visual_asset | `U4:trust-node` / pending external U4 refresh |
| license | ScoutFlow-original candidate path; third-party license not invoked |
| viewBox | `0 0 24 24` |

```svg
<svg viewBox="0 0 24 24" role="img" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 4C16 4 20 8 20 12C20 16 16 20 12 20C8 20 4 16 4 12C4 8 8 4 12 4Z M8 12H16"/></svg>
```

### Usage

- Use stroke-only at 1.5–1.8px on dark panels; inherit `currentColor` from semantic token, usually `sf.text.secondary`, `sf.accent.cyan`, `sf.success`, `sf.warning` or `sf.error`.
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `trust-node: ready`.
- Do not use this icon as a decorative watermark behind evidence rows; it belongs inside row headers, badges, compact buttons or template annotations.
...
### I03 Icon Spec — scope-blocked / trust-node — audit expansion 1
...
本节适用于 `I03 Icon Spec — scope-blocked / trust-node`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### I03 Icon Spec — scope-blocked / trust-node — audit expansion 2
...
### I03 Icon Spec — scope-blocked / trust-node — audit expansion 3
...
### I03 Icon Spec — scope-blocked / trust-node — audit expansion 4
