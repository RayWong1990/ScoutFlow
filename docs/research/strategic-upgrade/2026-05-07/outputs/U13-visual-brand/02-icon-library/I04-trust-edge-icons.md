---
title: "I04 Icon Spec — trust-edge / receipt-ledger"
...
# I04 Icon Spec — trust-edge / receipt-ledger
...
## Icon entry `trust-edge`

| Field | Value |
|---|---|
| icon_id | `trust-edge` |
| semantic_intent | Trust dependency edge |
| preferred_surface | Trust Trace |
| linked_state | `active` |
| linked_visual_asset | `U4:trust-edge` / pending external U4 refresh |
...
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `trust-edge: active`.
...
## Icon entry `receipt-ledger`

| Field | Value |
|---|---|
| icon_id | `receipt-ledger` |
| semantic_intent | Receipt ledger |
| preferred_surface | Trust Trace |
| linked_state | `preview_only` |
| linked_visual_asset | `U4:receipt-ledger` / pending external U4 refresh |
| license | ScoutFlow-original candidate path; third-party license not invoked |
| viewBox | `0 0 24 24` |
...
- Use stroke-only at 1.5–1.8px on dark panels; inherit `currentColor` from semantic token, usually `sf.text.secondary`, `sf.accent.cyan`, `sf.success`, `sf.warning` or `sf.error`.
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `receipt-ledger: preview_only`.
- Do not use this icon as a decorative watermark behind evidence rows; it belongs inside row headers, badges, compact buttons or template annotations.
...
### I04 Icon Spec — trust-edge / receipt-ledger — audit expansion 1
...
本节适用于 `I04 Icon Spec — trust-edge / receipt-ledger`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### I04 Icon Spec — trust-edge / receipt-ledger — audit expansion 2
...
### I04 Icon Spec — trust-edge / receipt-ledger — audit expansion 3
...
### I04 Icon Spec — trust-edge / receipt-ledger — audit expansion 4
