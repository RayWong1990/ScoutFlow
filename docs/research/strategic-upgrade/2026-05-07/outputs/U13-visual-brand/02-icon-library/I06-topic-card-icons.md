---
title: "I06 Icon Spec — topic-card / topic-vault"
...
# I06 Icon Spec — topic-card / topic-vault
...
## Icon entry `topic-card`

| Field | Value |
|---|---|
| icon_id | `topic-card` |
| semantic_intent | Topic card preview |
| preferred_surface | Topic Card Preview |
| linked_state | `candidate` |
| linked_visual_asset | `U4:topic-card` / pending external U4 refresh |
...
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `topic-card: candidate`.
...
## Icon entry `topic-vault`

| Field | Value |
|---|---|
| icon_id | `topic-vault` |
| semantic_intent | Topic vault surface |
| preferred_surface | Topic Card Vault |
| linked_state | `candidate` |
| linked_visual_asset | `U4:topic-vault` / pending external U4 refresh |
| license | ScoutFlow-original candidate path; third-party license not invoked |
| viewBox | `0 0 24 24` |
...
- Use stroke-only at 1.5–1.8px on dark panels; inherit `currentColor` from semantic token, usually `sf.text.secondary`, `sf.accent.cyan`, `sf.success`, `sf.warning` or `sf.error`.
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `topic-vault: candidate`.
- Do not use this icon as a decorative watermark behind evidence rows; it belongs inside row headers, badges, compact buttons or template annotations.
...
### I06 Icon Spec — topic-card / topic-vault — audit expansion 1
...
本节适用于 `I06 Icon Spec — topic-card / topic-vault`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### I06 Icon Spec — topic-card / topic-vault — audit expansion 2
...
### I06 Icon Spec — topic-card / topic-vault — audit expansion 3
...
### I06 Icon Spec — topic-card / topic-vault — audit expansion 4
