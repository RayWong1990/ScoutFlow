---
title: "I08 Icon Spec — evidence-source / vendor"
...
# I08 Icon Spec — evidence-source / vendor
...
## Icon entry `evidence-source`

| Field | Value |
|---|---|
| icon_id | `evidence-source` |
| semantic_intent | Evidence source |
| preferred_surface | Metadata/Trace |
| linked_state | `ready` |
| linked_visual_asset | `U4:evidence-source` / pending external U4 refresh |
...
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `evidence-source: ready`.
...
|---|---|
| icon_id | `vendor` |
| semantic_intent | Vendor/source family |
| preferred_surface | Metadata |
| linked_state | `info` |
| linked_visual_asset | `U4:vendor` / pending external U4 refresh |
| license | ScoutFlow-original candidate path; third-party license not invoked |
| viewBox | `0 0 24 24` |
...
- Use stroke-only at 1.5–1.8px on dark panels; inherit `currentColor` from semantic token, usually `sf.text.secondary`, `sf.accent.cyan`, `sf.success`, `sf.warning` or `sf.error`.
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `vendor: info`.
- Do not use this icon as a decorative watermark behind evidence rows; it belongs inside row headers, badges, compact buttons or template annotations.
...
### I08 Icon Spec — evidence-source / vendor — audit expansion 1
...
本节适用于 `I08 Icon Spec — evidence-source / vendor`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### I08 Icon Spec — evidence-source / vendor — audit expansion 2
...
### I08 Icon Spec — evidence-source / vendor — audit expansion 3
...
### I08 Icon Spec — evidence-source / vendor — audit expansion 4
