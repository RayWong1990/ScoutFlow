---
title: "I10 Icon Spec — runtime-block / browser-block"
...
# I10 Icon Spec — runtime-block / browser-block
...
## Icon entry `runtime-block`

| Field | Value |
|---|---|
| icon_id | `runtime-block` |
| semantic_intent | Runtime blocked |
| preferred_surface | Boundary |
| linked_state | `blocked` |
| linked_visual_asset | `U4:runtime-block` / pending external U4 refresh |
| license | ScoutFlow-original candidate path; third-party license not invoked |
| viewBox | `0 0 24 24` |

```svg
<svg viewBox="0 0 24 24" role="img" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M4 12H10 M14 12H20 M10 8L14 12L10 16"/></svg>
```
...
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `runtime-block: blocked`.
...
## Icon entry `browser-block`

| Field | Value |
|---|---|
| icon_id | `browser-block` |
| semantic_intent | Browser automation blocked |
| preferred_surface | Boundary |
| linked_state | `blocked` |
| linked_visual_asset | `U4:browser-block` / pending external U4 refresh |
| license | ScoutFlow-original candidate path; third-party license not invoked |
| viewBox | `0 0 24 24` |

```svg
<svg viewBox="0 0 24 24" role="img" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M5 5H19V19H5Z M8 8H16 M8 11H16 M8 14H13 M8 17H15"/></svg>
```

### Usage

- Use stroke-only at 1.5–1.8px on dark panels; inherit `currentColor` from semantic token, usually `sf.text.secondary`, `sf.accent.cyan`, `sf.success`, `sf.warning` or `sf.error`.
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `browser-block: blocked`.
- Do not use this icon as a decorative watermark behind evidence rows; it belongs inside row headers, badges, compact buttons or template annotations.
...
### I10 Icon Spec — runtime-block / browser-block — audit expansion 1
...
本节适用于 `I10 Icon Spec — runtime-block / browser-block`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### I10 Icon Spec — runtime-block / browser-block — audit expansion 2
...
### I10 Icon Spec — runtime-block / browser-block — audit expansion 3
...
### I10 Icon Spec — runtime-block / browser-block — audit expansion 4
