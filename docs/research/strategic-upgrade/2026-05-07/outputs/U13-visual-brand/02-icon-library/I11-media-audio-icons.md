---
title: "I11 Icon Spec — media-audio / transcript-blocked"
...
# I11 Icon Spec — media-audio / transcript-blocked
...
## Icon entry `media-audio`

| Field | Value |
|---|---|
| icon_id | `media-audio` |
| semantic_intent | Media/audio lane |
| preferred_surface | Scope |
| linked_state | `future_gate` |
| linked_visual_asset | `U4:media-audio` / pending external U4 refresh |
...
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `media-audio: future_gate`.
...
## Icon entry `transcript-blocked`

| Field | Value |
|---|---|
| icon_id | `transcript-blocked` |
| semantic_intent | Transcript blocked |
| preferred_surface | Scope |
| linked_state | `blocked` |
| linked_visual_asset | `U4:transcript-blocked` / pending external U4 refresh |
| license | ScoutFlow-original candidate path; third-party license not invoked |
| viewBox | `0 0 24 24` |
...
- Use stroke-only at 1.5–1.8px on dark panels; inherit `currentColor` from semantic token, usually `sf.text.secondary`, `sf.accent.cyan`, `sf.success`, `sf.warning` or `sf.error`.
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `transcript-blocked: blocked`.
- Do not use this icon as a decorative watermark behind evidence rows; it belongs inside row headers, badges, compact buttons or template annotations.
...
### I11 Icon Spec — media-audio / transcript-blocked — audit expansion 1
...
本节适用于 `I11 Icon Spec — media-audio / transcript-blocked`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### I11 Icon Spec — media-audio / transcript-blocked — audit expansion 2
...
### I11 Icon Spec — media-audio / transcript-blocked — audit expansion 3
