---
title: "I12 Icon Spec — graph-fork / route-api"
...
# I12 Icon Spec — graph-fork / route-api
...
## Icon entry `graph-fork`

| Field | Value |
|---|---|
| icon_id | `graph-fork` |
| semantic_intent | Failure branch |
| preferred_surface | Trace |
| linked_state | `probe_failed` |
| linked_visual_asset | `U4:graph-fork` / pending external U4 refresh |
...
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `graph-fork: probe_failed`.
...
## Icon entry `route-api`

| Field | Value |
|---|---|
| icon_id | `route-api` |
| semantic_intent | API route identity |
| preferred_surface | Metadata |
| linked_state | `info` |
| linked_visual_asset | `U4:route-api` / pending external U4 refresh |
| license | ScoutFlow-original candidate path; third-party license not invoked |
| viewBox | `0 0 24 24` |
...
- Use stroke-only at 1.5–1.8px on dark panels; inherit `currentColor` from semantic token, usually `sf.text.secondary`, `sf.accent.cyan`, `sf.success`, `sf.warning` or `sf.error`.
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `route-api: info`.
- Do not use this icon as a decorative watermark behind evidence rows; it belongs inside row headers, badges, compact buttons or template annotations.
...
### I12 Icon Spec — graph-fork / route-api — audit expansion 1
...
本节适用于 `I12 Icon Spec — graph-fork / route-api`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### I12 Icon Spec — graph-fork / route-api — audit expansion 2
...
### I12 Icon Spec — graph-fork / route-api — audit expansion 3
...
### I12 Icon Spec — graph-fork / route-api — audit expansion 4
