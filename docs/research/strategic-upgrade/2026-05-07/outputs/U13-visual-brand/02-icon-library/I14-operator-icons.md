---
title: "I14 Icon Spec — operator / station-shell"
...
# I14 Icon Spec — operator / station-shell
...
|---|---|
| icon_id | `operator` |
| semantic_intent | Single operator |
| preferred_surface | Brand |
| linked_state | `idle` |
| linked_visual_asset | `U4:operator` / pending external U4 refresh |
...
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `operator: idle`.
...
## Icon entry `station-shell`

| Field | Value |
|---|---|
| icon_id | `station-shell` |
| semantic_intent | Station shell |
| preferred_surface | Four Panel Shell |
| linked_state | `idle` |
| linked_visual_asset | `U4:station-shell` / pending external U4 refresh |
| license | ScoutFlow-original candidate path; third-party license not invoked |
| viewBox | `0 0 24 24` |
...
- Use stroke-only at 1.5–1.8px on dark panels; inherit `currentColor` from semantic token, usually `sf.text.secondary`, `sf.accent.cyan`, `sf.success`, `sf.warning` or `sf.error`.
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `station-shell: idle`.
- Do not use this icon as a decorative watermark behind evidence rows; it belongs inside row headers, badges, compact buttons or template annotations.
...
### I14 Icon Spec — operator / station-shell — audit expansion 1
...
本节适用于 `I14 Icon Spec — operator / station-shell`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### I14 Icon Spec — operator / station-shell — audit expansion 2
...
### I14 Icon Spec — operator / station-shell — audit expansion 3
...
### I14 Icon Spec — operator / station-shell — audit expansion 4
