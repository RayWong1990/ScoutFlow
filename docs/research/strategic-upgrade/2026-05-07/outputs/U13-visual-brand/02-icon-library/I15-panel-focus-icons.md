---
title: "I15 Icon Spec — panel-focus / queue"
...
# I15 Icon Spec — panel-focus / queue
...
## Icon entry `panel-focus`

| Field | Value |
|---|---|
| icon_id | `panel-focus` |
| semantic_intent | Focused panel |
| preferred_surface | Panel |
| linked_state | `editing` |
| linked_visual_asset | `U4:panel-focus` / pending external U4 refresh |
...
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `panel-focus: editing`.
...
|---|---|
| icon_id | `queue` |
| semantic_intent | Queue/staging |
| preferred_surface | Operations |
| linked_state | `pending` |
| linked_visual_asset | `U4:queue` / pending external U4 refresh |
| license | ScoutFlow-original candidate path; third-party license not invoked |
| viewBox | `0 0 24 24` |
...
- Use stroke-only at 1.5–1.8px on dark panels; inherit `currentColor` from semantic token, usually `sf.text.secondary`, `sf.accent.cyan`, `sf.success`, `sf.warning` or `sf.error`.
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `queue: pending`.
- Do not use this icon as a decorative watermark behind evidence rows; it belongs inside row headers, badges, compact buttons or template annotations.
...
### I15 Icon Spec — panel-focus / queue — audit expansion 1
...
本节适用于 `I15 Icon Spec — panel-focus / queue`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### I15 Icon Spec — panel-focus / queue — audit expansion 2
...
### I15 Icon Spec — panel-focus / queue — audit expansion 3
...
### I15 Icon Spec — panel-focus / queue — audit expansion 4
