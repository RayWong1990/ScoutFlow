---
title: "I16 Icon Spec — warning-gate / success-proof"
...
# I16 Icon Spec — warning-gate / success-proof
...
## Icon entry `warning-gate`

| Field | Value |
|---|---|
| icon_id | `warning-gate` |
| semantic_intent | Warning gate |
| preferred_surface | State |
| linked_state | `candidate` |
| linked_visual_asset | `U4:warning-gate` / pending external U4 refresh |
...
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `warning-gate: candidate`.
...
## Icon entry `success-proof`

| Field | Value |
|---|---|
| icon_id | `success-proof` |
| semantic_intent | Success proof |
| preferred_surface | State |
| linked_state | `ready` |
| linked_visual_asset | `U4:success-proof` / pending external U4 refresh |
| license | ScoutFlow-original candidate path; third-party license not invoked |
| viewBox | `0 0 24 24` |
...
- Use stroke-only at 1.5–1.8px on dark panels; inherit `currentColor` from semantic token, usually `sf.text.secondary`, `sf.accent.cyan`, `sf.success`, `sf.warning` or `sf.error`.
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `success-proof: ready`.
- Do not use this icon as a decorative watermark behind evidence rows; it belongs inside row headers, badges, compact buttons or template annotations.
...
### I16 Icon Spec — warning-gate / success-proof — audit expansion 1
...
本节适用于 `I16 Icon Spec — warning-gate / success-proof`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### I16 Icon Spec — warning-gate / success-proof — audit expansion 2
...
### I16 Icon Spec — warning-gate / success-proof — audit expansion 3
...
### I16 Icon Spec — warning-gate / success-proof — audit expansion 4
