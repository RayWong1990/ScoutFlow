---
title: "I17 Icon Spec — error-branch / candidate-badge"
...
# I17 Icon Spec — error-branch / candidate-badge
...
## Icon entry `error-branch`

| Field | Value |
|---|---|
| icon_id | `error-branch` |
| semantic_intent | Error branch |
| preferred_surface | Trace |
| linked_state | `probe_failed` |
| linked_visual_asset | `U4:error-branch` / pending external U4 refresh |
...
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `error-branch: probe_failed`.
...
## Icon entry `candidate-badge`

| Field | Value |
|---|---|
| icon_id | `candidate-badge` |
| semantic_intent | Candidate badge |
| preferred_surface | State |
| linked_state | `candidate` |
| linked_visual_asset | `U4:candidate-badge` / pending external U4 refresh |
| license | ScoutFlow-original candidate path; third-party license not invoked |
| viewBox | `0 0 24 24` |
...
- Use stroke-only at 1.5–1.8px on dark panels; inherit `currentColor` from semantic token, usually `sf.text.secondary`, `sf.accent.cyan`, `sf.success`, `sf.warning` or `sf.error`.
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `candidate-badge: candidate`.
- Do not use this icon as a decorative watermark behind evidence rows; it belongs inside row headers, badges, compact buttons or template annotations.
...
### I17 Icon Spec — error-branch / candidate-badge — audit expansion 1
...
本节适用于 `I17 Icon Spec — error-branch / candidate-badge`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### I17 Icon Spec — error-branch / candidate-badge — audit expansion 2
...
### I17 Icon Spec — error-branch / candidate-badge — audit expansion 3
...
### I17 Icon Spec — error-branch / candidate-badge — audit expansion 4
