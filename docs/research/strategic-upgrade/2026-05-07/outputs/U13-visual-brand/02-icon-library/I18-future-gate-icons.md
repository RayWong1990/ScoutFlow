---
title: "I18 Icon Spec — future-gate / dry-run"
...
# I18 Icon Spec — future-gate / dry-run
...
## Icon entry `future-gate`

| Field | Value |
|---|---|
| icon_id | `future-gate` |
| semantic_intent | Future gate |
| preferred_surface | Scope |
| linked_state | `future_gate` |
| linked_visual_asset | `U4:future-gate` / pending external U4 refresh |
...
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `future-gate: future_gate`.
...
|---|---|
| icon_id | `dry-run` |
| semantic_intent | Dry run |
| preferred_surface | Operations |
| linked_state | `preview_only` |
| linked_visual_asset | `U4:dry-run` / pending external U4 refresh |
| license | ScoutFlow-original candidate path; third-party license not invoked |
| viewBox | `0 0 24 24` |
...
- Use stroke-only at 1.5–1.8px on dark panels; inherit `currentColor` from semantic token, usually `sf.text.secondary`, `sf.accent.cyan`, `sf.success`, `sf.warning` or `sf.error`.
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `dry-run: preview_only`.
- Do not use this icon as a decorative watermark behind evidence rows; it belongs inside row headers, badges, compact buttons or template annotations.
...
### I18 Icon Spec — future-gate / dry-run — audit expansion 1
...
本节适用于 `I18 Icon Spec — future-gate / dry-run`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### I18 Icon Spec — future-gate / dry-run — audit expansion 2
...
### I18 Icon Spec — future-gate / dry-run — audit expansion 3
...
### I18 Icon Spec — future-gate / dry-run — audit expansion 4
