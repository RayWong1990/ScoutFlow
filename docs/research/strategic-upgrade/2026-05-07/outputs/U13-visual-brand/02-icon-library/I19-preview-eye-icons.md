---
title: "I19 Icon Spec — preview-eye / vault-lock"
...
# I19 Icon Spec — preview-eye / vault-lock
...
## Icon entry `preview-eye`

| Field | Value |
|---|---|
| icon_id | `preview-eye` |
| semantic_intent | Preview visibility |
| preferred_surface | Preview |
| linked_state | `preview_only` |
| linked_visual_asset | `U4:preview-eye` / pending external U4 refresh |
...
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `preview-eye: preview_only`.
...
## Icon entry `vault-lock`

| Field | Value |
|---|---|
| icon_id | `vault-lock` |
| semantic_intent | Vault lock |
| preferred_surface | Vault |
| linked_state | `blocked` |
| linked_visual_asset | `U4:vault-lock` / pending external U4 refresh |
| license | ScoutFlow-original candidate path; third-party license not invoked |
| viewBox | `0 0 24 24` |
...
- Use stroke-only at 1.5–1.8px on dark panels; inherit `currentColor` from semantic token, usually `sf.text.secondary`, `sf.accent.cyan`, `sf.success`, `sf.warning` or `sf.error`.
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `vault-lock: blocked`.
- Do not use this icon as a decorative watermark behind evidence rows; it belongs inside row headers, badges, compact buttons or template annotations.
...
### I19 Icon Spec — preview-eye / vault-lock — audit expansion 1
...
本节适用于 `I19 Icon Spec — preview-eye / vault-lock`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### I19 Icon Spec — preview-eye / vault-lock — audit expansion 2
...
### I19 Icon Spec — preview-eye / vault-lock — audit expansion 3
...
### I19 Icon Spec — preview-eye / vault-lock — audit expansion 4
