---
title: "I24 Icon Spec — raw-vault / para"
...
## Icon entry `raw-vault`

| Field | Value |
|---|---|
| icon_id | `raw-vault` |
| semantic_intent | RAW vault |
| preferred_surface | Vault |
| linked_state | `preview_only` |
| linked_visual_asset | `U4:raw-vault` / pending external U4 refresh |
...
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `raw-vault: preview_only`.
...
|---|---|
| icon_id | `para` |
| semantic_intent | PARA destination |
| preferred_surface | Vault |
| linked_state | `info` |
| linked_visual_asset | `U4:para` / pending external U4 refresh |
| license | ScoutFlow-original candidate path; third-party license not invoked |
| viewBox | `0 0 24 24` |
...
- Use stroke-only at 1.5–1.8px on dark panels; inherit `currentColor` from semantic token, usually `sf.text.secondary`, `sf.accent.cyan`, `sf.success`, `sf.warning` or `sf.error`.
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `para: info`.
- Do not use this icon as a decorative watermark behind evidence rows; it belongs inside row headers, badges, compact buttons or template annotations.
...
### I24 Icon Spec — raw-vault / para — audit expansion 1
...
本节适用于 `I24 Icon Spec — raw-vault / para`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### I24 Icon Spec — raw-vault / para — audit expansion 2
...
### I24 Icon Spec — raw-vault / para — audit expansion 3
...
### I24 Icon Spec — raw-vault / para — audit expansion 4
