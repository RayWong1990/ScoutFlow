---
title: "I25 Icon Spec — source-refresh / five-gate"
...
# I25 Icon Spec — source-refresh / five-gate
...
## Icon entry `source-refresh`

| Field | Value |
|---|---|
| icon_id | `source-refresh` |
| semantic_intent | Source refresh |
| preferred_surface | Evidence |
| linked_state | `needs_refresh` |
| linked_visual_asset | `U4:source-refresh` / pending external U4 refresh |
...
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `source-refresh: needs_refresh`.
...
## Icon entry `five-gate`

| Field | Value |
|---|---|
| icon_id | `five-gate` |
| semantic_intent | 5 Gate |
| preferred_surface | Review |
| linked_state | `ready` |
| linked_visual_asset | `U4:five-gate` / pending external U4 refresh |
| license | ScoutFlow-original candidate path; third-party license not invoked |
| viewBox | `0 0 24 24` |
...
- Use stroke-only at 1.5–1.8px on dark panels; inherit `currentColor` from semantic token, usually `sf.text.secondary`, `sf.accent.cyan`, `sf.success`, `sf.warning` or `sf.error`.
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `five-gate: ready`.
- Do not use this icon as a decorative watermark behind evidence rows; it belongs inside row headers, badges, compact buttons or template annotations.
...
### I25 Icon Spec — source-refresh / five-gate — audit expansion 1
...
本节适用于 `I25 Icon Spec — source-refresh / five-gate`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### I25 Icon Spec — source-refresh / five-gate — audit expansion 2
...
### I25 Icon Spec — source-refresh / five-gate — audit expansion 3
...
### I25 Icon Spec — source-refresh / five-gate — audit expansion 4
