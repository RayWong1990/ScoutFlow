---
title: "I27 Icon Spec — spacing / motion"
...
|---|---|
| icon_id | `spacing` |
| semantic_intent | Spacing grid |
| preferred_surface | Token |
| linked_state | `ready` |
| linked_visual_asset | `U4:spacing` / pending external U4 refresh |
...
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `spacing: ready`.
...
|---|---|
| icon_id | `motion` |
| semantic_intent | Motion token |
| preferred_surface | Token |
| linked_state | `ready` |
| linked_visual_asset | `U4:motion` / pending external U4 refresh |
| license | ScoutFlow-original candidate path; third-party license not invoked |
| viewBox | `0 0 24 24` |
...
- Use stroke-only at 1.5–1.8px on dark panels; inherit `currentColor` from semantic token, usually `sf.text.secondary`, `sf.accent.cyan`, `sf.success`, `sf.warning` or `sf.error`.
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `motion: ready`.
- Do not use this icon as a decorative watermark behind evidence rows; it belongs inside row headers, badges, compact buttons or template annotations.
...
### I27 Icon Spec — spacing / motion — audit expansion 1
...
本节适用于 `I27 Icon Spec — spacing / motion`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### I27 Icon Spec — spacing / motion — audit expansion 2
...
### I27 Icon Spec — spacing / motion — audit expansion 3
...
### I27 Icon Spec — spacing / motion — audit expansion 4
