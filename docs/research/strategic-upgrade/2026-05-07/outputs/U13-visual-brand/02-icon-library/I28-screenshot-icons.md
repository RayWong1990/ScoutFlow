---
title: "I28 Icon Spec — screenshot / mobile-safe"
...
# I28 Icon Spec — screenshot / mobile-safe
...
## Icon entry `screenshot`

| Field | Value |
|---|---|
| icon_id | `screenshot` |
| semantic_intent | Screenshot evidence |
| preferred_surface | Review |
| linked_state | `future_gate` |
| linked_visual_asset | `U4:screenshot` / pending external U4 refresh |
...
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `screenshot: future_gate`.
...
## Icon entry `mobile-safe`

| Field | Value |
|---|---|
| icon_id | `mobile-safe` |
| semantic_intent | Mobile safe area |
| preferred_surface | Template |
| linked_state | `ready` |
| linked_visual_asset | `U4:mobile-safe` / pending external U4 refresh |
| license | ScoutFlow-original candidate path; third-party license not invoked |
| viewBox | `0 0 24 24` |
...
- Use stroke-only at 1.5–1.8px on dark panels; inherit `currentColor` from semantic token, usually `sf.text.secondary`, `sf.accent.cyan`, `sf.success`, `sf.warning` or `sf.error`.
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `mobile-safe: ready`.
- Do not use this icon as a decorative watermark behind evidence rows; it belongs inside row headers, badges, compact buttons or template annotations.
...
### I28 Icon Spec — screenshot / mobile-safe — audit expansion 1
...
本节适用于 `I28 Icon Spec — screenshot / mobile-safe`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### I28 Icon Spec — screenshot / mobile-safe — audit expansion 2
...
### I28 Icon Spec — screenshot / mobile-safe — audit expansion 3
...
### I28 Icon Spec — screenshot / mobile-safe — audit expansion 4
