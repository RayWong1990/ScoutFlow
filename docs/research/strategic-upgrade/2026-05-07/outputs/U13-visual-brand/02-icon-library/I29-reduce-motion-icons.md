---
title: "I29 Icon Spec — reduce-motion / license-shield"
...
# I29 Icon Spec — reduce-motion / license-shield
...
## Icon entry `reduce-motion`

| Field | Value |
|---|---|
| icon_id | `reduce-motion` |
| semantic_intent | Reduced motion |
| preferred_surface | Accessibility |
| linked_state | `ready` |
| linked_visual_asset | `U4:reduce-motion` / pending external U4 refresh |
...
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `reduce-motion: ready`.
...
## Icon entry `license-shield`

| Field | Value |
|---|---|
| icon_id | `license-shield` |
| semantic_intent | License shield |
| preferred_surface | License |
| linked_state | `needs_review` |
| linked_visual_asset | `U4:license-shield` / pending external U4 refresh |
| license | ScoutFlow-original candidate path; third-party license not invoked |
| viewBox | `0 0 24 24` |
...
- Use stroke-only at 1.5–1.8px on dark panels; inherit `currentColor` from semantic token, usually `sf.text.secondary`, `sf.accent.cyan`, `sf.success`, `sf.warning` or `sf.error`.
- Pair icon with visible text. For state-bearing icons, screen reader label must include the state word, for example `license-shield: needs_review`.
- Do not use this icon as a decorative watermark behind evidence rows; it belongs inside row headers, badges, compact buttons or template annotations.
...
### I29 Icon Spec — reduce-motion / license-shield — audit expansion 1
...
本节适用于 `I29 Icon Spec — reduce-motion / license-shield`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### I29 Icon Spec — reduce-motion / license-shield — audit expansion 2
...
### I29 Icon Spec — reduce-motion / license-shield — audit expansion 3
...
### I29 Icon Spec — reduce-motion / license-shield — audit expansion 4
