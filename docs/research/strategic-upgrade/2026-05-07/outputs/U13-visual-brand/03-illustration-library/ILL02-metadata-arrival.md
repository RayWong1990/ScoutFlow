---
title: "ILL02 Illustration Spec — metadata-arrival / scope-lane-stack"
...
# ILL02 Illustration Spec — metadata-arrival / scope-lane-stack
...
## Illustration `metadata-arrival`

| Field | Value |
|---|---|
| illustration_id | `metadata-arrival` |
| intent | Metadata arriving |
| concept | evidence rows settling from empty to ready |
| linked_state | `idle / ready / candidate / future_gate / blocked` as applicable |
| linked_visual_asset | `U4:metadata-arrival` pending external refresh |
...
## Illustration `scope-lane-stack`

| Field | Value |
|---|---|
| illustration_id | `scope-lane-stack` |
| intent | Capture Scope lanes |
| concept | allowed current lane plus blocked future lanes |
| linked_state | `idle / ready / candidate / future_gate / blocked` as applicable |
| linked_visual_asset | `U4:scope-lane-stack` pending external refresh |
| license | ScoutFlow-original candidate geometry; no third-party art copied |
| format | Inline SVG concept, scalable |
...
### ILL02 Illustration Spec — metadata-arrival / scope-lane-stack — audit expansion 1
...
本节适用于 `ILL02 Illustration Spec — metadata-arrival / scope-lane-stack`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### ILL02 Illustration Spec — metadata-arrival / scope-lane-stack — audit expansion 2
...
### ILL02 Illustration Spec — metadata-arrival / scope-lane-stack — audit expansion 3
...
### ILL02 Illustration Spec — metadata-arrival / scope-lane-stack — audit expansion 4
...
### ILL02 Illustration Spec — metadata-arrival / scope-lane-stack — audit expansion 5
