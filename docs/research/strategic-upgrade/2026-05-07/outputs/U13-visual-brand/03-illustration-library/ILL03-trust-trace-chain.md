---
title: "ILL03 Illustration Spec — trust-trace-chain / topic-preview-card"
...
# ILL03 Illustration Spec — trust-trace-chain / topic-preview-card
...
## Illustration `trust-trace-chain`

| Field | Value |
|---|---|
| illustration_id | `trust-trace-chain` |
| intent | Trust Trace chain |
| concept | capture -> state -> job -> evidence -> receipt -> media -> audit |
| linked_state | `idle / ready / candidate / future_gate / blocked` as applicable |
| linked_visual_asset | `U4:trust-trace-chain` pending external refresh |
...
## Illustration `topic-preview-card`

| Field | Value |
|---|---|
| illustration_id | `topic-preview-card` |
| intent | Topic preview card |
| concept | candidate topic card before vault write |
| linked_state | `idle / ready / candidate / future_gate / blocked` as applicable |
| linked_visual_asset | `U4:topic-preview-card` pending external refresh |
| license | ScoutFlow-original candidate geometry; no third-party art copied |
| format | Inline SVG concept, scalable |
...
### ILL03 Illustration Spec — trust-trace-chain / topic-preview-card — audit expansion 1
...
本节适用于 `ILL03 Illustration Spec — trust-trace-chain / topic-preview-card`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### ILL03 Illustration Spec — trust-trace-chain / topic-preview-card — audit expansion 2
...
### ILL03 Illustration Spec — trust-trace-chain / topic-preview-card — audit expansion 3
...
### ILL03 Illustration Spec — trust-trace-chain / topic-preview-card — audit expansion 4
...
### ILL03 Illustration Spec — trust-trace-chain / topic-preview-card — audit expansion 5
...
### ILL03 Illustration Spec — trust-trace-chain / topic-preview-card — audit expansion 6
