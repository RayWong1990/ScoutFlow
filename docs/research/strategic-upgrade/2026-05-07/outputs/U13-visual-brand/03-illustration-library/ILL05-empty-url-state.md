---
title: "ILL05 Illustration Spec — empty-url-state / empty-metadata-state"
...
# ILL05 Illustration Spec — empty-url-state / empty-metadata-state
...
## Illustration `empty-url-state`

| Field | Value |
|---|---|
| illustration_id | `empty-url-state` |
| intent | Empty URL state |
| concept | operator needs paste action |
| linked_state | `idle / ready / candidate / future_gate / blocked` as applicable |
| linked_visual_asset | `U4:empty-url-state` pending external refresh |
...
## Illustration `empty-metadata-state`

| Field | Value |
|---|---|
| illustration_id | `empty-metadata-state` |
| intent | Empty metadata state |
| concept | unknown values not fake placeholders |
| linked_state | `idle / ready / candidate / future_gate / blocked` as applicable |
| linked_visual_asset | `U4:empty-metadata-state` pending external refresh |
| license | ScoutFlow-original candidate geometry; no third-party art copied |
| format | Inline SVG concept, scalable |
...
### ILL05 Illustration Spec — empty-url-state / empty-metadata-state — audit expansion 1
...
本节适用于 `ILL05 Illustration Spec — empty-url-state / empty-metadata-state`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### ILL05 Illustration Spec — empty-url-state / empty-metadata-state — audit expansion 2
...
### ILL05 Illustration Spec — empty-url-state / empty-metadata-state — audit expansion 3
...
### ILL05 Illustration Spec — empty-url-state / empty-metadata-state — audit expansion 4
...
### ILL05 Illustration Spec — empty-url-state / empty-metadata-state — audit expansion 5
...
### ILL05 Illustration Spec — empty-url-state / empty-metadata-state — audit expansion 6
