---
title: "ILL06 Illustration Spec — probe-failed-state / loading-settle-state"
...
# ILL06 Illustration Spec — probe-failed-state / loading-settle-state
...
## Illustration `probe-failed-state`

| Field | Value |
|---|---|
| illustration_id | `probe-failed-state` |
| intent | Probe failed state |
| concept | inline reason and retry boundary |
| linked_state | `idle / ready / candidate / future_gate / blocked` as applicable |
| linked_visual_asset | `U4:probe-failed-state` pending external refresh |
...
## Illustration `loading-settle-state`

| Field | Value |
|---|---|
| illustration_id | `loading-settle-state` |
| intent | Loading settle state |
| concept | subtle row-level motion |
| linked_state | `idle / ready / candidate / future_gate / blocked` as applicable |
| linked_visual_asset | `U4:loading-settle-state` pending external refresh |
| license | ScoutFlow-original candidate geometry; no third-party art copied |
| format | Inline SVG concept, scalable |
...
### ILL06 Illustration Spec — probe-failed-state / loading-settle-state — audit expansion 1
...
本节适用于 `ILL06 Illustration Spec — probe-failed-state / loading-settle-state`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### ILL06 Illustration Spec — probe-failed-state / loading-settle-state — audit expansion 2
...
### ILL06 Illustration Spec — probe-failed-state / loading-settle-state — audit expansion 3
...
### ILL06 Illustration Spec — probe-failed-state / loading-settle-state — audit expansion 4
...
### ILL06 Illustration Spec — probe-failed-state / loading-settle-state — audit expansion 5
