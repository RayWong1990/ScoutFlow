---
title: "ILL09 Illustration Spec — token-map / icon-grid"
...
# ILL09 Illustration Spec — token-map / icon-grid
...
## Illustration `token-map`

| Field | Value |
|---|---|
| illustration_id | `token-map` |
| intent | Token relationship map |
| concept | palette/spacing/type/motion links |
| linked_state | `idle / ready / candidate / future_gate / blocked` as applicable |
| linked_visual_asset | `U4:token-map` pending external refresh |
...
| Field | Value |
|---|---|
| illustration_id | `icon-grid` |
| intent | Icon grid illustration |
| concept | 60 symbolic assets |
| linked_state | `idle / ready / candidate / future_gate / blocked` as applicable |
| linked_visual_asset | `U4:icon-grid` pending external refresh |
| license | ScoutFlow-original candidate geometry; no third-party art copied |
| format | Inline SVG concept, scalable |
...
### ILL09 Illustration Spec — token-map / icon-grid — audit expansion 1
...
本节适用于 `ILL09 Illustration Spec — token-map / icon-grid`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### ILL09 Illustration Spec — token-map / icon-grid — audit expansion 2
...
### ILL09 Illustration Spec — token-map / icon-grid — audit expansion 3
...
### ILL09 Illustration Spec — token-map / icon-grid — audit expansion 4
...
### ILL09 Illustration Spec — token-map / icon-grid — audit expansion 5
...
### ILL09 Illustration Spec — token-map / icon-grid — audit expansion 6
