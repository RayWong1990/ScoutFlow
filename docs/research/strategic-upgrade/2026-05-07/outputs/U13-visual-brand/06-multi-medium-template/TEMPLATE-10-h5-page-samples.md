---
title: TEMPLATE-10 H5 Page Layout Samples
...
# TEMPLATE-10 H5 Page Layout Samples

## Template mission

`H5 Page Layout Samples` is a multi-medium brand surface for ScoutFlow. It must feel like the same evidence workstation whether exported as a slide, poster, social card or H5 view.

| Field | Value |
|---|---|
| template_id | `h5-page-samples` |
| canvas / size | desktop/tablet/mobile |
| primary slots | 10 layout samples: station, preview, audit, report, index, handoff, empty, error, loading, comparison |
| linked_visual_asset | `U4:h5-page-samples` pending external refresh |
...
## Ten H5 layout samples

| Sample | Slot order | Notes |
|---|---|---|
| station-default | URL → Metadata → Scope → Trace | canonical |
| station-compact | URL → Metadata/Scope → Trace | laptop |
| station-mobile | URL → Metadata → Scope → Trace | safe area |
| topic-preview | Header → Evidence → Preview shelf | no authority write |
| vault-preview | Preview → receipt → blocked true write | future_gate visible |
| visual-audit | Gate cards → case table → verdict | human-in-loop |
| report-index | Phase filters → report cards | no KPI dashboard drift |
| handoff | Source digest → next gates | cold-start |
| empty-state | action prompt → blocked lanes | no fake data |
| error-state | inline reason → retry boundary | no toast-only error |
...
```mermaid
flowchart LR
  brand[Deep sea brand tokens] --> grid[Template grid]
  grid --> claim[Claim / title]
  grid --> evidence[Evidence panel]
  grid --> status[State strip]
  status --> gates[Next gate / boundary]
```
...
### TEMPLATE-10 H5 Page Layout Samples — audit expansion 1
...
本节适用于 `TEMPLATE-10 H5 Page Layout Samples`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### TEMPLATE-10 H5 Page Layout Samples — audit expansion 2
...
### TEMPLATE-10 H5 Page Layout Samples — audit expansion 3
...
### TEMPLATE-10 H5 Page Layout Samples — audit expansion 4
...
### TEMPLATE-10 H5 Page Layout Samples — audit expansion 5
...
### TEMPLATE-10 H5 Page Layout Samples — audit expansion 6
...
### TEMPLATE-10 H5 Page Layout Samples — audit expansion 7
...
### TEMPLATE-10 H5 Page Layout Samples — audit expansion 8
