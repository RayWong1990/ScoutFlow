---
title: "PHASE-01 Foundation: palette/token and four-panel IA"
...
# PHASE-01 Foundation: palette/token and four-panel IA

## Phase mission

U13 tokens stabilize visual vocabulary before image generation. This phase map is a candidate planning artifact, not a promotion of any authority document.

## Evolution path

```mermaid
flowchart LR
  U13[U13 Brand Atlas] --> U4[U4 Visual Asset Spine]
  U13 --> U7[U7 State Library]
  U4 --> PFV[PF-V GPT-Image-2 prompts]
  U7 --> PFV
  PFV --> Review[5-Gate human review]
  Review --> Dispatch[future dispatch evidence]
  Dispatch --> Defer[defer or promote by separate gate]
```
...
|---|---|---|
| Claim label | candidate/not-authority visible | user assumes deployment approval |
| Asset reuse | token/icon/illustration IDs reused | prompt invents new visual language |
| State bridge | U7 state word attached | blocked lane looks active |
| Review | 5-Gate cases referenced | technical render called visual pass |
| Boundary | no package/runtime/browser/vault unlock | scope creep |

## Output expectation

- Phase assets should reference `linked_visual_asset` and `linked_state` fields.
- PF-V generated images should include a prompt hash, asset IDs and state IDs.
- Dispatch audits should record what was actually rendered and what remains gated.
- Cross-phase drift should be treated as a product quality risk, not a style preference.

## Human checkpoints

Human review is required when a phase claims visual pass, uses third-party assets, changes production code, unlocks screenshots/browser automation, or touches authority docs.
...
### PHASE-01 Foundation: palette/token and four-panel IA — audit expansion 1
...
本节适用于 `PHASE-01 Foundation: palette/token and four-panel IA`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### PHASE-01 Foundation: palette/token and four-panel IA — audit expansion 2
...
### PHASE-01 Foundation: palette/token and four-panel IA — audit expansion 3
...
### PHASE-01 Foundation: palette/token and four-panel IA — audit expansion 4
...
### PHASE-01 Foundation: palette/token and four-panel IA — audit expansion 5
...
### PHASE-01 Foundation: palette/token and four-panel IA — audit expansion 6
...
### PHASE-01 Foundation: palette/token and four-panel IA — audit expansion 7
...
### PHASE-01 Foundation: palette/token and four-panel IA — audit expansion 8
...
### PHASE-01 Foundation: palette/token and four-panel IA — audit expansion 9
...
### PHASE-01 Foundation: palette/token and four-panel IA — audit expansion 10
