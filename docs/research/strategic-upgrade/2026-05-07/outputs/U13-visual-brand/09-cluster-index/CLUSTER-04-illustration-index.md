...
# CLUSTER-04 Illustration Library Index

## Cluster scope

This index covers 30 illustration candidates in 15 files. It is designed for fast human audit and PF-V prompt assembly.

## Read order

1. Start with README and MASTER-BRAND-ATLAS.
2. Read token files before visual asset files.
3. Read panel specs before template specs.
4. Run the 5-Gate master suite against any image or H5 surface.
5. Check live-web/license/source limitations before production use.

## Cluster map

```mermaid
flowchart TD
  README --> Master[MASTER-BRAND-ATLAS]
  Master --> Tokens
  Master --> Assets
  Assets --> Icons
  ...
  Tokens --> Panels
  Panels --> Templates
  Templates --> Gates[5-Gate]
  Gates --> Phase[Phase Maps]
  Phase --> Linked[Linked Asset-State]
```

## Index rows

| Item type | Expected files | Coverage note |
|---|---:|---|
| token | 5 | single-source candidate tokens |
| icon | 30 | two icons per file, 60 total |
| illustration | 15 | two illustrations per file, 30 total |
| pattern | 10 | two patterns per file, 20 total |
| panel | 8 | four-panel plus topic/nav specs |
| template | 10 | PPT/poster/social/H5 |
| five-gate | 5 + master | 30+ cases |
| phase | 5 | cross-phase evolution |

## Audit reminders

- This cluster is not authority.
- Missing external U4/U7/live web evidence is explicit, not hidden.
- Every consumer should preserve state labels, token names, and file IDs when generating new prompts.
...
### CLUSTER-04 Illustration Library Index — audit expansion 1
...
本节适用于 `CLUSTER-04 Illustration Library Index`：审查者应记录 source label、state word、token reference、layout slot、failure class、defer condition 与 human reviewer note。若证据缺失，写 `needs_evidence`，不得写 `passed by assumption`。


### CLUSTER-04 Illustration Library Index — audit expansion 2
...
### CLUSTER-04 Illustration Library Index — audit expansion 3
...
### CLUSTER-04 Illustration Library Index — audit expansion 4
...
### CLUSTER-04 Illustration Library Index — audit expansion 5
...
### CLUSTER-04 Illustration Library Index — audit expansion 6
...
### CLUSTER-04 Illustration Library Index — audit expansion 7
...
### CLUSTER-04 Illustration Library Index — audit expansion 8
