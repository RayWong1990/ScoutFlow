---
title: "Whisper.cpp Metal baseline for local ASR intake"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "A01"
cluster: "A_ASR"
framework: "whisper.cpp + Metal"
model: "Whisper family GGML/GGUF local models"
min_chip: "M1"
...
# A01 — Whisper.cpp Metal baseline for local ASR intake
...
[recipe-spec] Private long-form audio needs deterministic local transcription with usable segment timestamps.

## Optimal local configuration

[recipe-spec] Compile or install a Metal-capable whisper.cpp binary, pin the model file by hash, normalize audio before transcription, and use a stable JSON transcript schema. Use the smaller reliable model for hot-path triage and reserve larger models for cold-path repair.
...
|---|---|---|
| real_time_factor | paste-time measurement | no value included |
...
| language_accuracy | paste-time measurement | no value included |
...
```bash
whisper-cli -m models/whisper.gguf -f input.wav -oj -otxt
```
...
## Extended operating rationale — Whisper.cpp Metal baseline for local ASR intake
