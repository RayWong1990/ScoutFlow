---
title: "Whisper.cpp quantization and thread tuning"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "A02"
cluster: "A_ASR"
framework: "whisper.cpp + Metal + Accelerate"
model: "Whisper quantized local variants"
min_chip: "M1"
...
# A02 — Whisper.cpp quantization and thread tuning
...
[recipe-spec] Choosing quant level and CPU thread count without starving the UI or damaging transcript quality.

## Optimal local configuration

[recipe-spec] Run the same fixture corpus across selected quantizations, keep the decoder settings stable, and compare quality before speed. Limit CPU threads when the machine is used interactively, and serialize heavy transcription jobs until memory and thermal evidence supports concurrency.
...
|---|---|---|
| rtf_by_quant | paste-time measurement | no value included |
| manual_error_flags | paste-time measurement | no value included |
| thread_count | paste-time measurement | no value included |
...
```bash
for m in models/*.gguf; do whisper-cli -m "$m" -f fixture.wav -oj; done
```

```bash
ps -o pid,pcpu,pmem,command -p $PID
```
...
## Extended operating rationale — Whisper.cpp quantization and thread tuning
