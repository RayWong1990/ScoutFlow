---
title: "Parakeet TDT 0.6B Apple Silicon ASR candidate"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "A05"
cluster: "A_ASR"
framework: "Parakeet local wrapper"
...
# A05 — Parakeet TDT 0.6B Apple Silicon ASR candidate
...
[recipe-spec] A prompt-named ASR model should be evaluated for clean speech, long-form material, and Apple Silicon viability.

## Optimal local configuration

[recipe-spec] First verify availability, license, model identity, and local runtime. Use the same normalized WAV corpus as Whisper. Compare punctuation, timestamp support, language coverage, hallucination risk, and memory profile before deciding whether it belongs in hot or cold path.
...
|---|---|---|
| license_status | paste-time measurement | no value included |
| load_memory | paste-time measurement | no value included |
| rtf | paste-time measurement | no value included |
| timestamp_support | paste-time measurement | no value included |
...
```bash
ffmpeg -i input.mp4 -ar 16000 -ac 1 fixture.wav
```

```bash
python run_parakeet_candidate.py --input fixture.wav --json out.json
```
...
## Extended operating rationale — Parakeet TDT 0.6B Apple Silicon ASR candidate
