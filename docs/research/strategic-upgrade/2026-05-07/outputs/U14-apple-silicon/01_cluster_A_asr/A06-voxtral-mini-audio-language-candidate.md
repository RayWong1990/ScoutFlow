---
title: "Voxtral Mini audio-language candidate"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "A06"
cluster: "A_ASR"
framework: "Voxtral local audio LLM wrapper"
model: "Voxtral Mini"
min_chip: "M4"
...
# A06 — Voxtral Mini audio-language candidate
...
[recipe-spec] An audio-language model may help with clip reasoning, but it should not replace deterministic ASR.

## Optimal local configuration

[recipe-spec] Use Voxtral-like audio-language models only as cold-path reviewers for short clips already segmented and transcribed. Record whether it improves summaries, intent labels, and anomaly tagging without inventing content.
...
|---|---|---|
| clip_latency | paste-time measurement | no value included |
| memory_peak | paste-time measurement | no value included |
| agreement_with_asr | paste-time measurement | no value included |
| hallucination_flags | paste-time measurement | no value included |
| review_time | paste-time measurement | no value included |
...
```bash
python run_voxtral_candidate.py --clip clip.wav --transcript clip.json --out review.json
```

```bash
python compare_audio_llm_review.py
```
...
- Whisper transcript plus text LLM
- manual review
- smaller text-only model
- disable for sensitive raw audio
...
## Extended operating rationale — Voxtral Mini audio-language candidate
