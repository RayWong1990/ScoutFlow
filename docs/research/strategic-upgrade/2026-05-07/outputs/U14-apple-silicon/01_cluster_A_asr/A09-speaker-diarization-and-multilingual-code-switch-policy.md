---
title: "Speaker diarization and multilingual code-switch policy"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "A09"
cluster: "A_ASR"
framework: "pyannote-style diarization + language router"
model: "speaker diarization model plus multilingual ASR"
min_chip: "M2"
...
# A09 — Speaker diarization and multilingual code-switch policy
...
[recipe-spec] Meeting audio and Chinese-English material need speaker turns and stable language handling.

## Optimal local configuration

[recipe-spec] Run diarization after audio normalization and before transcript stitching when multi-speaker context matters. Detect suspicious language flips, preserve original script, and store anonymous speaker labels unless identity mapping is explicitly approved.
...
|---|---|---|
| diarization_review | paste-time measurement | no value included |
| language_flip_count | paste-time measurement | no value included |
| overlap_handling | paste-time measurement | no value included |
| entity_preservation | paste-time measurement | no value included |
...
```bash
python diarize_candidate.py input.wav --out speakers.rttm
```

```bash
python code_switch_audit.py transcript.json --out language_audit.json
```
...
- no diarization but keep timestamps
- force language per file
- manual speaker labels
- larger multilingual cold pass
...
## Extended operating rationale — Speaker diarization and multilingual code-switch policy
