---
title: "VAD preprocessing with Silero MPS candidate"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "A10"
cluster: "A_ASR"
framework: "Silero VAD / PyTorch MPS watchlist"
...
# A10 — VAD preprocessing with Silero MPS candidate
...
[recipe-spec] Voice activity detection can reduce ASR cost and hallucination risk if it does not cut speech.

## Optimal local configuration

[recipe-spec] Use VAD to propose speech spans, not to delete raw audio. Verify PyTorch MPS availability, compare CPU and MPS results, and keep padding around detected speech. Store VAD spans as sidecar JSON for ASR and review.
...
|---|---|---|
| vad_precision_review | paste-time measurement | no value included |
| speech_cut_count | paste-time measurement | no value included |
| asr_cost_reduction | paste-time measurement | no value included |
| mps_available | paste-time measurement | no value included |
| span_padding_effect | paste-time measurement | no value included |
...
```bash
python vad_segments.py input.wav --out vad.json
```
...
## Extended operating rationale — VAD preprocessing with Silero MPS candidate
