---
title: "sentence-transformers MPS embedding triage"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "B01"
cluster: "B_EMBEDDING_RETRIEVAL"
framework: "sentence-transformers + PyTorch MPS"
model: "bge-m3 / nomic / mxbai candidates"
min_chip: "M2"
...
# B01 — sentence-transformers MPS embedding triage
...
[recipe-spec] Local dense embedding for notes, transcripts, captions, and visual-DAM text metadata.

## Optimal local configuration

[recipe-spec] Pin model revision, normalize chunks, verify PyTorch MPS device use, and compare multilingual retrieval quality against BM25. Use deterministic batching and write model hash into every vector row.
...
|---|---|---|
| docs_per_second | paste-time measurement | no value included |
...
| failed_chunks | paste-time measurement | no value included |
...
```bash
python embed_batch.py --model MODEL --input chunks.jsonl --out vectors.jsonl
```
...
## Extended operating rationale — sentence-transformers MPS embedding triage
