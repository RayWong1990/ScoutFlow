---
title: "sqlite-vec and sqlite-vss local index policy"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "B02"
cluster: "B_EMBEDDING_RETRIEVAL"
framework: "SQLite vector extensions"
model: "sqlite-vec / sqlite-vss"
min_chip: "M1"
...
# B02 — sqlite-vec and sqlite-vss local index policy
...
[recipe-spec] Embedding search should remain close to the local vault without requiring a server.

## Optimal local configuration

[recipe-spec] Use SQLite sidecar tables for source hash, chunk id, embedding model, vector dimension, schema version, and rebuild version. Treat extension loading and Faiss dependency chains as Apple Silicon compatibility checks.
...
|---|---|---|
| extension_load | paste-time measurement | no value included |
| insert_rate | paste-time measurement | no value included |
| query_latency | paste-time measurement | no value included |
| recall_review | paste-time measurement | no value included |
| db_size | paste-time measurement | no value included |
...
```bash
sqlite3 scoutflow_vec.db ".load ./vec0"
```

```bash
python build_sqlite_vector_index.py --vectors vectors.jsonl
```
...
## Extended operating rationale — sqlite-vec and sqlite-vss local index policy
