---
title: "LanceDB Apple Silicon visual-DAM sidecar"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "B03"
cluster: "B_EMBEDDING_RETRIEVAL"
framework: "LanceDB local"
model: "text and multimodal embedding rows"
min_chip: "M2"
...
# B03 — LanceDB Apple Silicon visual-DAM sidecar
...
[recipe-spec] Visual-DAM needs vector search over captions, OCR, pHash groups, thumbnails, and asset metadata.

## Optimal local configuration

[recipe-spec] Use LanceDB as a rebuildable sidecar, not the system of record. Store raw media outside the DB and link through source hash, object path, derived artifact id, embedding model, and rebuild version.
...
|---|---|---|
| ingest_rate | paste-time measurement | no value included |
| query_latency | paste-time measurement | no value included |
| index_size | paste-time measurement | no value included |
| recall_review | paste-time measurement | no value included |
| rebuild_time | paste-time measurement | no value included |
...
```bash
python build_lancedb_dam.py --manifest assets.jsonl --table visual_dam
```

```bash
python query_lancedb.py --text "screenshot diagram"
```
...
## Extended operating rationale — LanceDB Apple Silicon visual-DAM sidecar
