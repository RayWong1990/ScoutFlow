---
title: "DuckDB vector and analytic retrieval sidecar"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "B04"
cluster: "B_EMBEDDING_RETRIEVAL"
framework: "DuckDB + vector extension watchlist"
model: "dense vectors plus metadata tables"
min_chip: "M2"
...
# B04 — DuckDB vector and analytic retrieval sidecar
...
[recipe-spec] Retrieval diagnostics need fast metadata joins and batch reporting alongside vector experiments.

## Optimal local configuration

[recipe-spec] Use DuckDB for Parquet, metadata scans, and rebuild reports. Treat vector extension support as a local verification item. Keep export paths to SQLite or LanceDB if a feature is absent.
...
|---|---|---|
| scan_time | paste-time measurement | no value included |
| join_time | paste-time measurement | no value included |
| extension_available | paste-time measurement | no value included |
| report_time | paste-time measurement | no value included |
| schema_validity | paste-time measurement | no value included |
...
```bash
duckdb scoutflow.duckdb "INSTALL vector; LOAD vector;"
```

```bash
python export_chunks_to_parquet.py
```
...
- DuckDB metadata only
- SQLite vector sidecar
- LanceDB sidecar
- Parquet export
...
## Extended operating rationale — DuckDB vector and analytic retrieval sidecar
