---
title: "Ollama local embedding API for privacy-first retrieval"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "B05"
cluster: "B_EMBEDDING_RETRIEVAL"
framework: "Ollama local API"
model: "local embedding models"
min_chip: "M1"
...
# B05 — Ollama local embedding API for privacy-first retrieval
...
[recipe-spec] A localhost embedding API can simplify ScoutFlow integration while keeping data local.

## Optimal local configuration

[recipe-spec] Bind only to localhost, pin model tags, capture `ollama show`, and limit concurrent embedding clients. Treat service convenience as an integration choice, not a cloud dependency.
...
|---|---|---|
| cold_start | paste-time measurement | no value included |
| embeddings_per_second | paste-time measurement | no value included |
| queue_latency | paste-time measurement | no value included |
| model_memory | paste-time measurement | no value included |
...
```bash
curl http://localhost:11434/api/embeddings -d @embed_request.json
```
...
## Extended operating rationale — Ollama local embedding API for privacy-first retrieval
