...
# B06 — Apple Foundation Models embedding watchlist
...
[recipe-spec] A future Swift-native local embedding route could reduce packaging friction if Apple exposes suitable APIs.

## Optimal local configuration

[recipe-spec] Create an adapter boundary that accepts chunks and emits vectors with model identity. Do not change the retrieval schema until availability, privacy behavior, vector dimension, and quality are verified on the target OS.
...
| embedding_dimension | paste-time measurement | no value included |
| recall_review | paste-time measurement | no value included |
...
```bash
swift run FoundationEmbeddingProbe --input chunks.jsonl --out vectors.jsonl
```
...
## Extended operating rationale — Apple Foundation Models embedding watchlist
