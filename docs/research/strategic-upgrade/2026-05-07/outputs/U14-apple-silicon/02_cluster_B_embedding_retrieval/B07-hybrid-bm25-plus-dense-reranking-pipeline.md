---
title: "Hybrid BM25 plus dense reranking pipeline"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "B07"
cluster: "B_EMBEDDING_RETRIEVAL"
framework: "SQLite FTS5 + dense vectors + local reranker"
model: "BM25 plus embedding model plus reranker"
min_chip: "M1"
...
# B07 — Hybrid BM25 plus dense reranking pipeline
...
[recipe-spec] ScoutFlow search needs exact terms, semantic recall, bilingual queries, and source-span auditability.

## Optimal local configuration

[recipe-spec] Use BM25/FTS5 and dense vectors as parallel candidate generators. Rerank only top candidates with a local model. Preserve per-stage scores, source spans, and query normalization details.
...
|---|---|---|
| recall_at_k_review | paste-time measurement | no value included |
| latency_by_stage | paste-time measurement | no value included |
| rerank_cost | paste-time measurement | no value included |
| duplicate_rate | paste-time measurement | no value included |
| operator_success | paste-time measurement | no value included |
...
```bash
python hybrid_search.py --query "ASR lane" --top-k 20
```

```bash
python rerank_candidates.py --candidates candidates.jsonl
```
...
## Extended operating rationale — Hybrid BM25 plus dense reranking pipeline
