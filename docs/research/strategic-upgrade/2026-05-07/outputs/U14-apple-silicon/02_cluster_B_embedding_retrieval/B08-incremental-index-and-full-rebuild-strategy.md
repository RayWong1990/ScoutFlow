---
title: "Incremental index and full rebuild strategy"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "B08"
cluster: "B_EMBEDDING_RETRIEVAL"
framework: "index coordinator"
model: "embedding/index models"
min_chip: "M1"
...
# B08 — Incremental index and full rebuild strategy
...
[recipe-spec] Daily retrieval updates should be cheap, but schema or model changes need full rebuilds.

## Optimal local configuration

[recipe-spec] Use content hashes and model-version hashes for invalidation. Incremental updates are default; full rebuilds happen after chunker, model, schema, or corruption changes. Dry-run diffs before writing.
...
|---|---|---|
| changed_chunks | paste-time measurement | no value included |
| rebuild_time | paste-time measurement | no value included |
| orphan_count | paste-time measurement | no value included |
| schema_version | paste-time measurement | no value included |
| smoke_test_pass | paste-time measurement | no value included |
...
```bash
python index_diff.py --old old.jsonl --new new.jsonl
```
...
- read-only manifest
- BM25-only while rebuilding
- copy-on-write DB rebuild
...
## Extended operating rationale — Incremental index and full rebuild strategy
