---
title: "llama.cpp Metal model matrix for local reasoning"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "C01"
cluster: "C_LLM_INFERENCE"
framework: "llama.cpp + Metal"
model: "Llama / Qwen / DeepSeek / Phi GGUF candidates"
min_chip: "M2"
...
# C01 — llama.cpp Metal model matrix for local reasoning
...
[recipe-spec] ScoutFlow needs local summarization, entity extraction, reranking support, and judge tasks.

## Optimal local configuration

[recipe-spec] Build or install llama.cpp with Metal, pin commit/build flags, test multiple model families and quantizations on the same prompt suite, and split hot-path small models from cold-path larger models.
...
| eval_time | paste-time measurement | no value included |
...
```bash
llama-cli -m model.gguf -p "extract entities" -ngl 999 -n 256
```
...
## Extended operating rationale — llama.cpp Metal model matrix for local reasoning
