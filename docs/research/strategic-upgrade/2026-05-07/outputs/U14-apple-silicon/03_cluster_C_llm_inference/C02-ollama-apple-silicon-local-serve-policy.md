---
title: "Ollama Apple Silicon local serve policy"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "C02"
cluster: "C_LLM_INFERENCE"
framework: "Ollama"
model: "local chat and embedding models"
min_chip: "M1"
...
# C02 — Ollama Apple Silicon local serve policy
...
[recipe-spec] A localhost model server can make ScoutFlow tools simpler without using cloud inference.

## Optimal local configuration

[recipe-spec] Restrict to localhost, capture model tags, limit concurrency, and group requests to avoid model thrash. Keep direct llama.cpp or MLX as reproducibility fallback.
...
| memory_resident | paste-time measurement | no value included |
| queue_latency | paste-time measurement | no value included |
| model_switch_time | paste-time measurement | no value included |
...
## Extended operating rationale — Ollama Apple Silicon local serve policy
