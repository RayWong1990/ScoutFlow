---
title: "Apple Foundation Models Swift integration watchlist"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "C04"
cluster: "C_LLM_INFERENCE"
framework: "Apple Foundation Models Swift API watchlist"
model: "system local language model if available"
min_chip: "M5"
...
# C04 — Apple Foundation Models Swift integration watchlist
...
[recipe-spec] A future local Swift model API could simplify app integration if availability and privacy are proven.

## Optimal local configuration

[recipe-spec] Define a thin adapter that accepts prompts and emits structured JSON with provenance. Do not depend on the framework until OS availability, model behavior, privacy, limits, and licensing are verified.
...
|---|---|---|
| api_available | paste-time measurement | no value included |
| latency | paste-time measurement | no value included |
| structured_output_rate | paste-time measurement | no value included |
| quality_review | paste-time measurement | no value included |
| privacy_confirmed | paste-time measurement | no value included |
...
```bash
swift run FoundationModelProbe --prompt prompts/entity_extract.txt
```
...
## Extended operating rationale — Apple Foundation Models Swift integration watchlist
