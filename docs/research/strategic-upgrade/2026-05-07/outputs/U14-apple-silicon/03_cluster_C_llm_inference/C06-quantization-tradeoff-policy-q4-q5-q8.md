---
title: "Quantization tradeoff policy Q4/Q5/Q8"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "C06"
cluster: "C_LLM_INFERENCE"
framework: "llama.cpp GGUF quantization"
model: "Q4_K_M / Q5_K_M / Q8_0 candidates"
min_chip: "M2"
...
# C06 — Quantization tradeoff policy Q4/Q5/Q8
...
[recipe-spec] Lower quantization can improve speed but damage extraction fidelity or JSON validity.

## Optimal local configuration

[recipe-spec] Benchmark a fixed prompt suite at multiple quant levels. Promote lower quant only if entity extraction, citation handling, and structured output remain acceptable. Use higher precision for high-value final passes.
...
|---|---|---|
| tokens_per_second | paste-time measurement | no value included |
| json_valid_rate | paste-time measurement | no value included |
| entity_accuracy_review | paste-time measurement | no value included |
| memory_peak | paste-time measurement | no value included |
| operator_acceptance | paste-time measurement | no value included |
...
```bash
for m in q4.gguf q5.gguf q8.gguf; do llama-bench -m "$m"; done
```

```bash
python judge_json_validity.py outputs/*.json
```
...
## Extended operating rationale — Quantization tradeoff policy Q4/Q5/Q8
