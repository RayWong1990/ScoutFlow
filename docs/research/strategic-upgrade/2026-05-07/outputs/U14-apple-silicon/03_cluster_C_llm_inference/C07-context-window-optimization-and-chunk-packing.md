---
title: "Context window optimization and chunk packing"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "C07"
cluster: "C_LLM_INFERENCE"
framework: "local LLM prompt builder"
...
# C07 — Context window optimization and chunk packing
...
[recipe-spec] Huge context windows can waste memory and degrade latency.

## Optimal local configuration

[recipe-spec] Use task-specific budgets, pack only source spans needed for the answer, trim duplicates, and prefer retrieval plus short prompts over blanket large-context runs unless long-context quality is proven.
...
|---|---|---|
| prefill_time | paste-time measurement | no value included |
| context_tokens | paste-time measurement | no value included |
| source_span_coverage | paste-time measurement | no value included |
| answer_quality_review | paste-time measurement | no value included |
...
```bash
python prompt_pack.py --budget 4096 --query query.txt --chunks chunks.jsonl
```

```bash
llama-cli -c 4096 -m model.gguf -f packed_prompt.txt
```
...
## Extended operating rationale — Context window optimization and chunk packing
