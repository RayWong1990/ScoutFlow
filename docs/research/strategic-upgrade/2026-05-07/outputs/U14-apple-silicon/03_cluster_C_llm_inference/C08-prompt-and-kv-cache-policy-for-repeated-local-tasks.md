---
title: "Prompt and KV cache policy for repeated local tasks"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "C08"
cluster: "C_LLM_INFERENCE"
framework: "llama.cpp / Ollama cache behavior"
model: "local LLM with cache support"
min_chip: "M2"
...
# C08 — Prompt and KV cache policy for repeated local tasks
...
[recipe-spec] Repeated rubrics and extraction prompts can waste prefill time.

## Optimal local configuration

[recipe-spec] Separate static rubric from dynamic content, capture whether cache reuse is supported, and invalidate after model, quant, tokenizer, prompt, or runner change. Protect caches because they may contain sensitive prompt context.
...
|---|---|---|
| cache_hit_rate | paste-time measurement | no value included |
| prefill_reduction | paste-time measurement | no value included |
| wrong_cache_incidents | paste-time measurement | no value included |
| disk_cache_size | paste-time measurement | no value included |
| model_version_match | paste-time measurement | no value included |
...
```bash
python cache_probe.py --model model.gguf --prompt rubric.txt
```

```bash
ls -lh ~/.cache/scoutflow-llm || true
```
...
## Extended operating rationale — Prompt and KV cache policy for repeated local tasks
