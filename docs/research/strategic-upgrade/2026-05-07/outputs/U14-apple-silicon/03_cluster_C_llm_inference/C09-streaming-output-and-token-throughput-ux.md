---
title: "Streaming output and token throughput UX"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "C09"
cluster: "C_LLM_INFERENCE"
framework: "local LLM streaming API"
model: "Ollama/llama.cpp compatible models"
min_chip: "M1"
...
# C09 — Streaming output and token throughput UX
...
[recipe-spec] Streaming makes local models feel usable, but structured outputs still need validation.

## Optimal local configuration

[recipe-spec] Stream text to the UI while buffering JSON until validity checks pass. Record first-token latency separately from total throughput. Provide cancellation and downgrade controls.
...
|---|---|---|
| first_token_latency | paste-time measurement | no value included |
| tokens_per_second | paste-time measurement | no value included |
| cancel_success | paste-time measurement | no value included |
| json_validity | paste-time measurement | no value included |
| operator_jank | paste-time measurement | no value included |
...
```bash
curl -N http://localhost:11434/api/generate -d @stream_request.json
```

```bash
python stream_validate_json.py
```
...
## Extended operating rationale — Streaming output and token throughput UX
