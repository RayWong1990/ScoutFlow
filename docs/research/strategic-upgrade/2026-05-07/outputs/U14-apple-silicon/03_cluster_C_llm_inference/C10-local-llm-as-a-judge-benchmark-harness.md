---
title: "Local LLM-as-a-judge benchmark harness"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "C10"
cluster: "C_LLM_INFERENCE"
framework: "local judge model + rubric"
model: "small local judge plus reference set"
min_chip: "M2"
...
# C10 — Local LLM-as-a-judge benchmark harness
...
[recipe-spec] A local judge can prioritize review but must not become truth authority.

## Optimal local configuration

[recipe-spec] Build a fixed rubric and gold mini-set. Judge output should include reasons, source spans, and confidence. Human review remains authority, especially for vault or DAM promotion.
...
|---|---|---|
| human_agreement | paste-time measurement | no value included |
| false_accept_rate | paste-time measurement | no value included |
| false_reject_rate | paste-time measurement | no value included |
| tokens_per_item | paste-time measurement | no value included |
| latency_per_item | paste-time measurement | no value included |
...
```bash
python local_judge.py --rubric rubric.md --cases cases.jsonl --out scores.jsonl
```

```bash
python compare_human_judge.py scores.jsonl human.csv
```
...
## Extended operating rationale — Local LLM-as-a-judge benchmark harness
