---
title: "Pillow Sharp OpenCV Apple Silicon image baseline"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "D01"
cluster: "D_IMAGE_VISUAL"
framework: "Pillow / Sharp / OpenCV"
model: "native image operations"
min_chip: "M1"
...
# D01 — Pillow Sharp OpenCV Apple Silicon image baseline
...
[recipe-spec] Resize, crop, normalize, and metadata extraction are prerequisites for generated image post-processing and visual-DAM intake.

## Optimal local configuration

[recipe-spec] Use Pillow for Python orchestration, Sharp/libvips for high-throughput resize, and OpenCV for vision transforms when needed. Preserve EXIF and color profile intentionally; never overwrite originals.
...
| metadata_preserved | paste-time measurement | no value included |
| quality_diff | paste-time measurement | no value included |
...
```bash
python image_batch_baseline.py --manifest images.jsonl
```

```bash
magick identify -verbose image.jpg || true
```
...
- single-thread Pillow
- defer huge images
- skip nonessential transform
- process copies only
...
## Extended operating rationale — Pillow Sharp OpenCV Apple Silicon image baseline
