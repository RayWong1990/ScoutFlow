---
title: "Core ML conversion and inference recipe"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "D03"
cluster: "D_IMAGE_VISUAL"
framework: "Core ML / coremltools"
model: "converted vision or classifier model"
min_chip: "M2"
...
# D03 — Core ML conversion and inference recipe
...
[recipe-spec] Core ML can package local classifiers or taggers without a Python server if conversion is stable.

## Optimal local configuration

[recipe-spec] Convert only models with license clearance and stable preprocessing. Keep original model, conversion script, mlpackage hash, compute unit choice, and test image outputs.
...
|---|---|---|
| conversion_success | paste-time measurement | no value included |
| latency_per_image | paste-time measurement | no value included |
| compute_unit_used | paste-time measurement | no value included |
| memory_peak | paste-time measurement | no value included |
| classification_agreement | paste-time measurement | no value included |
...
```bash
python convert_to_coreml.py --model model.pt --out model.mlpackage
```

```bash
swift run CoreMLProbe --model model.mlpackage --image sample.jpg
```
...
## Extended operating rationale — Core ML conversion and inference recipe
