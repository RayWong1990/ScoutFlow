---
title: "GPT-Image-2 post-processing crop dedup thumbnail pipeline"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "D04"
cluster: "D_IMAGE_VISUAL"
framework: "ImageIO / Sharp / pHash / manifest"
...
# D04 — GPT-Image-2 post-processing crop dedup thumbnail pipeline
...
[recipe-spec] Generated image sets need local crop, dedup, thumbnail, and metadata ledger before DAM handoff.

## Optimal local configuration

[recipe-spec] Treat generated outputs as raw inputs to a deterministic local pipeline. Emit crop boxes, thumbnail path, pHash, duplicate cluster id, dimensions, color profile, prompt id, and operator notes.
...
|---|---|---|
| images_per_second | paste-time measurement | no value included |
| duplicate_clusters | paste-time measurement | no value included |
| thumbnail_latency | paste-time measurement | no value included |
...
```bash
python image2_postprocess.py --in generated/ --out candidate_dam/
```

```bash
python phash_cluster.py --manifest images.jsonl --threshold REVIEW
```
...
- skip crop and thumbnail only
- batch overnight
- lower proxy resolution
...
## Extended operating rationale — GPT-Image-2 post-processing crop dedup thumbnail pipeline
