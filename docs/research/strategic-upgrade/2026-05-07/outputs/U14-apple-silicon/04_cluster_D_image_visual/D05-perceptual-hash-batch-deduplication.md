---
title: "Perceptual hash batch deduplication"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "D05"
cluster: "D_IMAGE_VISUAL"
framework: "pHash / imagehash / OpenCV"
model: "visual duplicate detector"
min_chip: "M1"
...
# D05 — Perceptual hash batch deduplication
...
[recipe-spec] Near-duplicate screenshots and generated variants should be clustered before embedding and review.

## Optimal local configuration

[recipe-spec] Compute pHash on normalized proxies, cluster by reviewable threshold, and keep representative selection reversible. Store every duplicate edge and threshold in JSONL.
...
|---|---|---|
| hashes_per_second | paste-time measurement | no value included |
| cluster_precision_review | paste-time measurement | no value included |
| false_merge_count | paste-time measurement | no value included |
| false_split_count | paste-time measurement | no value included |
...
```bash
python phash_manifest.py --images images.jsonl --out hashes.jsonl
```

```bash
python cluster_hashes.py --threshold REVIEW
```
...
- skip dedup for high-value review
...
## Extended operating rationale — Perceptual hash batch deduplication
