---
title: "Stable Diffusion local MLX candidate"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "D06"
cluster: "D_IMAGE_VISUAL"
framework: "MLX stable diffusion / local image generation"
model: "MLX-compatible diffusion model"
min_chip: "M3"
...
# D06 — Stable Diffusion local MLX candidate
...
[recipe-spec] Local image generation can protect private drafts if model license and memory behavior are acceptable.

## Optimal local configuration

[recipe-spec] Keep local diffusion isolated and optional. Capture prompt, seed, dimensions, model hash, package version, memory, thermal state, and output hash. Do not make it a visual-DAM dependency.
...
|---|---|---|
| seconds_per_image | paste-time measurement | no value included |
| memory_peak | paste-time measurement | no value included |
| thermal_state | paste-time measurement | no value included |
| output_hash_recorded | paste-time measurement | no value included |
...
```bash
python -m mlx_stable_diffusion.generate --prompt "private draft" --n_images 1
```
...
- disable local generation
- smaller resolution
- deferred batch
- cloud API not approved for private prompts
...
## Extended operating rationale — Stable Diffusion local MLX candidate
