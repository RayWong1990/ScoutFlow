---
title: "Background removal with MPS candidate"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "D07"
cluster: "D_IMAGE_VISUAL"
framework: "rembg / PyTorch MPS / Core ML alternative"
model: "segmentation/background removal model"
min_chip: "M2"
...
# D07 — Background removal with MPS candidate
...
[recipe-spec] Background removal can help presentation assets but can also damage boundaries and provenance.

## Optimal local configuration

[recipe-spec] Run on copies only. Verify MPS or Core ML backend at runtime. Store mask, alpha output, source hash, model hash, and review flags before using outputs downstream.
...
| mask_quality_review | paste-time measurement | no value included |
| mps_used | paste-time measurement | no value included |
...
```bash
python bg_remove_candidate.py --manifest images.jsonl --out masks/
```
...
## Extended operating rationale — Background removal with MPS candidate
