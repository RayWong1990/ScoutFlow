---
title: "8K image handling and memory pressure policy"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "D08"
cluster: "D_IMAGE_VISUAL"
framework: "ImageIO / libvips / tiled processing"
model: "large image pipeline"
min_chip: "M2"
...
# D08 — 8K image handling and memory pressure policy
...
[recipe-spec] Huge screenshots, scans, and generated images can trigger swap when loaded eagerly.

## Optimal local configuration

[recipe-spec] Use streaming or tiled decode when possible, cap workers, generate proxies for search, and attach derived metadata. Avoid loading many full-resolution arrays in Python at once.
...
|---|---|---|
| max_rss | paste-time measurement | no value included |
| swap_delta | paste-time measurement | no value included |
| images_completed | paste-time measurement | no value included |
| proxy_quality | paste-time measurement | no value included |
| failure_recovery | paste-time measurement | no value included |
...
```bash
python large_image_probe.py --manifest huge.jsonl --max-workers 1
```
...
## Extended operating rationale — 8K image handling and memory pressure policy
