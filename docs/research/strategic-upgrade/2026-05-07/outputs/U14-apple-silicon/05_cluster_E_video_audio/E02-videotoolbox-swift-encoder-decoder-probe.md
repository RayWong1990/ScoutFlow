---
title: "VideoToolbox Swift encoder decoder probe"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "E02"
cluster: "E_VIDEO_AUDIO"
framework: "VideoToolbox / AVFoundation Swift"
model: "native media pipeline"
min_chip: "M2"
...
# E02 — VideoToolbox Swift encoder decoder probe
...
[recipe-spec] A Swift-native probe can improve packaging and structured metadata extraction.

## Optimal local configuration

[recipe-spec] Use Swift only when it improves integration or metadata reliability. Emit JSON for codec, duration, dimensions, frame rate, color, audio tracks, and hardware path.
...
|---|---|---|
| probe_latency | paste-time measurement | no value included |
| decode_success | paste-time measurement | no value included |
| metadata_completeness | paste-time measurement | no value included |
| memory_peak | paste-time measurement | no value included |
| failure_count | paste-time measurement | no value included |
...
```bash
swift run VideoToolboxProbe --input in.mov --out probe.json
```
...
## Extended operating rationale — VideoToolbox Swift encoder decoder probe
