---
title: "Audio extraction 44.1k to 16k mono for ASR"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "E03"
cluster: "E_VIDEO_AUDIO"
framework: "FFmpeg / afconvert"
model: "ASR-ready WAV"
min_chip: "M1"
...
# E03 — Audio extraction 44.1k to 16k mono for ASR
...
[recipe-spec] ASR benchmark and production candidates need stable WAV inputs from video or audio sources.

## Optimal local configuration

[recipe-spec] Normalize to model-required WAV, usually 16 kHz mono PCM for Whisper-style ASR. Preserve source hash and original sample-rate metadata. Avoid destructive filters unless measured.
...
|---|---|---|
| extract_speed | paste-time measurement | no value included |
| duration_match | paste-time measurement | no value included |
| sample_rate_confirmed | paste-time measurement | no value included |
| asr_quality_delta | paste-time measurement | no value included |
| file_size | paste-time measurement | no value included |
...
```bash
ffmpeg -i input.mp4 -vn -ar 16000 -ac 1 -c:a pcm_s16le input.asr.wav
```

```bash
ffprobe -show_streams input.asr.wav
```
...
## Extended operating rationale — Audio extraction 44.1k to 16k mono for ASR
