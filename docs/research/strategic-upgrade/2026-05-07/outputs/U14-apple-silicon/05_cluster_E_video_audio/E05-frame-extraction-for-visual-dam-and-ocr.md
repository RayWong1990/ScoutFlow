---
title: "Frame extraction for visual-DAM and OCR"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "E05"
cluster: "E_VIDEO_AUDIO"
framework: "FFmpeg / VideoToolbox decode / ImageIO"
model: "video frames"
min_chip: "M1"
...
# E05 — Frame extraction for visual-DAM and OCR
...
[recipe-spec] Representative frames support visual search, OCR, and thumbnail review.

## Optimal local configuration

[recipe-spec] Use scene-change or interval extraction based on workload. Store frame timestamp, source hash, frame hash, command, and thumbnail path. Avoid full dumps unless needed.
...
|---|---|---|
| frames_per_second | paste-time measurement | no value included |
| frame_count | paste-time measurement | no value included |
| ocr_hit_rate | paste-time measurement | no value included |
| storage_growth | paste-time measurement | no value included |
| duplicate_rate | paste-time measurement | no value included |
...
```bash
ffmpeg -i in.mp4 -vf fps=1/5 frames/%06d.jpg
```

```bash
python frame_manifest.py frames/ --source in.mp4
```
...
## Extended operating rationale — Frame extraction for visual-DAM and OCR
