---
title: "Cluster E — Video / Audio Pipeline Index"
status: "candidate / not-authority"
authority: "not-authority"
cluster: "E_VIDEO_AUDIO"
created_at: "2026-05-07"
---

# Cluster E — Video / Audio Pipeline Index
...
[prompt-provided] This index covers FFmpeg, VideoToolbox, audio extraction, subtitles, frames, containers. It connects the cluster recipes and provides a dispatch surface for a human operator. The design rule is simple: choose the smallest local path that preserves raw evidence, then collect paste-time metrics before expanding model size, accelerator use, or concurrency.

## Recipes

| Recipe | File | Framework | Model | Min chip | Memory |
|---|---|---|---|---|---:|
| E01 | [FFmpeg VideoToolbox hardware acceleration baseline](E01-ffmpeg-videotoolbox-hardware-acceleration-baseline.md) | FFmpeg + VideoToolbox | h264/hevc videotoolbox codecs | M1 | 8 GB |
| E02 | [VideoToolbox Swift encoder decoder probe](E02-videotoolbox-swift-encoder-decoder-probe.md) | VideoToolbox / AVFoundation Swift | native media pipeline | M2 | 16 GB |
| E03 | [Audio extraction 44.1k to 16k mono for ASR](E03-audio-extraction-44-1k-to-16k-mono-for-asr.md) | FFmpeg / afconvert | ASR-ready WAV | M1 | 8 GB |
| E04 | [Subtitle render and transcript sidecar generation](E04-subtitle-render-and-transcript-sidecar-generation.md) | FFmpeg / subtitle tools | SRT/VTT/ASS sidecars | M1 | 8 GB |
| E05 | [Frame extraction for visual-DAM and OCR](E05-frame-extraction-for-visual-dam-and-ocr.md) | FFmpeg / VideoToolbox decode / ImageIO | video frames | M1 | 8 GB |
| E06 | [Container conversion and stream-copy policy](E06-container-conversion-and-stream-copy-policy.md) | FFmpeg / AVFoundation | media containers | M1 | 8 GB |
...
## Extended operating rationale — Cluster E — Video / Audio Pipeline

**Rationale 1.** Media preprocessing is a correctness gate for ASR and visual indexing. Stream copy should be preferred when conversion is only for container compatibility. Transcode should be used only when downstream tools require it or when a proxy policy explicitly calls for it.

**Rationale 2.** VideoToolbox can be valuable, but hardware acceleration must be verified by command output and quality review. A codec path that is fast but damages color, timestamps, subtitles, or audio synchronization should not be promoted.
...
**Rationale 15.** Media preprocessing is a correctness gate for ASR and visual indexing. Stream copy should be preferred when conversion is only for container compatibility. Transcode should be used only when downstream tools require it or when a proxy policy explicitly calls for it.

**Rationale 16.** VideoToolbox can be valuable, but hardware acceleration must be verified by command output and quality review. A codec path that is fast but damages color, timestamps, subtitles, or audio synchronization should not be promoted.
