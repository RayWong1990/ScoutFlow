---
title: "LaunchAgent spec for local candidate workers"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "F01"
cluster: "F_SYSTEM_INTEGRATION"
framework: "launchd / LaunchAgent spec only"
model: "ScoutFlow local worker"
min_chip: "M1"
...
# F01 — LaunchAgent spec for local candidate workers
...
[recipe-spec] A future worker may need reliable startup, but this pack must not install it.

## Optimal local configuration

[recipe-spec] Write a plist spec with explicit working directory, log paths, environment, throttle interval, and disabled-by-default state. A human must review before any `launchctl` load.
...
|---|---|---|
| plist_validity | paste-time measurement | no value included |
| start_success | paste-time measurement | no value included |
| crash_loop_count | paste-time measurement | no value included |
| log_completeness | paste-time measurement | no value included |
| disable_success | paste-time measurement | no value included |
...
```bash
plutil -lint com.scoutflow.worker.plist
```

```bash
launchctl print gui/$UID/com.scoutflow.worker
```
...
## Extended operating rationale — LaunchAgent spec for local candidate workers
