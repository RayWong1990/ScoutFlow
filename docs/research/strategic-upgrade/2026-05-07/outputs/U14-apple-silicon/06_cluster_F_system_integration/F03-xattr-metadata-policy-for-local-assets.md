---
title: "xattr metadata policy for local assets"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "F03"
cluster: "F_SYSTEM_INTEGRATION"
framework: "xattr / Finder metadata"
model: "asset metadata tags"
min_chip: "M1"
...
# F03 — xattr metadata policy for local assets
...
[recipe-spec] Extended attributes can hold lightweight pointers but are fragile as authority.

## Optimal local configuration

[recipe-spec] Use xattr sparingly for non-sensitive manifest ids or candidate status. Do not store transcripts, private summaries, secrets, or large metadata in xattr.
...
|---|---|---|
| write_success | paste-time measurement | no value included |
| read_success | paste-time measurement | no value included |
| metadata_size | paste-time measurement | no value included |
| backup_preservation | paste-time measurement | no value included |
...
```bash
xattr -w com.scoutflow.manifest_id abc123 file.jpg
```
...
## Extended operating rationale — xattr metadata policy for local assets
