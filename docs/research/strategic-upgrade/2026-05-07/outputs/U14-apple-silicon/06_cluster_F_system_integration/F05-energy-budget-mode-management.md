---
title: "Energy budget mode management"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "F05"
cluster: "F_SYSTEM_INTEGRATION"
framework: "macOS power observation / scheduler policy"
model: "safe balanced max modes"
min_chip: "M1"
...
# F05 — Energy budget mode management
...
[recipe-spec] The scheduler needs modes for battery-safe collection, daily balanced work, and plugged-in batch runs.

## Optimal local configuration

[recipe-spec] Create ScoutFlow configuration modes rather than global macOS changes. Max mode admits heavy jobs only when plugged in and thermals are acceptable. Balanced mode protects UI.
...
|---|---|---|
| battery_drain | paste-time measurement | no value included |
| job_completion_time | paste-time measurement | no value included |
| thermal_state | paste-time measurement | no value included |
| ui_responsiveness | paste-time measurement | no value included |
| queue_backlog | paste-time measurement | no value included |
...
```bash
python scheduler.py --mode balanced
```
...
## Extended operating rationale — Energy budget mode management
