---
title: "Memory pressure detection and queue admission"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "F07"
cluster: "F_SYSTEM_INTEGRATION"
framework: "memory_pressure / vm_stat / scheduler"
model: "unified memory guard"
min_chip: "M1"
...
# F07 — Memory pressure detection and queue admission
...
[recipe-spec] Unified memory pressure should control whether heavy local AI jobs can start.

## Optimal local configuration

[recipe-spec] Read memory pressure before launching jobs. Yellow pressure starts downgrade; red pressure blocks new heavy work. Serialize model loads and log decisions.
...
|---|---|---|
| memory_pressure_state | paste-time measurement | no value included |
| swap_delta | paste-time measurement | no value included |
| jobs_admitted | paste-time measurement | no value included |
...
```bash
python admission_control.py --dry-run
```
...
## Extended operating rationale — Memory pressure detection and queue admission
