---
title: "Cluster F — System Integration Index"
...
# Cluster F — System Integration Index
...
[prompt-provided] This index covers launchd specs, Spotlight watchlist, xattr, powermetrics, thermal and memory guards. It connects the cluster recipes and provides a dispatch surface for a human operator. The design rule is simple: choose the smallest local path that preserves raw evidence, then collect paste-time metrics before expanding model size, accelerator use, or concurrency.

## Recipes

| Recipe | File | Framework | Model | Min chip | Memory |
|---|---|---|---|---|---:|
| F01 | [LaunchAgent spec for local candidate workers](F01-launchagent-spec-for-local-candidate-workers.md) | launchd / LaunchAgent spec only | ScoutFlow local worker | M1 | 8 GB |
| F02 | [Spotlight and mdimporter integration watchlist](F02-spotlight-and-mdimporter-integration-watchlist.md) | Spotlight / mdimporter | metadata importer | M1 | 8 GB |
| F03 | [xattr metadata policy for local assets](F03-xattr-metadata-policy-for-local-assets.md) | xattr / Finder metadata | asset metadata tags | M1 | 8 GB |
| F04 | [Activity Monitor and powermetrics capture protocol](F04-activity-monitor-and-powermetrics-capture-protocol.md) | Activity Monitor / powermetrics | system telemetry | M1 | 8 GB |
| F05 | [Energy budget mode management](F05-energy-budget-mode-management.md) | macOS power observation / scheduler policy | safe balanced max modes | M1 | 8 GB |
| F06 | [Thermal throttle detection and downgrade](F06-thermal-throttle-detection-and-downgrade.md) | powermetrics / scheduler guard | thermal state policy | M2 | 16 GB |
| F07 | [Memory pressure detection and queue admission](F07-memory-pressure-detection-and-queue-admission.md) | memory_pressure / vm_stat / scheduler | unified memory guard | M1 | 8 GB |
| F08 | [Background task assertion and sleep policy](F08-background-task-assertion-and-sleep-policy.md) | caffeinate / NSProcessInfo watchlist | long-running local jobs | M1 | 8 GB |
...
## Extended operating rationale — Cluster F — System Integration
...
**Rationale 15.** System integration recipes must be conservative. launchd specs, xattr, Spotlight, background assertions, and telemetry can improve operator experience, but they can also create invisible state. Candidate files should remain dry-run until a human reviews installation, scope, logs, and rollback.
