---
title: "Unified Memory budget for mixed local AI workloads"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "G01"
cluster: "G_RESOURCE_MANAGEMENT"
framework: "scheduler / memory budget"
model: "ASR + LLM + embedding + image models"
min_chip: "M2"
...
# G01 — Unified Memory budget for mixed local AI workloads
...
[recipe-spec] All local AI jobs share unified memory with the OS, browser, and media buffers.

## Optimal local configuration

[recipe-spec] Create per-job budgets from measured peaks plus safety margin. Reserve memory for OS/UI, run one large model at a time until evidence supports concurrency, and store budgets by chip/memory tier.
...
|---|---|---|
| peak_rss | paste-time measurement | no value included |
| swap_delta | paste-time measurement | no value included |
| jobs_completed | paste-time measurement | no value included |
| ui_jank | paste-time measurement | no value included |
| budget_violations | paste-time measurement | no value included |
...
```bash
python memory_budget_probe.py --job llama
```
...
## Extended operating rationale — Unified Memory budget for mixed local AI workloads
