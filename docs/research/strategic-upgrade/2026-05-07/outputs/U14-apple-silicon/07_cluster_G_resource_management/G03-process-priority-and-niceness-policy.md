---
title: "Process priority and niceness policy"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "G03"
cluster: "G_RESOURCE_MANAGEMENT"
framework: "nice / renice / launchd hints"
model: "local workers"
min_chip: "M1"
...
# G03 — Process priority and niceness policy
...
[recipe-spec] Cold-path jobs should not make the laptop feel broken.

## Optimal local configuration

[recipe-spec] Run cold-path jobs at lower priority and interactive work at normal priority. Avoid priority hacks that starve system services. Log priority with benchmark results.
...
|---|---|---|
| job_time | paste-time measurement | no value included |
| ui_responsiveness | paste-time measurement | no value included |
| cpu_share | paste-time measurement | no value included |
| queue_latency | paste-time measurement | no value included |
| interruptions | paste-time measurement | no value included |
...
```bash
nice -n 10 python cold_path_batch.py
```

```bash
ps -o pid,nice,command -p $PID
```
...
## Extended operating rationale — Process priority and niceness policy
