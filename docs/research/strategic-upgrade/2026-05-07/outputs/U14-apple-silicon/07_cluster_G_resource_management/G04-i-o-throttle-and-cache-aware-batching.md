---
title: "I/O throttle and cache-aware batching"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "G04"
cluster: "G_RESOURCE_MANAGEMENT"
framework: "filesystem scheduler policy"
model: "APFS local SSD workload"
min_chip: "M1"
...
# G04 — I/O throttle and cache-aware batching
...
[recipe-spec] Large media reads and derived artifact writes can stall local work.

## Optimal local configuration

[recipe-spec] Batch by source directory, avoid re-decoding the same media, write temp files then rename on success, and track cache hits. Keep large transcodes out of cloud sync folders unless approved.
...
|---|---|---|
| read_rate | paste-time measurement | no value included |
| write_rate | paste-time measurement | no value included |
| cache_hit_rate | paste-time measurement | no value included |
| failed_writes | paste-time measurement | no value included |
| job_latency | paste-time measurement | no value included |
...
```bash
python io_batch_probe.py --manifest media.jsonl
```
...
- smaller batches
- read-only dry run
- external SSD staging after review
- defer transcodes
...
## Extended operating rationale — I/O throttle and cache-aware batching
