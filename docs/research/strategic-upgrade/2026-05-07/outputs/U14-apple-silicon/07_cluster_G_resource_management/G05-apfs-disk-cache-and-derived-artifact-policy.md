---
title: "APFS disk cache and derived artifact policy"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "G05"
cluster: "G_RESOURCE_MANAGEMENT"
framework: "APFS / cache manager"
...
# G05 — APFS disk cache and derived artifact policy
...
[recipe-spec] Derived transcripts, thumbnails, embeddings, frames, masks, and models need rebuildable storage policy.

## Optimal local configuration

[recipe-spec] Use hash-addressed cache paths, TTL for low-value derivatives, keep rules for reviewed artifacts, and purge reports. Never put secrets in paths.
...
|---|---|---|
| cache_size | paste-time measurement | no value included |
| hit_rate | paste-time measurement | no value included |
| orphan_count | paste-time measurement | no value included |
| rebuild_success | paste-time measurement | no value included |
| purge_success | paste-time measurement | no value included |
...
```bash
python cache_report.py --root .scoutflow-cache
```

```bash
du -sh .scoutflow-cache
```
...
## Extended operating rationale — APFS disk cache and derived artifact policy
