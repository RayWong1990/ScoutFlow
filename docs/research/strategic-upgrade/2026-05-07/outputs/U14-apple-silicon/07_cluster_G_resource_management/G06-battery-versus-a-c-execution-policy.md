---
title: "Battery versus A/C execution policy"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "G06"
cluster: "G_RESOURCE_MANAGEMENT"
framework: "power-source scheduler"
model: "job mode matrix"
min_chip: "M1"
...
# G06 — Battery versus A/C execution policy
...
[recipe-spec] Some local AI jobs are acceptable on battery; others should wait for plugged-in operation.

## Optimal local configuration

[recipe-spec] On battery, permit lightweight indexing and review. Require A/C for long ASR batches, large LLM, diffusion, video transcodes, and full rebuilds unless the operator overrides intentionally.
...
|---|---|---|
| battery_delta | paste-time measurement | no value included |
| jobs_deferred | paste-time measurement | no value included |
| completion_time | paste-time measurement | no value included |
| thermal_state | paste-time measurement | no value included |
| override_count | paste-time measurement | no value included |
...
```bash
python scheduler.py --power-policy auto
```
...
## Extended operating rationale — Battery versus A/C execution policy
