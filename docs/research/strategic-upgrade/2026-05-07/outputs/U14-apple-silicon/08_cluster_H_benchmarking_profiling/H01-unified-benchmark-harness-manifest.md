---
title: "Unified benchmark harness manifest"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "H01"
cluster: "H_BENCHMARKING_PROFILING"
framework: "Python/CLI benchmark harness"
model: "all local workloads"
min_chip: "M1"
...
# H01 — Unified benchmark harness manifest
...
[recipe-spec] A single evidence format makes ASR, embedding, LLM, image, video, and telemetry comparable.

## Optimal local configuration

[recipe-spec] Use JSONL manifests with input hash, command, model hash, environment, start/end time, metrics, quality notes, and fallback flags. Run dry mode before writing outputs.
...
| run_reproducibility | paste-time measurement | no value included |
| metric_completeness | paste-time measurement | no value included |
| failure_capture | paste-time measurement | no value included |
| review_status | paste-time measurement | no value included |
...
```bash
python benchmark_harness.py --manifest jobs.jsonl --dry-run
```

```bash
python validate_bench_jsonl.py results.jsonl
```
...
## Extended operating rationale — Unified benchmark harness manifest
