---
title: "LLM benchmark prompt suite"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "H04"
cluster: "H_BENCHMARKING_PROFILING"
framework: "local LLM benchmark harness"
model: "llama.cpp / Ollama / MLX candidates"
min_chip: "M1"
...
# H04 — LLM benchmark prompt suite
...
[recipe-spec] Local model choices should be compared on ScoutFlow tasks, not only chat impressions.

## Optimal local configuration

[recipe-spec] Use prompts for entity extraction, transcript summary, visual caption normalization, runbook answer, JSON emission, and self-critique. Score with human rubric; use local judge only as secondary signal.
...
| human_score | paste-time measurement | no value included |
...
```bash
python bench_llm.py --suite scoutflow_prompts.jsonl --runner ollama
```

```bash
python score_llm_outputs.py outputs/
```
...
## Extended operating rationale — LLM benchmark prompt suite
