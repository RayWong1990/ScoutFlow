---
title: "Image pipeline benchmark manifest"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "H05"
cluster: "H_BENCHMARKING_PROFILING"
framework: "image benchmark harness"
model: "crop / dedup / thumbnail / OCR"
min_chip: "M1"
...
# H05 — Image pipeline benchmark manifest
...
[recipe-spec] Image throughput is useful only when metadata and provenance survive.

## Optimal local configuration

[recipe-spec] Use a manifest with image class, dimensions, format, hash, and expected operations. Score processing time, memory, metadata preservation, duplicate precision, and review flags.
...
| duplicate_precision | paste-time measurement | no value included |
...
```bash
python bench_image_pipeline.py --manifest image_cases.jsonl
```

```bash
python audit_image_outputs.py out/
```
...
## Extended operating rationale — Image pipeline benchmark manifest
