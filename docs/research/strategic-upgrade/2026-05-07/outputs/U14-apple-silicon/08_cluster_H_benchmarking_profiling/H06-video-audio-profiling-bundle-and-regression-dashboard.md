---
title: "Video audio profiling bundle and regression dashboard"
status: "candidate / not-authority"
authority: "not-authority"
recipe_id: "H06"
cluster: "H_BENCHMARKING_PROFILING"
framework: "FFmpeg benchmark + profile bundle + dashboard"
model: "VideoToolbox / FFmpeg / system telemetry"
min_chip: "M1"
...
# H06 — Video audio profiling bundle and regression dashboard
...
[recipe-spec] Media and system regressions need historical comparison with provenance.

## Optimal local configuration

[recipe-spec] Bundle ffprobe, command, hardware snapshot, package versions, telemetry, result metrics, and quality notes. Render a Markdown or SQLite dashboard comparing against the last accepted local baseline.
...
|---|---|---|
| speed_factor | paste-time measurement | no value included |
| bundle_completeness | paste-time measurement | no value included |
| redaction_success | paste-time measurement | no value included |
| regression_flag | paste-time measurement | no value included |
| baseline_age | paste-time measurement | no value included |
...
```bash
python bench_video.py --manifest video_cases.jsonl
```

```bash
python collect_profile_bundle.py --run-id RUN123
```

```bash
python render_perf_dashboard.py --results results.db --out DASHBOARD.md
```
...
## Extended operating rationale — Video audio profiling bundle and regression dashboard
