---
decision_id: "PR-191-t-p1a-154-overflow-candidate-registry-for-db-vnext-and-blocked-runtime-l"
status: "candidate / decision-atlas / not-authority"
authority: "not-authority"
pr_number: 191
pr_exists_verified: true
merged: true
pr_url: "https://github.com/RayWong1990/ScoutFlow/pull/191"
pr_title: "T-P1A-154: Overflow candidate registry for DB vNext and blocked runtime lanes"
merged_at: "2026-05-05T18:11:57Z"
cluster: "C02 Authority Sync / Wave Closeout"
decision_kind: "authority-sync"
...
linked_patterns: ["candidate_not_authority_frontmatter", "preview_only_before_write", "boundary_preservation"]
linked_anti_patterns: []
canonical_decision_log_unchanged: true
---

# PR #191 — T-P1A-154: Overflow candidate registry for DB vNext and blocked runtime lanes

## Evidence snapshot

| Field | Value |
|---|---|
| PR | [#191](https://github.com/RayWong1990/ScoutFlow/pull/191) |
| Merged at | `2026-05-05T18:11:57Z` |
| Cluster | `C02 Authority Sync / Wave Closeout` |
| Decision kind | `authority-sync` |
...
Docs-only overflow candidate registry for DB vNext and blocked runtime lanes; no authority writeback or runtime approval.
...
The chosen decision was to merge **T-P1A-154: Overflow candidate registry for DB vNext and blocked runtime lanes** as a authority-sync item inside **C02 Authority Sync / Wave Closeout**. The operational payload, as observed from the PR metadata/body summary, was: Docs-only overflow candidate registry for DB vNext and blocked runtime lanes; no authority writeback or runtime approval.

## Rationale

The rationale is not simply “ship the file”. In ScoutFlow terms, the PR turns a loose memory item into a reviewable artifact while maintaining candidate/not-authority separation. This is why the atlas marks the impact as `cross-phase` and attribution as `introduced`. When the PR introduced a new candidate, the value was future-searchability and controlled continuation. When it exposed a gap, the value was traceability: a later auditor can see which claim was narrowed, which evidence was downgraded, and which boundary remained intact. The recurring design choice is to prefer an explicit partial truth over an implicit complete-sounding claim.

## Outcome and downstream signal

The immediate outcome is a merged checkpoint in the PR ledger at `2026-05-05T18:11:57Z`. Downstream, this card should be read together with linked patterns `candidate_not_authority_frontmatter, preview_only_before_write, boundary_preservation` and with the amend chain `{'amends': [], 'amended_by': [], 'related_prs': []}`. No U11 anti-pattern ID is attached because the U11 registry was not available in this build; inventing AP IDs would make the atlas less useful. The package therefore records a cross-link gap rather than pretending the registry was loaded.
...
- Pattern files: `02_patterns/PATTERN-candidate_not_authority_frontmatter.md`, `02_patterns/PATTERN-preview_only_before_write.md`, `02_patterns/PATTERN-boundary_preservation.md`
