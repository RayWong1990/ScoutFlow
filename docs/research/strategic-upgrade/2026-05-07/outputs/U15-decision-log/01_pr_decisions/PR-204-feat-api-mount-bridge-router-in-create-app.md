---
decision_id: "PR-204-feat-api-mount-bridge-router-in-create-app"
status: "candidate / decision-atlas / not-authority"
authority: "not-authority"
pr_number: 204
pr_exists_verified: true
merged: true
pr_url: "https://github.com/RayWong1990/ScoutFlow/pull/204"
pr_title: "feat(api): mount bridge router in create_app"
merged_at: "2026-05-06T09:12:41Z"
...
# PR #204 — feat(api): mount bridge router in create_app

## Evidence snapshot

| Field | Value |
|---|---|
| PR | [#204](https://github.com/RayWong1990/ScoutFlow/pull/204) |
| Merged at | `2026-05-06T09:12:41Z` |
| Cluster | `C04 PF-LP Run-1` |
...
Mounted bridge router from create_app, exposed write_enabled=false on dry-run commit schema/response, aligned route/openapi/preview/commit contract tests.
...
The chosen decision was to merge **feat(api): mount bridge router in create_app** as a contract item inside **C04 PF-LP Run-1**. The operational payload, as observed from the PR metadata/body summary, was: Mounted bridge router from create_app, exposed write_enabled=false on dry-run commit schema/response, aligned route/openapi/preview/commit contract tests.
...
The immediate outcome is a merged checkpoint in the PR ledger at `2026-05-06T09:12:41Z`. Downstream, this card should be read together with linked patterns `preview_only_before_write` and with the amend chain `{'amended_by': [231], 'related_prs': [231], 'amends': []}`. No U11 anti-pattern ID is attached because the U11 registry was not available in this build; inventing AP IDs would make the atlas less useful. The package therefore records a cross-link gap rather than pretending the registry was loaded.
