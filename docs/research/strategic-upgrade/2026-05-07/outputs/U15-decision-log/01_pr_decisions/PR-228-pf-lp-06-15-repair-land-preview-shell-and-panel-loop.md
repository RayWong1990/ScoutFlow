---
decision_id: "PR-228-pf-lp-06-15-repair-land-preview-shell-and-panel-loop"
status: "candidate / decision-atlas / not-authority"
authority: "not-authority"
pr_number: 228
pr_exists_verified: true
merged: true
pr_url: "https://github.com/RayWong1990/ScoutFlow/pull/228"
pr_title: "PF-LP-06-15 repair: land preview shell and panel loop"
merged_at: "2026-05-06T10:09:19Z"
cluster: "C05 PF-LP Run-2 / Window-2 / PF-GLOBAL / PF-C3"
decision_kind: "contract"
impact_radius: "multi-cluster"
...
amend_chain: {"amended_by": [239], "related_prs": [232, 233, 234, 239], "amends": []}
linked_patterns: ["preview_only_before_write"]
linked_anti_patterns: []
canonical_decision_log_unchanged: true
---

# PR #228 — PF-LP-06-15 repair: land preview shell and panel loop

## Evidence snapshot

| Field | Value |
|---|---|
| PR | [#228](https://github.com/RayWong1990/ScoutFlow/pull/228) |
| Merged at | `2026-05-06T10:09:19Z` |
| Cluster | `C05 PF-LP Run-2 / Window-2 / PF-GLOBAL / PF-C3` |
| Decision kind | `contract` |
| Impact radius | `multi-cluster` |
...
Landed preview shell bridge from URL bar capture creation into preview loading/reset, stateful preview panel behavior, copy/download actions, fail-loud blocked states, and frontend contract tests.
...
The chosen decision was to merge **PF-LP-06-15 repair: land preview shell and panel loop** as a contract item inside **C05 PF-LP Run-2 / Window-2 / PF-GLOBAL / PF-C3**. The operational payload, as observed from the PR metadata/body summary, was: Landed preview shell bridge from URL bar capture creation into preview loading/reset, stateful preview panel behavior, copy/download actions, fail-loud blocked states, and frontend contract tests.

## Rationale

The rationale is not simply “ship the file”. In ScoutFlow terms, the PR turns a loose memory item into a reviewable artifact while maintaining candidate/not-authority separation. This is why the atlas marks the impact as `multi-cluster` and attribution as `both`. When the PR introduced a new candidate, the value was future-searchability and controlled continuation. When it exposed a gap, the value was traceability: a later auditor can see which claim was narrowed, which evidence was downgraded, and which boundary remained intact. The recurring design choice is to prefer an explicit partial truth over an implicit complete-sounding claim.

## Outcome and downstream signal

The immediate outcome is a merged checkpoint in the PR ledger at `2026-05-06T10:09:19Z`. Downstream, this card should be read together with linked patterns `preview_only_before_write` and with the amend chain `{'amended_by': [239], 'related_prs': [232, 233, 234, 239], 'amends': []}`. No U11 anti-pattern ID is attached because the U11 registry was not available in this build; inventing AP IDs would make the atlas less useful. The package therefore records a cross-link gap rather than pretending the registry was loaded.
...
- Pattern files: `02_patterns/PATTERN-preview_only_before_write.md`
- Amend chain: `{"amended_by": [239], "related_prs": [232, 233, 234, 239], "amends": []}`
