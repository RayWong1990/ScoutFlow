---
decision_id: "PR-230-pf-lp-12-add-localhost-preview-dev-runbook"
status: "candidate / decision-atlas / not-authority"
authority: "not-authority"
pr_number: 230
pr_exists_verified: true
merged: true
pr_url: "https://github.com/RayWong1990/ScoutFlow/pull/230"
pr_title: "PF-LP-12: add localhost preview dev runbook"
merged_at: "2026-05-06T10:11:39Z"
cluster: "C05 PF-LP Run-2 / Window-2 / PF-GLOBAL / PF-C3"
decision_kind: "boundary"
...
# PR #230 — PF-LP-12: add localhost preview dev runbook

## Evidence snapshot

| Field | Value |
|---|---|
| PR | [#230](https://github.com/RayWong1990/ScoutFlow/pull/230) |
| Merged at | `2026-05-06T10:11:39Z` |
| Cluster | `C05 PF-LP Run-2 / Window-2 / PF-GLOBAL / PF-C3` |
...
Added localhost preview dev runbook for backend/H5 startup, environment setup, manual evidence capture, and stop-lines; output remains shape-only and preview-only.
...
The chosen decision was to merge **PF-LP-12: add localhost preview dev runbook** as a boundary item inside **C05 PF-LP Run-2 / Window-2 / PF-GLOBAL / PF-C3**. The operational payload, as observed from the PR metadata/body summary, was: Added localhost preview dev runbook for backend/H5 startup, environment setup, manual evidence capture, and stop-lines; output remains shape-only and preview-only.

## Rationale

The rationale is not simply “ship the file”. In ScoutFlow terms, the PR turns a loose memory item into a reviewable artifact while maintaining candidate/not-authority separation. This is why the atlas marks the impact as `multi-cluster` and attribution as `introduced`. When the PR introduced a new candidate, the value was future-searchability and controlled continuation. When it exposed a gap, the value was traceability: a later auditor can see which claim was narrowed, which evidence was downgraded, and which boundary remained intact. The recurring design choice is to prefer an explicit partial truth over an implicit complete-sounding claim.

## Outcome and downstream signal

The immediate outcome is a merged checkpoint in the PR ledger at `2026-05-06T10:11:39Z`. Downstream, this card should be read together with linked patterns `preview_only_before_write` and with the amend chain `{'amends': [], 'amended_by': [], 'related_prs': []}`. No U11 anti-pattern ID is attached because the U11 registry was not available in this build; inventing AP IDs would make the atlas less useful. The package therefore records a cross-link gap rather than pretending the registry was loaded.
