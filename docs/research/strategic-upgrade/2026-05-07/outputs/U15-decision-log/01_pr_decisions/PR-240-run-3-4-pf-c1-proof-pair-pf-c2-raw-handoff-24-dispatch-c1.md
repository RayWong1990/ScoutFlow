---
decision_id: "PR-240-run-3-4-pf-c1-proof-pair-pf-c2-raw-handoff-24-dispatch-c1-pass-c2-partia"
status: "candidate / decision-atlas / not-authority"
authority: "not-authority"
pr_number: 240
pr_exists_verified: true
merged: true
pr_url: "https://github.com/RayWong1990/ScoutFlow/pull/240"
pr_title: "Run-3+4: PF-C1 proof pair + PF-C2 RAW handoff (24 dispatch / C1 pass / C2 partial pending RAW intake)"
merged_at: "2026-05-06T16:03:36Z"
cluster: "C09 Run-3+4 Combined Closeout"
decision_kind: "other"
impact_radius: "cross-phase"
introduced_or_exposed: "both"
attribution_confidence: "high"
source_depth: "live GitHub PR metadata and PR body summary inspected in this run"
amend_chain: {"amends": [], "amended_by": [], "related_prs": [239]}
linked_patterns: ["single_pr_compressed_closeout", "partial_signal_preservation", "human_in_loop_gate"]
linked_anti_patterns: []
canonical_decision_log_unchanged: true
---

# PR #240 — Run-3+4: PF-C1 proof pair + PF-C2 RAW handoff (24 dispatch / C1 pass / C2 partial pending RAW intake)

## Evidence snapshot

| Field | Value |
|---|---|
| PR | [#240](https://github.com/RayWong1990/ScoutFlow/pull/240) |
| Merged at | `2026-05-06T16:03:36Z` |
| Cluster | `C09 Run-3+4 Combined Closeout` |
| Decision kind | `other` |
| Impact radius | `cross-phase` |
| Introduced/exposed | `both` |
...
Combined Run-3 PF-C1 and Run-4 PF-C2 single-PR closeout: C1 pass, C2 partial, can_open_c4 false, ready_for_run_5 pending PF-V handoff; 24 dispatch local worktree-only until merge with RAW staged in repo for future enrichment.
...
The chosen decision was to merge **Run-3+4: PF-C1 proof pair + PF-C2 RAW handoff (24 dispatch / C1 pass / C2 partial pending RAW intake)** as a other item inside **C09 Run-3+4 Combined Closeout**. The operational payload, as observed from the PR metadata/body summary, was: Combined Run-3 PF-C1 and Run-4 PF-C2 single-PR closeout: C1 pass, C2 partial, can_open_c4 false, ready_for_run_5 pending PF-V handoff; 24 dispatch local worktree-only until merge with RAW staged in repo for future enrichment.

## Rationale

The rationale is not simply “ship the file”. In ScoutFlow terms, the PR turns a loose memory item into a reviewable artifact while maintaining candidate/not-authority separation. This is why the atlas marks the impact as `cross-phase` and attribution as `both`. When the PR introduced a new candidate, the value was future-searchability and controlled continuation. When it exposed a gap, the value was traceability: a later auditor can see which claim was narrowed, which evidence was downgraded, and which boundary remained intact. The recurring design choice is to prefer an explicit partial truth over an implicit complete-sounding claim.

## Outcome and downstream signal

The immediate outcome is a merged checkpoint in the PR ledger at `2026-05-06T16:03:36Z`. Downstream, this card should be read together with linked patterns `single_pr_compressed_closeout, partial_signal_preservation, human_in_loop_gate` and with the amend chain `{'amends': [], 'amended_by': [], 'related_prs': [239]}`. No U11 anti-pattern ID is attached because the U11 registry was not available in this build; inventing AP IDs would make the atlas less useful. The package therefore records a cross-link gap rather than pretending the registry was loaded.


## Anchor closeout detail

PR #240 closes Run-3 and Run-4 as one combined PR under user pre-authorization. The key verdict is deliberately mixed: C1 pass, C2 partial, `can_open_c4=false`, and `ready_for_run_5=yes_pending_pf_v_handoff`. That is a high-value truthfulness pattern because a combined PR can easily hide partial results. Here the body preserves the partial C2 state and blocks C4 hardening until C2 full readiness or PF-V handoff.

The execution mode is also important: 24 dispatches were held in a local worktree until merge, per-dispatch PRs were held back to preserve honest partial signal, and RAW transfer to `~/workspace/raw/` was skipped while staging placeholder enrichment in repo for future use. This is a decision about evidence hygiene, not just delivery speed. The PR keeps authority files untouched, preserves `write_enabled=False`, stays inside allowed paths, and avoids browser automation/migration/tool redlines.
...
- Pattern files: `02_patterns/PATTERN-single_pr_compressed_closeout.md`, `02_patterns/PATTERN-partial_signal_preservation.md`, `02_patterns/PATTERN-human_in_loop_gate.md`
- Amend chain: `{"amends": [], "amended_by": [], "related_prs": [239]}`
