...
本 node 的任务，是把 `E-SCOUTFLOW` 从跨 session 的散点记忆里抽出来，作为一个可查、可演化、可追溯的知识节点。它不是新 authority，也不替代 ScoutFlow 的 `docs/current.md`、`docs/task-index.md`、`docs/decision-log.md`、PRD/SRD、RAW 规则或原始 handoff；它只把已经可见的证据、用户口径、候选报告与 live readback 组织成一个 supplementary view。ScoutFlow 是 single-user、local-first 的采集、证据、receipt、Trust Trace 与 preview/proof 系统；当前不是企业 KM，也不是未经授权的 runtime 下载器。

## 2. What

`ScoutFlow / 采集线` 在本 atlas 中的定义是：一个与 `entity` 相关的记忆事实/模式/教训/引用单元。它的 claim label 是 `canonical + candidate synthesis`，因此阅读时必须区分三层：第一，来自 repo authority 或已经合并决策的 canonical/promoted 事实；第二，来自 Dispatch127-176、外部六份报告、post176 输出的 candidate synthesis；第三，来自本次任务 prompt 或不可访问本机 memory 的 unavailable placeholder。凡是第三类，都不能被写成确定历史。
...
- `02_repo_sources/docs/PRD-v2-2026-05-04.md:L9` — > 当前角色：ScoutFlow 的产品基线；历史长文与 amendment trail 全部保留在 `docs/archive/`
- `02_repo_sources/docs/PRD-v2-2026-05-04.md:L15` — - 项目名：`ScoutFlow`
- `02_repo_sources/docs/PRD-v2-2026-05-04.md:L16` — - 项目根：`/Users/wanglei/workspace/ScoutFlow/`
...
对于实际推进，`E-SCOUTFLOW` 最常见的错误用法是把它当成“已经批准”或“已经完成”的证明。正确用法是把它变成决策前的 checklist：是否有 user gate，是否有 external audit，是否有 CI/validation，是否与 current/task-index 一致，是否引入 runtime/migration/frontend/vault true-write 漂移，是否让 RAW 与 ScoutFlow 的 SoR 关系变得模糊。

## 7. Linked

本节点在图谱中的主要邻居包括：E-BRIDGE-THIN-API, E-H5-CAPTURE-STATION, E-RAW-VAULT, E-RECEIPT-LEDGER, E-TRUST-TRACE, E-VAULTWRITER, T-AUTHORITY-FIRST, T-CANDIDATE-NOT-AUTHORITY。这些链接经过双向化处理：如果 A 指向 B，B 的 frontmatter 也会包含 A。图谱链接代表“需要一起读”，不是因果证明。比如 theme 可以连接 lesson，lesson 可以连接 pattern，pattern 可以连接 milestone；这只是为了降低冷启动成本，不代表所有节点有同等 authority 级别。
...
如果忘记 `ScoutFlow / 采集线`，下一次 session 很可能会发生以下连锁：先误读来源层级，再误判当前 gate，最后把本来可以快推的 proof 变成大包重排，或把本来需要停线的 runtime/migration/authority 变成“似乎已经被前面批准”。这正是 cross-session memory graph 要解决的问题：不是让模型背更多历史，而是让模型在启动时就知道哪些历史必须谨慎、哪些资产可以复用、哪些内容必须重新 readback。

本节点的最短复述是：ScoutFlow 是 single-user、local-first 的采集、证据、receipt、Trust Trace 与 preview/proof 系统；当前不是企业 KM，也不是未经授权的 runtime 下载器。。在用户要求“深度复盘研究”时，应把它作为证据化复盘的一部分；在用户要求“继续执行”时，应把它作为边界检查的一部分；在用户要求“路线怎么接”时，应把它作为判断 post176 主线是否聚焦 product proof 的一部分。
