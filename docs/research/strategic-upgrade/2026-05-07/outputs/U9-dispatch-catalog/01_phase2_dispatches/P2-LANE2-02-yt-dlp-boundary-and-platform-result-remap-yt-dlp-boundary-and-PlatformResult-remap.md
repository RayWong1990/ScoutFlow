---
title: P2-LANE2-02-yt-dlp-boundary-and-platform-result-remap — yt-dlp boundary and PlatformResult remap
...
owner_tool: GPT Pro external audit / Codex Desktop
...
prerequisites:
  - overflow lane runtime_tools still Hold
  - C-BBD-001 draft-only acknowledged
  - P2-LANE2-01-bbdown-metadata-only-legal-recheck
blocks:
  - downstream dispatches must cite this output before expanding scope
verification_method: external audit required for any concern
risk_level: high
---

[claim]
# P2-LANE2-02-yt-dlp-boundary-and-platform-result-remap — yt-dlp boundary and PlatformResult remap

[claim]
## §1 Mission

[candidate] 把 yt-dlp 可能触达的平台结果重新映射到 PlatformResult 与 local preflight，避免 executable_not_found 被误写为平台失败。
...
[boundary] Lane-specific boundary: runtime_tools 仍处 Hold；本任务只能做 legal / preflight / sandbox / fixture 文案，不得运行 BBDown live、yt-dlp、ffmpeg、ASR 或 audio_transcript。
...
2. **Pass 2 — scope proof** — Map this task to PF-O1 / LANE2 and identify exactly which prior proof or partial result is being used; never infer a runtime gate from a candidate doc.
...
[boundary]
- `docs/research/post-frozen/phase2/lane2/yt-dlp-boundary-platform-result-remap.md`
- `docs/research/post-frozen/phase2/lane2/runtime-risk-table.md`
...
[output]
```yaml
dispatch_id: P2-LANE2-02-yt-dlp-boundary-and-platform-result-remap
...
[verification] Audit window: external audit required for any concern. A 3-window audit should compare (1) prompt intent, (2) repo diff, and (3) receipt/readback. Informal audit is acceptable only for docs-only low-risk artifacts and must still report limitations.
...
[output]
```yaml
P2-LANE2-02-yt-dlp-boundary-and-platform-result-remap_COMPLETE: <true|false>
verdict: <clear|concern|reject|partial>
files_count: <int>
files_changed:
  - <path>
