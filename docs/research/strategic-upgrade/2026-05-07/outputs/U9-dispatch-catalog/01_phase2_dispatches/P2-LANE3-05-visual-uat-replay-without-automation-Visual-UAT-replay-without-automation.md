---
title: P2-LANE3-05-visual-uat-replay-without-automation — Visual UAT replay without automation
...
owner_tool: Codex Desktop / external audit reviewer
...
prerequisites:
  - overflow lane browser_automation still Hold
  - Run-2 synthetic UAT partial acknowledged
  - P2-LANE3-04-login-required-gate-and-consent
blocks:
  - downstream dispatches must cite this output before expanding scope
verification_method: human visual verdict + external audit
risk_level: high
---

[claim]
# P2-LANE3-05-visual-uat-replay-without-automation — Visual UAT replay without automation

[claim]
## §1 Mission

[candidate] 设计 visual UAT replay 的人工 readback 方法，复用 Run-2 synthetic partial 教训，不把 partial 叫成 pass。
...
[boundary] Lane-specific boundary: browser_automation 仍处 Hold；本任务只能做 screenshot packet、profile isolation、human verdict 文案，不得启动 Playwright 自动化或读取登录态。
...
2. **Pass 2 — scope proof** — Map this task to PF-C4 / LANE3 and identify exactly which prior proof or partial result is being used; never infer a runtime gate from a candidate doc.
...
[claim]
- `docs/research/post-frozen/phase2/lane3/visual-uat-replay-without-automation.md`
- `docs/research/post-frozen/phase2/lane3/visual-gate-readback.md`
...
[output]
```yaml
dispatch_id: P2-LANE3-05-visual-uat-replay-without-automation
...
[verification] Audit window: human visual verdict + external audit. A 3-window audit should compare (1) prompt intent, (2) repo diff, and (3) receipt/readback. Informal audit is acceptable only for docs-only low-risk artifacts and must still report limitations.
...
[output]
```yaml
P2-LANE3-05-visual-uat-replay-without-automation_COMPLETE: <true|false>
verdict: <clear|concern|reject|partial>
files_count: <int>
files_changed:
  - <path>
