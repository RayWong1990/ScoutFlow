---
title: P2-LANE4-03-single-migration-script-dry-run — Single migration script dry-run
...
owner_tool: Codex Desktop / GPT Pro contract reviewer
...
prerequisites:
  - overflow lane dbvnext_migration still Hold
  - services/api/migrations remains forbidden
  - P2-LANE4-02-schema-evolution-v0-candidate
blocks:
  - downstream dispatches must cite this output before expanding scope
verification_method: external migration audit required
risk_level: high
---

[claim]
# P2-LANE4-03-single-migration-script-dry-run — Single migration script dry-run

[claim]
## §1 Mission

[candidate] 设计 single migration script 的 dry-run contract、fixtures、rollback checksum 与 audit fields，但保持 migration gate closed。
...
[boundary] Lane-specific boundary: dbvnext_migration 仍处 Hold；本任务只能做 fixture / rollback / audit 文案，不得写 services/api/migrations/** 或改变 schema authority。
...
2. **Pass 2 — scope proof** — Map this task to PF-C3 / LANE4 and identify exactly which prior proof or partial result is being used; never infer a runtime gate from a candidate doc.
...
[boundary]
- `docs/research/post-frozen/phase2/lane4/single-migration-script-dry-run.md`
- `docs/specs/post-frozen/dbvnext-candidate-fixture-index.md`
...
[output]
```yaml
dispatch_id: P2-LANE4-03-single-migration-script-dry-run
...
[verification] Audit window: external migration audit required. A 3-window audit should compare (1) prompt intent, (2) repo diff, and (3) receipt/readback. Informal audit is acceptable only for docs-only low-risk artifacts and must still report limitations.
...
[output]
```yaml
P2-LANE4-03-single-migration-script-dry-run_COMPLETE: <true|false>
verdict: <clear|concern|reject|partial>
files_count: <int>
files_changed:
  - <path>
