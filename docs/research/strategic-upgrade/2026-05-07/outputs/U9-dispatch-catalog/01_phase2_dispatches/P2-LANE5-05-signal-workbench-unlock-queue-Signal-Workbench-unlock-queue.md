---
title: P2-LANE5-05-signal-workbench-unlock-queue — Signal Workbench unlock queue
...
prerequisites:
  - overflow lane full_signal_workbench still Hold
  - C1 pass but C2 partial acknowledged
  - P2-LANE5-04-lp001-risk-recheck
blocks:
  - downstream dispatches must cite this output before expanding scope
verification_method: informal + 3-window if scope expands
risk_level: medium
---

[claim]
# P2-LANE5-05-signal-workbench-unlock-queue — Signal Workbench unlock queue

[claim]
## §1 Mission

[candidate] 把 full_signal_workbench 的后续打开顺序写成 candidate queue，所有条目必须带 can_open_* false 与 human gate。
...
[boundary] Lane-specific boundary: full_signal_workbench 仍处 Hold；本任务只能做 read-only / candidate UI / usefulness verdict 文案，不得绕过 LP-001 直接创建 capture。
...
2. **Pass 2 — scope proof** — Map this task to PF-C1 / LANE5 and identify exactly which prior proof or partial result is being used; never infer a runtime gate from a candidate doc.
...
[claim]
- `docs/research/post-frozen/phase2/lane5/signal-workbench-unlock-queue.md`
- `docs/research/post-frozen/phase2/lane5/workbench-readonly-index.md`
...
[output]
```yaml
dispatch_id: P2-LANE5-05-signal-workbench-unlock-queue
...
[verification] Audit window: informal + 3-window if scope expands. A 3-window audit should compare (1) prompt intent, (2) repo diff, and (3) receipt/readback. Informal audit is acceptable only for docs-only low-risk artifacts and must still report limitations.
...
[output]
```yaml
P2-LANE5-05-signal-workbench-unlock-queue_COMPLETE: <true|false>
verdict: <clear|concern|reject|partial>
files_count: <int>
files_changed:
  - <path>
