---
title: P3-Hypothesis-04-conflict-resolution-rubric — Hypothesis conflict resolution rubric
...
prerequisites:
  ...
  - P2-LANE5-04-lp001-risk-recheck
  - Hypothesis v0 contract readback
  - P3-Hypothesis-03-comparison-state-machine-test
blocks:
  - downstream dispatches must cite this output before expanding scope
verification_method: contract audit + fixture parse
risk_level: medium
---

[claim]
# P3-Hypothesis-04-conflict-resolution-rubric — Hypothesis conflict resolution rubric

[claim]
## §1 Mission

[candidate] 生成 hypothesis conflict resolution rubric，明确 human reviewer 如何处理 competing interpretations。
...
[boundary] Lane-specific boundary: Hypothesis v0→v1 只允许 candidate contract 与 evidence matrix，不得改变 capture state words。
...
2. **Pass 2 — scope proof** — Map this task to PF-C3 / Hypothesis and identify exactly which prior proof or partial result is being used; never infer a runtime gate from a candidate doc.
...
[claim]
- `docs/research/post-frozen/phase3/hypothesis/04-conflict-resolution-rubric.md`
- `docs/specs/post-frozen/hypothesis-v1-candidate-contract.md`
...
[output]
```yaml
dispatch_id: P3-Hypothesis-04-conflict-resolution-rubric
...
[verification] Audit window: contract audit + fixture parse. A 3-window audit should compare (1) prompt intent, (2) repo diff, and (3) receipt/readback. Informal audit is acceptable only for docs-only low-risk artifacts and must still report limitations.
...
[output]
```yaml
P3-Hypothesis-04-conflict-resolution-rubric_COMPLETE: <true|false>
verdict: <clear|concern|reject|partial>
files_count: <int>
files_changed:
  - <path>
