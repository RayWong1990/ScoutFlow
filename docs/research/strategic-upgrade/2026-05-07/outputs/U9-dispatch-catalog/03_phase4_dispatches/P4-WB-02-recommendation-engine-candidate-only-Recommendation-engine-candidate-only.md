---
title: P4-WB-02-recommendation-engine-candidate-only — Recommendation engine candidate-only
...
prerequisites:
  - P3-TopicCard-05-human-edit-cost-regression
  - P3-Signal-04-signal-scoring-vocab-promotion
  - P2-LANE5-05-signal-workbench-unlock-queue
  - P4-WB-01-signal-list-readonly-preview
blocks:
  - downstream dispatches must cite this output before expanding scope
verification_method: 3-window audit after dry-run
risk_level: medium
---

[claim]
# P4-WB-02-recommendation-engine-candidate-only — Recommendation engine candidate-only

[claim]
## §1 Mission

[candidate] 把 recommendation engine 限定为 candidate scoring matrix，不连接 runtime、不写 queue、不绕过 LP-001。
...
[boundary] Lane-specific boundary: Signal Workbench full mode 仍处 Hold；本任务只能排 read-only proof 与 guard，不得引入 ranking runtime。
...
2. **Pass 2 — scope proof** — Map this task to PF-O1 / WB and identify exactly which prior proof or partial result is being used; never infer a runtime gate from a candidate doc.
...
[claim]
- `docs/research/post-frozen/phase4/workbench/recommendation-engine-candidate-only.md`
- `docs/research/post-frozen/phase4/README-readback-index.md`
...
[output]
```yaml
dispatch_id: P4-WB-02-recommendation-engine-candidate-only
...
[verification] Audit window: 3-window audit after dry-run. A 3-window audit should compare (1) prompt intent, (2) repo diff, and (3) receipt/readback. Informal audit is acceptable only for docs-only low-risk artifacts and must still report limitations.
...
[output]
```yaml
P4-WB-02-recommendation-engine-candidate-only_COMPLETE: <true|false>
verdict: <clear|concern|reject|partial>
files_count: <int>
files_changed:
  - <path>
