---
title: MOD-STATE-03-state-word-drift-audit — State word drift audit
...
prerequisites:
  ...
  - MOD-STATE-02-five-gate-automation
blocks:
...
[claim]
# MOD-STATE-03-state-word-drift-audit — State word drift audit

[claim]
## §1 Mission

[candidate] 设计 state-word drift audit，检查 candidate prompt 是否误改 capture lifecycle。
...
[boundary] Lane-specific boundary: U7 state-library module 只做 vocabulary / guard / drift audit，不得改 existing state words。
...
2. **Pass 2 — scope proof** — Map this task to state-library / STATE and identify exactly which prior proof or partial result is being used; never infer a runtime gate from a candidate doc.
...
[audit]
- `docs/research/post-frozen/modules/state/state-word-drift-audit.md`
...
[output]
```yaml
dispatch_id: MOD-STATE-03-state-word-drift-audit
...
[output]
```yaml
MOD-STATE-03-state-word-drift-audit_COMPLETE: <true|false>
verdict: <clear|concern|reject|partial>
files_count: <int>
files_changed:
  - <path>
