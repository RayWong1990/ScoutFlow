---
title: retrieval CLUSTER INDEX — U6 retrieval module
...
[claim]
# retrieval Cluster Index — U6 retrieval module

[claim]
## §1 Mission

[candidate] 管理 visual-DAM、hybrid search、freshness router 与 citation packaging。
...
|---|---|---|---|---|
| `MOD-RETRIEVAL-01-visual-dam-object-contract` | Module | RETRIEVAL | Visual DAM object contract | `U4-U8 strategic-upgrade prompt/output ZIP manually reviewed if available`, `PRD/SRD v3 candidate not promoted unless user says so` |
| `MOD-RETRIEVAL-02-hybrid-search-fixture` | Module | RETRIEVAL | Hybrid search fixture | `U4-U8 strategic-upgrade prompt/output ZIP manually reviewed if available`, `PRD/SRD v3 candidate not promoted unless user says so` |
| `MOD-RETRIEVAL-03-source-freshness-router` | Module | RETRIEVAL | Source freshness router | `U4-U8 strategic-upgrade prompt/output ZIP manually reviewed if available`, `PRD/SRD v3 candidate not promoted unless user says so` |
| `MOD-RETRIEVAL-04-citation-packaging-contract` | Module | RETRIEVAL | Citation packaging contract | `U4-U8 strategic-upgrade prompt/output ZIP manually reviewed if available`, `PRD/SRD v3 candidate not promoted unless user says so` |

[diagram]
```mermaid
flowchart TD
  retrieval --> MOD_RETRIEVAL_01_visual_dam_object_contract
  retrieval --> MOD_RETRIEVAL_02_hybrid_search_fixture
  retrieval --> MOD_RETRIEVAL_03_source_freshness_router
  retrieval --> MOD_RETRIEVAL_04_citation_packaging_contract
  retrieval --> AUDIT_retrieval[readback + boundary audit]
```
...
[claim]
[detail] retrieval sequencing guard: the next worker should prefer a smaller concern report over a larger speculative artifact when evidence is missing. ScoutFlow's current discipline rewards accurate partial states because they prevent unsafe lane expansion.
