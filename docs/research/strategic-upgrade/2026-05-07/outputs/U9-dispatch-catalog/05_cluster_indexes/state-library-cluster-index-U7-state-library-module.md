---
title: state-library CLUSTER INDEX — U7 state library module
status: candidate / cluster-index / not-authority
authority: not-authority
created_at: 2026-05-07
cluster: state-library
...
[claim]
# state-library Cluster Index — U7 state library module

[claim]
## §1 Mission

[candidate] 管理 state words、5-gate automation 与 drift audit，不改现有 lifecycle。
...
|---|---|---|---|---|
| `MOD-STATE-01-state-library-contract` | Module | STATE | State library contract | `U4-U8 strategic-upgrade prompt/output ZIP manually reviewed if available`, `PRD/SRD v3 candidate not promoted unless user says so` |
| `MOD-STATE-02-five-gate-automation` | Module | STATE | Five-gate automation | `U4-U8 strategic-upgrade prompt/output ZIP manually reviewed if available`, `PRD/SRD v3 candidate not promoted unless user says so` |
| `MOD-STATE-03-state-word-drift-audit` | Module | STATE | State word drift audit | `U4-U8 strategic-upgrade prompt/output ZIP manually reviewed if available`, `PRD/SRD v3 candidate not promoted unless user says so` |

[diagram]
```mermaid
flowchart TD
  state_library --> MOD_STATE_01_state_library_contract
  state_library --> MOD_STATE_02_five_gate_automation
  state_library --> MOD_STATE_03_state_word_drift_audit
  state_library --> AUDIT_state_library[readback + boundary audit]
```
...
[claim]
[detail] state-library sequencing guard: the next worker should prefer a smaller concern report over a larger speculative artifact when evidence is missing. ScoutFlow's current discipline rewards accurate partial states because they prevent unsafe lane expansion.
