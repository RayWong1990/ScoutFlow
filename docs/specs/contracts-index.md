...
| apps/capture-station/src/features/topic-card-vault/** + apps/capture-station/src/features/topic-card-preview/** | PR #161 / #162 bounded frontend candidates | bounded candidate surfaces；fresh-worktree local validation reproduced via `npm install --no-package-lock` + `npm test` + `npm run build`；bounded screenshot packet + Playwright execution evidence collected in raw local packet only；not global visual terminal verdict；not package strategy approval；not runtime、vault-write、or frontend execution approval |
| docs/current.md + docs/task-index.md + docs/decision-log.md | Dispatch173 / 174 authority writebacks landed | Wave 5 candidate chain closed and Wave 6 candidate lane opened；not execution approval；not runtime approval；no code-bearing next gate |
| docs/research/wave5/overflow-candidate-registry-for-db-vnext-and-blocked-runtime-lanes-2026-05-05.md + docs/research/wave5/step3-cold-start-handoff-packet-contract-2026-05-05.md | Dispatch175 / 176 landed Wave 6 overflow/handoff surfaces on `main` | candidate overflow/handoff surfaces only；not execution approval；not runtime approval；not migration approval |
