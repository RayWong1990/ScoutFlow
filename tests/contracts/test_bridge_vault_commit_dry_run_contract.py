...
def test_vault_commit_route_stays_dry_run(tmp_path: Path, monkeypatch) -> None:
...
def test_vault_commit_route_requires_vault_root(tmp_path: Path, monkeypatch) -> None:
...
def test_vault_commit_route_rejects_unknown_capture(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("SCOUTFLOW_VAULT_ROOT", "/tmp/scoutflow-vault")
    client = build_client(tmp_path)

    response = client.post("/captures/cap_missing/vault-commit")
