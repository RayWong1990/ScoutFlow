...
def test_render_commit_ledger_markdown_matches_gate_message() -> None:
    from scoutflow_api.vault.commit import WRITE_DISABLED_MESSAGE, render_commit_ledger_markdown
    from scoutflow_api.vault.schemas import VaultCommitDryRun

    markdown = render_commit_ledger_markdown(
        VaultCommitDryRun(
            capture_id="cap_123",
            target_path="/tmp/scoutflow-vault/00-Inbox/scoutflow-cap_123-bv1xx411c7md.md",
        )
